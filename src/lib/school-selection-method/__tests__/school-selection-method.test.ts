...
describe('T-Y14 学校・学科別入学者選抜の評価方法', () => {
  ...
  it('nara: 令和9年度の二次選抜は各高校での検査(面接)なし・奈良北は一次選抜の3教科得点が300・奈良商工は150、第2希望校の奈良は3教科240点', () => {
    const r2 = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'nara', '奈良北', '二次選抜', '普通科');
    expect(r2?.ratioType).toBe('一次選抜学力検査(3教科)300:調査書144');
    expect(r2?.interviewRequired).toBe(false);
    const shoko = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'nara', '奈良商工', '二次選抜', '観光科');
    expect(shoko?.ratioType).toBe('一次選抜学力検査(3教科)150:調査書234');
    expect(shoko?.note).toContain('令和9年度は各高校での二次選抜の検査(面接等)を実施しない');
  ...
  it('aomori: schoolsは青森県内全6地区(東青・西北五・中弘南黒・上十三・下北むつ・三八)の分割版を完全収録している(44校177レコード・令和9年度版)', () => {
  ...
  it('shizuoka: schoolsは令和9年度PDF全28頁(全日制86校+単位制定時制ふじのくに国際)の87校263レコードの学校裁量枠を収録している(南伊豆分校・松崎・稲取・土肥分校・沼津東理数・清水東理数・磐田南理数・浜松北普通・春野校舎は設定なしのため対象外)', () => {
    const record = getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'shizuoka');
    expect(record?.schools?.length).toBe(263);
    const schoolNames = new Set(record?.schools?.map((s) => s.schoolName));
    expect(schoolNames).toEqual(new Set(['下田', '伊豆伊東', '熱海', '伊豆総合', '韮山', '伊豆中央', '田方農業', '三島南', '三島北', '御殿場', '御殿場南', '小山', '裾野', '沼津東', '沼津西', '沼津城北', '沼津工業', '沼津商業', '市立沼津', '吉原', '吉原工業', '富士', '富士東', '富士宮東', '富士宮北', '富士宮西', '富岳館', '富士市立', '清水東', '清水西', '清水南', '静岡市立清水桜が丘', '静岡', '静岡城北', '静岡東', '静岡西', '駿河総合', '静岡農業', '科学技術', '静岡商業', '静岡市立', '焼津中央', '焼津水産', '清流館', '藤枝東', '藤枝西', '藤枝北', '島田', '島田工業', '島田商業', '川根', '榛原', '相良', '掛川東', '掛川西', '掛川工業', '横須賀', '池新田', '小笠', '遠江総合', '袋井', '袋井商業', '磐田南', '磐田北', '磐田農業', '磐田西', '天竜', '浜松北', '浜松西', '浜松南', '浜松湖東', '浜松湖南', '浜松江之島', '浜松東', '浜松大平台', '浜松工業', '浜松城北工業', '浜松商業', '浜名', '浜北西', '浜松湖北', '浜松湖北佐久間分校', '新居', '湖西', '浜松市立', 'ふじのくに国際']));
  });
  ...
  it('shizuoka: 出典は令和9年度版PDF(1082226ページ配下のr9〜)であり令和5年度版(1031855配下)ではない(2026-09-19に取り違えを訂正)', () => {
    const record = getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'shizuoka');
    expect(record?.source?.url).toContain('/001/082/226/r9');
    expect(record?.source?.url).not.toContain('/001/031/855/');
    // 令和9年度の下田Ⅰは13%(令和5年度版の8%とは異なる)
    expect(findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'shizuoka', '下田', '学校裁量枠Ⅰ', '普通')?.ratioType).toBe('選抜割合13%程度(希望者対象)');
  });
  ...
  it('tokyo: n2_16頁1-3の文化・スポーツ等特別推薦は9校25レコード(頁1-3)+頁4-9の合計で、種目・募集人数・満点(調査書+面接+実技)・数値目標を持つ', () => {
    ...
    // 頁34-36
    expect(by('田無工科')).toHaveLength(2);
    expect(by('田無工科').every((s) => s.ratioType === '調査書200点+個人面接400点+実技検査200点')).toBe(true);
    expect(by('芝商業')).toHaveLength(2);
    expect(by('芝商業').every((s) => s.ratioType === '調査書300点+個人面接100点+実技検査200点')).toBe(true);
    ...
    expect(by('つばさ総合')).toHaveLength(3);
    expect(by('つばさ総合').filter((s) => s.ratioType.includes('集団面接100点')).map((s) => s.department.split('(')[0])).toEqual(['陸上競技']);
    expect(by('つばさ総合').find((s) => s.department.startsWith('美術'))?.note).toContain('65×50cm以内');
    expect(by('世田谷総合')).toHaveLength(2);
    expect(by('世田谷総合').every((s) => s.ratioType === '調査書500点+個人面接200点+実技検査300点')).toBe(true);
    // 頁37-39(最終)
    ...
    for (const s of list) {
      const nums = [...s.ratioType.matchAll(/([0-9]+)点/g)].map((m) => Number(m[1]));
  ...
  it('fukushima: 郡山商業の特色選抜はA型375点・B型330点+実技45点でいずれも満点625点・安積黎明はA型(プレゼン)/B型(実技)とも特色検査100点で満点620点', () => {
    const g = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '郡山商業', '特色選抜', '商業科・会計科');
    expect(g?.ratioType).toBe('A型=学力検査250点:調査書375点/B型=学力検査250点:調査書330点:実技45点');
    expect(g?.note).toContain('選抜資料の満点はA型・B型とも625点');
    expect(250 + 375).toBe(625);
    expect(250 + 330 + 45).toBe(625);
  ...
  it('fukushima: 須賀川創英館(Ⅰ型405点/Ⅱ型270点+実技135点=655点)と清陵情報(A型300点/B型295点+実技100点=550点/645点)の型ごとの満点が資料と一致する', () => {
    const s1 = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '須賀川創英館', '特色選抜', '普通科');
    expect(s1?.note).toContain('選抜資料の満点はⅠ型・Ⅱ型とも655点');
    expect(250 + 405).toBe(655);
    expect(250 + 270 + 135).toBe(655);
    const c = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '清陵情報', '特色選抜', '工業科・情報電子科');
    expect(c?.note).toContain('選抜資料の満点はA型550点・B型645点');
    expect(250 + 300).toBe(550);
    expect(250 + 295 + 100).toBe(645);
    // 清陵情報の一般選抜の募集定員: 情報電子科40・電子機械科80・情報処理科80・情報会計科40
    const q = (d: string) => findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '清陵情報', '一般選抜', d)?.note;
    expect(q('工業科・情報電子科')).toContain('募集定員40人');
    expect(q('工業科・電子機械科')).toContain('募集定員80人');
    expect(q('商業科・情報処理科')).toContain('募集定員80人');
    expect(q('商業科・情報会計科')).toContain('募集定員40人');
  });

  it('fukushima: 光南の特色選抜はA型650点・B型850点(+プレゼン200)・C型1000点(+スキルテスト350)・白河普通科はA/B型500点・C型550点(実技100点)で型ごとの満点が資料と一致する', () => {
    const k = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '光南', '特色選抜', '総合学科');
    expect(k?.note).toContain('選抜資料の満点はA型650点・B型850点・C型1000点');
    expect(250 + 250 + 150).toBe(650);
    expect(650 + 200).toBe(850);
    expect(650 + 350).toBe(1000);
    const sh = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '白河', '特色選抜', '普通科');
    expect(sh?.note).toContain('A型・B型が500点・C型が550点');
    expect(250 + 175 + 75).toBe(500);
    expect(250 + 175 + 25 + 100).toBe(550);
    // 岩瀬農業は農業科6学科×3区分=18レコード
    expect((getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima')?.schools ?? []).filter((x) => x.schoolName === '岩瀬農業')).toHaveLength(18);
  });

  it('fukushima: 修明の連携型選抜は全5学科(募集枠50%)でA型=実技90点・B型=面接90点のいずれも満点700点、一般選抜は連携型・特色との併願者に別途一般面接を実施する', () => {
    const recs = (getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima')?.schools ?? []).filter((x) => x.schoolName === '修明');
    expect(recs).toHaveLength(20);
    const renkei = recs.filter((x) => x.selectionCategory === '連携型選抜');
    expect(renkei).toHaveLength(5);
    for (const x of renkei) {
      expect(x.ratioType).toBe('A型=学力検査250点:調査書360点:実技90点/B型=学力検査250点:調査書360点:面接90点');
      expect(x.note).toContain('選抜資料の満点はA型・B型とも700点');
    }
  ...
  it('fukushima: 喜多方の特色選抜はA型(傾斜270+65)・B型(135+200)とも調査書335点で満点750点・一般選抜は学力検査の成績を2倍する比重', () => {
    const t = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '喜多方', '特色選抜', '普通科');
    expect(t?.note).toContain('選抜資料の満点はA型・B型とも750点');
    expect(270 + 65).toBe(335);
    expect(135 + 200).toBe(335);
    expect(250 + 335 + 165).toBe(750);
    expect(findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '喜多方', '一般選抜', '普通科')?.ratioType).toBe('学力検査の成績を2倍する');
    // 喜多方桐桜は4学科(工業3+商業1)×3区分=12レコードで一般選抜の募集定員は各35人
  ...
  it('fukushima: 連携型選抜は修明・南会津・ふたば未来学園・相馬総合の4校のみ(南会津は募集枠60%・面接は段階評価・満点500点)で川口の特色面接は430点(満点930点)', () => {
    const recs = getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima')?.schools ?? [];
    const renkei = recs.filter((x) => x.selectionCategory === '連携型選抜');
    expect([...new Set(renkei.map((x) => x.schoolName))].sort()).toEqual(['ふたば未来学園', '修明', '南会津', '相馬総合']);
  ...
  it('fukushima: 磐城の一般選抜は学力検査の成績を5倍する比重(全県最大)・特色選抜は実技100点で満点550点、平工業は工業科5学科で特色満点615点', () => {
    expect(findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '磐城', '一般選抜', '普通科')?.ratioType).toBe('学力検査の成績を5倍する');
    const t = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '磐城', '特色選抜', '普通科');
    expect(t?.note).toContain('選抜資料の満点は550点');
    expect(250 + 170 + 30 + 100).toBe(550);
    const h = (getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima')?.schools ?? []).filter((x) => x.schoolName === '平工業');
    expect(h).toHaveLength(15);
    expect(h.filter((x) => x.selectionCategory === '特色選抜').every((x) => x.note?.includes('選抜資料の満点は615点'))).toBe(true);
  });

  it('fukushima: いわき総合といわき光洋は特色検査=実技500点で満点1000点・いわき光洋の一般選抜は学力検査の成績を2倍・いわき商業情報は4学科(A型作文/B型実技とも特色検査30点で満点530点)', () => {
    expect(250 + 250 + 500).toBe(1000);
    expect(findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', 'いわき光洋', '一般選抜', '文理科')?.ratioType).toBe('学力検査の成績を2倍する');
    const j = (getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima')?.schools ?? []).filter((x) => x.schoolName === 'いわき商業情報');
    expect(j).toHaveLength(12);
    expect(j.filter((x) => x.selectionCategory === '特色選抜').every((x) => x.note?.includes('選抜資料の満点は530点'))).toBe(true);
    expect(250 + 250 + 30).toBe(530);
  });

  it('fukushima: 小名浜海星は6学科×3区分の18レコードで特色検査260点(A型作文/B型実技)により満点800点・一般選抜の募集定員は普通40/商業40/海洋20/情報通信20/食品システム40/海洋工学40', () => {
    const recs = (getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima')?.schools ?? []).filter((x) => x.schoolName === '小名浜海星');
    expect(recs).toHaveLength(18);
    expect(250 + 190 + 100 + 260).toBe(800);
    const q = (d: string) => findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '小名浜海星', '一般選抜', d)?.note;
    expect(q('普通科')).toContain('募集定員40人');
    expect(q('水産科・海洋科')).toContain('募集定員20人');
    expect(q('水産科・情報通信科')).toContain('募集定員20人');
    expect(q('水産科・海洋工学科')).toContain('募集定員40人');
    // いわき湯本は特色検査400点で満点800点
    expect(250 + 135 + 15 + 400).toBe(800);
  });

  it('fukushima: ふたば未来学園はトップアスリート型が実技500点で満点1000点・アカデミック/スペシャリスト型は面接150点で満点650点(連携型選抜も同様でJFAアカデミー福島は実技免除)、相馬理数科は数学・英語2倍で学力350点', () => {
    expect(250 + 250 + 500).toBe(1000);
    expect(250 + 250 + 150).toBe(650);
    const f = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', 'ふたば未来学園', '特色選抜', '総合学科');
    expect(f?.note).toContain('トップアスリート型が1000点');
    expect(findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', 'ふたば未来学園', '連携型選抜', '総合学科')?.note).toContain('JFAアカデミー福島に該当する者は実技を免除');
    const so = findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '相馬', '特色選抜', '理数科');
    expect(so?.ratioType).toContain('学力検査350点');
    expect(so?.note).toContain('選抜資料の満点は700点');
    expect(350 + 190 + 60 + 100).toBe(700);
  });

  it('fukushima: 原町は特色選抜でA型(学力検査400点・調査書380点)=810点・B型(実技250点)=700点、小高産業技術はA型面接30点/B型面接10点+実技20点でいずれも満点530点、相馬農業は面接200点+作文50点で満点750点', () => {
    expect(400 + 380 + 30).toBe(810);
    expect(250 + 170 + 30 + 250).toBe(700);
    expect(findSchoolSelectionRecord(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima', '原町', '特色選抜', '普通科')?.note).toContain('A型810点・B型700点');
    expect(250 + 250 + 30).toBe(530);
    expect(250 + 250 + 10 + 20).toBe(530);
    const k = (getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'fukushima')?.schools ?? []).filter((x) => x.schoolName === '小高産業技術');
    expect(k).toHaveLength(15);
    expect(k.filter((x) => x.selectionCategory === '特色選抜').every((x) => x.note?.includes('A型・B型とも530点'))).toBe(true);
    expect(250 + 250 + 200 + 50).toBe(750);
  });
  ...
  it('kyoto: 令和9年度「前期選抜独自枠等募集要項」の京都市・乙訓学区26校58レコード＋城陽学区8校18レコード＋相楽学区3校13レコード＋北桑田2レコード＋亀岡4レコード＋南丹3レコード＋園部1レコード＋農芸1レコード＋須知3レコード＋綾部本校2レコード＋綾部東分校2レコード＋福知山本校2レコード＋工業3レコード＋大江1レコード＋東舞鶴1レコード＋西舞鶴3レコード＋海洋1レコード＋宮津天橋宮津学舎2レコード＋宮津天橋加悦谷学舎2レコード＋峰山3レコード＋丹後緑風網野学舎3レコード＋丹後緑風久美浜学舎2レコードを収録し、一覧表と個別学校ページを突合または個別学校ページ直読みのいずれかで検証済みの学校のみに限定する', () => {
    const record = getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'kyoto');
    expect(record?.fiscalYear).toBe('令和9年度（2027年度）');
    const schools = record?.schools ?? [];
    expect(schools).toHaveLength(130);
    expect(new Set(schools.map((x) => x.schoolName)).size).toBe(56);
    ...
    // 相楽学区(後半PDF・頁18-27)は3校13レコード。田辺は普通科3型+専門4学科(各学科15人を
    // 1学科1レコードに分割)の計7レコード
    const tanabe = schools.filter((x) => x.schoolName === '京都府立田辺高等学校');
    expect(tanabe).toHaveLength(7);
    expect(new Set(tanabe.map((x) => x.department))).toEqual(
      new Set(['普通科', 'デザイン科', '機械科', '電気科', '自動車科']),
    );
    const kizu = schools.filter((x) => x.schoolName === '京都府立木津高等学校');
    expect(kizu).toHaveLength(4);
    const nanyo = schools.filter((x) => x.schoolName === '京都府立南陽高等学校');
    expect(nanyo).toHaveLength(2);
    expect(new Set(nanyo.map((x) => x.department))).toEqual(new Set(['普通科', 'サイエンスリサーチ科']));
    // 北桑田(本校)は普通科(京都市・乙訓枠を含む30人)+京都フォレスト科15人の2レコード
    ...
    // 南丹は総合学科(単位制)のA1/A2/Bの3型3レコード(全て同一学科)
    const nantan = schools.filter((x) => x.schoolName === '京都府立南丹高等学校');
    expect(nantan).toHaveLength(3);
    expect(nantan.every((x) => x.department === '総合学科（単位制）')).toBe(true);
    // 園部・農芸はいずれも単一学科A方式のみの1レコード校
    expect(schools.filter((x) => x.schoolName === '京都府立園部高等学校')).toHaveLength(1);
    expect(schools.filter((x) => x.schoolName === '京都府立農芸高等学校')).toHaveLength(1);
    // 須知は普通科(A/Bの2型)+食品科学科(A型)の2学科3レコード
    const suchi = schools.filter((x) => x.schoolName === '京都府立須知高等学校');
    expect(suchi).toHaveLength(3);
    expect(new Set(suchi.map((x) => x.department))).toEqual(new Set(['普通科', '食品科学科']));
    // 綾部(本校)は普通科A方式+普通科(スポーツ総合専攻)C方式の2レコード
    ...
    // 福知山(本校)は普通科A方式+文理科学科A方式の2学科2レコード
    ...
    // 大江は地域創生科(単位制)A方式のみの1レコード校
    expect(schools.filter((x) => x.schoolName === '京都府立大江高等学校')).toHaveLength(1);
    // 東舞鶴は普通科A方式のみの1レコード校
    expect(schools.filter((x) => x.schoolName === '京都府立東舞鶴高等学校')).toHaveLength(1);
    // 西舞鶴は普通科(A1/A2の2型)+理数探究科(A型)の2学科3レコード
    const nishimaizuru = schools.filter((x) => x.schoolName === '京都府立西舞鶴高等学校');
    expect(nishimaizuru).toHaveLength(3);
    expect(new Set(nishimaizuru.map((x) => x.department))).toEqual(new Set(['普通科', '理数探究科']));
    // 海洋は3学科をまとめた「海洋学科群」1レコードのみ
    expect(schools.filter((x) => x.schoolName === '京都府立海洋高等学校')).toHaveLength(1);
    // 宮津天橋(宮津学舎)は普通科(単位制)A方式+建築科(単位制)A方式の2学科2レコード
    const miyazutenkyo = schools.filter((x) => x.schoolName === '京都府立宮津天橋高等学校（宮津学舎）');
    expect(miyazutenkyo).toHaveLength(2);
    expect(new Set(miyazutenkyo.map((x) => x.department))).toEqual(new Set(['普通科（単位制）', '建築科（単位制）']));
    // 宮津天橋(加悦谷学舎)は宮津学舎と別学校として収録・普通科(単位制)A/Bの2方式2レコード
    ...
    // 丹後緑風(網野学舎)は普通科(単位制・A1/A2の2型)+企画経営科(単位制・A方式)の
    // 2学科3レコード
    const taNgoRyokufuAminoGakusha = schools.filter(
      (x) => x.schoolName === '京都府立丹後緑風高等学校（網野学舎）',
    );
    expect(taNgoRyokufuAminoGakusha).toHaveLength(3);
    expect(new Set(taNgoRyokufuAminoGakusha.map((x) => x.department))).toEqual(
      new Set(['普通科（単位制）', '企画経営科（単位制）']),
    );
    // 丹後緑風(久美浜学舎)は網野学舎と別学校として収録・アグリサイエンス科+
    // みらいクリエイト科の2学科2レコード
    const taNgoRyokufuKumihamaGakusha = schools.filter(
      (x) => x.schoolName === '京都府立丹後緑風高等学校（久美浜学舎）',
    );
    expect(taNgoRyokufuKumihamaGakusha).toHaveLength(2);
    expect(new Set(taNgoRyokufuKumihamaGakusha.map((x) => x.department))).toEqual(
      new Set(['アグリサイエンス科（単位制）', 'みらいクリエイト科（単位制）']),
    );
  });
