...
describe('T-Y14 学校・学科別入学者選抜の評価方法', () => {
  ...
  it('tokyo: schoolsは頁1-4の56校(頁1=日比谷/三田/戸山/竹早/向丘/上野/日本橋/本所/城東/東/深川/大崎/小山台・頁2=八潮/駒場/目黒/大森/田園調布/雪谷/桜町/千歳丘/松原/青山/広尾/鷺宮/武蔵丘・頁3=杉並/豊多摩/西/豊島/文京/竹台/板橋/大山/北園/高島/井草/石神井/田柄/練馬・頁4=光丘/青井/足立/足立新田/足立西/江北/淵江/葛飾野/南葛飾/江戸川/葛西南/小岩/小松川/篠崎/紅葉川/片倉)168レコードを収録している(推薦に基づく選抜+第一次募集+第二次募集の3区分)', () => {
    const record = getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'tokyo');
    expect(record?.schools?.length).toBe(168);
    const schoolNames = new Set(record?.schools?.map((s) => s.schoolName));
    expect(schoolNames).toEqual(new Set(['日比谷', '三田', '戸山', '竹早', '向丘', '上野', '日本橋', '本所', '城東', '東', '深川', '大崎', '小山台', '八潮', '駒場', '目黒', '大森', '田園調布', '雪谷', '桜町', '千歳丘', '松原', '青山', '広尾', '鷺宮', '武蔵丘', '杉並', '豊多摩', '西', '豊島', '文京', '竹台', '板橋', '大山', '北園', '高島', '井草', '石神井', '田柄', '練馬', '光丘', '青井', '足立', '足立新田', '足立西', '江北', '淵江', '葛飾野', '南葛飾', '江戸川', '葛西南', '小岩', '小松川', '篠崎', '紅葉川', '片倉']));
    ...
    // 第二次募集で個人面接があるのは八潮300・大森300・雪谷100・杉並300・高島300・練馬300・青井300・南葛飾100・小松川300・片倉200の10校のみ
    const m2 = second.filter((s) => s.interviewRequired === true);
    expect(m2.map((s) => s.schoolName).sort()).toEqual(['八潮', '大森', '雪谷', '杉並', '高島', '練馬', '青井', '南葛飾', '小松川', '片倉'].sort());
    expect(m2.find((s) => s.schoolName === '雪谷')?.ratioType).toBe('学力検査6:調査書4(600点:400点)+個人面接100点');
  });

  it('tokyo: 推薦に基づく選抜は全56校が推薦枠割合20%で、面接・討論の点数と小論文/作文の点数がnoteの合計と一致する(日比谷・竹早は結合セルのため面接実施を断定しない)', () => {
    ...
    expect(list.filter((s) => s.note?.includes('特別推薦の実施あり')).map((s) => s.schoolName).sort()).toEqual(['上野', '向丘', '大崎', '本所', '東', '深川', '城東', '八潮', '目黒', '大森', '雪谷', '桜町', '千歳丘', '広尾', '鷺宮', '武蔵丘', '杉並', '豊多摩', '豊島', '文京', '板橋', '大山', '高島', '石神井', '練馬', '光丘', '足立', '足立新田', '足立西', '淵江', '葛飾野', '南葛飾', '江戸川', '葛西南', '小岩', '篠崎', '紅葉川', '片倉'].sort());
    expect(list.filter((s) => s.note?.includes('特別推薦の実施なし'))).toHaveLength(15 + 3);
