...
describe('T-Y14 学校・学科別入学者選抜の評価方法', () => {
  ...
  it('tokyo: schoolsは頁1-6の87校(頁1=日比谷/三田/戸山/竹早/向丘/上野/日本橋/本所/城東/東/深川/大崎/小山台・頁2=八潮/駒場/目黒/大森/田園調布/雪谷/桜町/千歳丘/松原/青山/広尾/鷺宮/武蔵丘・頁3=杉並/豊多摩/西/豊島/文京/竹台/板橋/大山/北園/高島/井草/石神井/田柄/練馬・頁4=光丘/青井/足立/足立新田/足立西/江北/淵江/葛飾野/南葛飾/江戸川/葛西南/小岩/小松川/篠崎/紅葉川/片倉・頁5=八王子北/八王子東/富士森/松が谷/立川/武蔵野北/多摩/府中/府中西/府中東/昭和/拝島/神代/調布北/調布南・頁6=小川/成瀬/野津田/町田/山崎/小金井北/小平/小平西/小平南/日野/日野台/南平/東村山西/国立/福生/狛江)261レコードを収録している(推薦に基づく選抜+第一次募集+第二次募集の3区分)', () => {
    ...
    // 第二次募集で個人面接があるのは八潮300・大森300・雪谷100・杉並300・高島300・練馬300・青井300・南葛飾100・小松川300・片倉200・八王子北100・小川150・野津田200・小平100・小平西100(以上個人面接)・府中東200・山崎300(以上集団面接)の17校のみ
    const m2 = second.filter((s) => s.interviewRequired === true);
    expect(m2.map((s) => s.schoolName).sort()).toEqual(['八潮', '大森', '雪谷', '杉並', '高島', '練馬', '青井', '南葛飾', '小松川', '片倉', '八王子北', '府中東', '小川', '野津田', '山崎', '小平', '小平西'].sort());
  ...
  it('tokyo: 推薦に基づく選抜は全87校が推薦枠割合20%で、面接・討論の点数と小論文/作文の点数がnoteの合計と一致する(日比谷・竹早は結合セルのため面接実施を断定しない)', () => {
    ...
    expect(list.filter((s) => s.note?.includes('特別推薦の実施あり')).map((s) => s.schoolName).sort()).toEqual(['上野', '向丘', '大崎', '本所', '東', '深川', '城東', '八潮', '目黒', '大森', '雪谷', '桜町', '千歳丘', '広尾', '鷺宮', '武蔵丘', '杉並', '豊多摩', '豊島', '文京', '板橋', '大山', '高島', '石神井', '練馬', '光丘', '足立', '足立新田', '足立西', '淵江', '葛飾野', '南葛飾', '江戸川', '葛西南', '小岩', '篠崎', '紅葉川', '片倉', '八王子北', '富士森', '松が谷', '府中', '府中西', '府中東', '拝島', '小川', '山崎', '小平西', '小平南', '日野', '東村山西', '福生', '狛江'].sort());
    expect(list.filter((s) => s.note?.includes('特別推薦の実施なし'))).toHaveLength(15 + 3 + 8 + 8);
