...
describe('T-Y14 学校・学科別入学者選抜の評価方法', () => {
  ...
  it('kyoto: 令和9年度「前期選抜独自枠等募集要項」の京都市・乙訓学区26校58レコード＋城陽学区8校18レコード＋相楽学区3校13レコード＋北桑田2レコード＋亀岡4レコード＋南丹3レコード＋園部1レコード＋農芸1レコード＋須知3レコード＋綾部本校2レコード＋綾部東分校2レコード＋福知山本校2レコード＋工業3レコード＋大江1レコード＋東舞鶴1レコード＋西舞鶴3レコード＋海洋1レコード＋宮津天橋宮津学舎2レコード＋宮津天橋加悦谷学舎2レコード＋峰山3レコード＋丹後緑風網野学舎3レコードを収録し、一覧表と個別学校ページを突合または個別学校ページ直読みのいずれかで検証済みの学校のみに限定する', () => {
    const record = getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'kyoto');
    expect(record?.fiscalYear).toBe('令和9年度（2027年度）');
    const schools = record?.schools ?? [];
    expect(schools).toHaveLength(128);
    expect(new Set(schools.map((x) => x.schoolName)).size).toBe(55);
