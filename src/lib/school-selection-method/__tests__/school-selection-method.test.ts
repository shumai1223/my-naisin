...
describe('T-Y14 学校・学科別入学者選抜の評価方法', () => {
  ...
  it('shizuoka: schoolsは令和9年度PDF頁1-13(藤枝東まで)の45校132レコードの学校裁量枠を収録している(南伊豆分校・松崎・稲取・土肥分校・沼津東理数・清水東理数は設定なしのため対象外)', () => {
    const record = getSchoolSelectionMethod(SCHOOL_SELECTION_METHOD_BY_PREFECTURE, 'shizuoka');
    expect(record?.schools?.length).toBe(132);
    const schoolNames = new Set(record?.schools?.map((s) => s.schoolName));
    expect(schoolNames).toEqual(new Set(['下田', '伊豆伊東', '熱海', '伊豆総合', '韮山', '伊豆中央', '田方農業', '三島南', '三島北', '御殿場', '御殿場南', '小山', '裾野', '沼津東', '沼津西', '沼津城北', '沼津工業', '沼津商業', '市立沼津', '吉原', '吉原工業', '富士', '富士東', '富士宮東', '富士宮北', '富士宮西', '富岳館', '富士市立', '清水東', '清水西', '清水南', '静岡市立清水桜が丘', '静岡', '静岡城北', '静岡東', '静岡西', '駿河総合', '静岡農業', '科学技術', '静岡商業', '静岡市立', '焼津中央', '焼津水産', '清流館', '藤枝東']));
  });
