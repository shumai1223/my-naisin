...
export const prefectureGuides: Record<string, PrefectureGuide> = {
  tokyo: {
    ...
    // 実際は提出物・授業態度等も含む総合評価であるため、テストの合計という言い切りを避けて
    // 修正。⑤は「主体的態度C評価が評定を下げる要因になる」という仕組み自体は正しいが、
    // 「急増中」という傾向の断定は裏付けが取れなかったため削除し、仕組みの説明に留めた。
    pitfalls: {
      title: '東京都立入試の注意点',
      items: [
        ...
        '中3の2学期（12月）に確定する成績が調査書点になります。評定は定期テストだけでなく提出物・授業態度なども含めた総合評価で、1学期の内容も踏まえて付けられるため、1学期からの積み重ねが重要です。',
        '上位校（日比谷・西・国立など）では「自校作成問題」（国語・数学・英語）が課され、内申点以上に「解く力」が試されます。',
        '観点別評価の「主体的に学習に取り組む態度」がC評価だと、知識・技能や思考力が高くても評定が伸び悩む要因になります。授業中の発言や提出物への取り組みも評定を左右します。'
      ]
    },
    faq: [
      {
  ...
  kanagawa: {
    ...
    // ④は事実誤りと判明（現行制度は「内申:学力」の2項目・合計10の比率で、旧「3項目(内申・学力・
    // 面接)」時代の名残と思われる「2:8:2」表記が誤って残っていた）ため実際の比率表記に修正。
    pitfalls: {
      title: '神奈川県入試の「独自ルール」',
      items: [
        '<strong>面接の廃止</strong>：令和6(2024)年度入試から各校判断での実施に移行し、原則は学力検査と調査書のみで合否判定します（一部の学校では特色検査の一環として面接が課されます）。',
        '<strong>中3の成績は2倍</strong>：中2の45点満点 ＋ 中3の90点満点 ＝ 135点満点。中3での伸びが2倍の重みを持ちます。',
        '<strong>特色検査の壁</strong>：横浜翠嵐や柏陽、希望ケ丘などの上位校では、教科横断型の「特色検査」が課されます。内申点だけでなく、この検査への対策も重要です。',
        '<strong>S値の計算</strong>：内申点と学力検査の比率(合計10)は学校ごとに「3:7」「5:5」「7:3」などに設定されます。内申が低い人は学力検査寄りの比率の学校を選ぶ戦略が有効。',
        '第2次選考の判定材料：募集人員の一部を占める第2次選考では、中3の「主体的に学習に取り組む態度」の観点別評価が判定材料になります。'
      ]
    },
    faq: [
      {
  ...
  osaka: {
    ...
    // 2026-08-01: 5項目をWebSearchで個別に裏取り。①②④⑤は一次情報と一致し正確。
    // ③は事実誤りと判明（本日別作業(prefecture-traps.ts)で確認済みの「タイプⅠ〜Ⅴ」5段階が
    // 正しく、「I〜III」という3タイプ表記は誤り）ため修正した。比率の数値・方向性(内申:当日の
    // 順で3:7がタイプⅠ=当日重視)自体は正しかったため据え置き。
    pitfalls: {
      title: '大阪府入試を生き抜く注意点',
      items: [
        '<strong>チャレンジテストの影響</strong>：府内統一の「チャレンジテスト」の結果により、各中学校の評定平均の範囲(目安±0.3)が制限されます。自分の頑張りだけでなく、学校全体のレベルも評定に関わります。',
        '<strong>英語外部検定（英検）</strong>：一般選抜では、英検2級で当日点80%、準1級以上で100%の読み替え（最低保証）が適用されます。当日の得点がこれを上回ればそちらが採用されます。',
        '<strong>選抜タイプ（Ⅰ〜Ⅴ）</strong>：学校によって「内申点：当日点」の比率がタイプⅠ「3:7」からタイプⅤ「7:3」まで5段階に分かれます。トップ校（文理学科等）はタイプⅠが多く、当日点勝負です。',
        '実技教科も等倍だが、3年分合算されると大きな差に。特に中3は「6倍」換算（45点×6＝270点分）されるため、中3の副教科は絶対に落とせません。',
        '「C問題」の難易度：国語・数学・英語で難易度別の問題（A・B・C）があり、上位校が採用するC問題は全国屈指の難易度です。'
      ]
    },
    faq: [
      {
  ...
  shizuoka: {
    ...
    pitfalls: {
      title: '静岡県の注意点',
      items: [
        '中3の第2学期末までの成績のみが対象（中1・中2は含まれない）',
        '各高校が定員の50%以内で独自方法（実技・作文・適応力検査等）で先に合格を決める「学校裁量枠」がある',
        '学校裁量枠の残りは「共通枠」で、単純な内申＋当日点の合計ではなく3段階審査（学力上位約75%→面接等約10%→総合審査約15%）で選抜される',
        '面接は全員が対象',
        '志望校の学校裁量枠の内容・共通枠の段階配分は募集要項で必ず確認する'
      ]
    },
    faq: [
      {
        question: '静岡県の内申点は何点満点ですか？',
        answer: '静岡県の内申点（調査書）は45点満点です。中学3年生の第2学期末までの9教科×5段階＝45点として計算され、全教科が等倍のシンプルな配点です。'
      },
      {
        question: '静岡県はいつの成績が内申点になりますか？',
        answer: '静岡県は中学3年生の、第2学期末までの成績が対象です。中1・中2の成績は内申点には含まれません。'
      },
      {
        question: '静岡県の「学校裁量枠」「共通枠」とは何ですか？',
        answer: '静岡県の公立高校一般選抜は、各高校が定員の50%以内で独自の方法（実技・作文・適応力検査等）で先に合格者を決める「学校裁量枠」と、残りの「共通枠」に分かれます。共通枠も学力検査上位約75%・面接等約10%・総合審査約15%の3段階審査で、内申点と当日点を単純に合計しても合否は出ません。'
      },
      {
        question: '静岡県で内申点を上げるコツは？',
        answer: '中3の、しかも第2学期末までに対象期間が限られるため、この期間の定期テスト・提出物・授業態度に集中することが最も効果的です。全教科が等倍なので、苦手教科の底上げが内申点アップに直結します。'
      }
    ],
    sources: [
      {
        ...
        lastVerified: '2026年6月13日',
        notes: '令和8年度静岡県公立高等学校入学者選抜関係発表資料'
      }
    ],
    related: [
      { title: '静岡県の内申点計算ツール', url: '/shizuoka/naishin' },
      { title: '静岡県の総合得点・合否の仕組み（学校裁量枠・共通枠）', url: '/shizuoka/total-score' },
      { title: '静岡県から逆算する', url: '/reverse?pref=shizuoka' },
      { title: '内申点の基本を学ぶ', url: '/blog/naishin-guide' }
    ]
  },
  ...
  aichi: {
    ...
    // 2026-08-01: 4項目をWebSearchで個別に裏取り。①は事実誤りと判明（実際は5つの計算式
    // Ⅰ〜Ⅴがあり、大きく3つの傾向(均等型Ⅰ/内申重視型Ⅱ・Ⅳ/当日点重視型Ⅲ・Ⅴ)に分類される
    // 制度で、「3タイプ（I型・II型・III型）」という表現は5つの計算式のうち2つ(Ⅳ・Ⅴ)が
    // 存在しないかのような誤解を招く。文中で直後に「V型」を引用しており原文自体に矛盾があった）
    // ため修正。②③④は一次情報と一致し正確。
    pitfalls: {
      title: '愛知県公立高校入試の「型」と戦略',
      items: [
        '<strong>「校内順位」を決定する5つの計算式：</strong>愛知県では、高校ごとに評定と学力検査の掛け率が異なる5つの計算式(Ⅰ〜Ⅴ)のいずれかを採用しており、大きく均等型(Ⅰ)・内申重視型(Ⅱ・Ⅳ)・当日点重視型(Ⅲ・Ⅴ)の3傾向に分かれます。旭丘などの進学校は当日点重視型(Ⅲ・Ⅴ)を採用する例が多く、内申が少し低くても当日点での大逆転が可能です。',
  ...
  aomori: {
    ...
    // 「学校ごとに7:3・5:5等の比率を選べる」という記述は他県(山形・鳥取等)の変動比率制度を
    // 誤って適用したものだった。比率の柔軟性があるのは一般選抜と別枠の「特色化選抜」における
    // 学年重み付け・傾斜配点の方）ため修正。⑤は特定校の目安点として妥当な範囲の記述に留まる。
    pitfalls: {
      title: '青森県の公立高校入試の重要ポイントと戦略',
      items: [
        ...
        '<strong>一般選抜は固定635点満点：</strong>青森県の一般選抜は、学力検査（500点満点）と調査書点（135点満点）の単純合計635点満点で判定されます（他県のような学校ごとの比率選択制ではありません）。これとは別に、学校ごとに条件が異なる「特色化選抜」枠があり、学年の重み付けや特定教科の傾斜配点を行う学校もあります。',
        '<strong>面接と実技試験の重要性：</strong>一般選抜では全員に学力検査と面接が課されるほか、学科によっては専門的な実技試験も行われます。特に、特色化選抜では、これらの結果が合否に直結することもあります。単に勉強ができるだけでなく、自分の考えを述べたり、スキルを証明したりする「表現力」も磨いておく必要があります。',
  ...
  iwate: {
    summary3lines: {
      ...
      maxScore: '内申点660点満点→実際の選抜では500点満点に圧縮して評価（令和7年度以降）'
    },
    examples: {
      all3: '396点 / 中堅校の目安（660点満点上の素点。実際の選抜で使われる500点満点への圧縮後の点数は募集要項でご確認ください）',
      all4: '528点 / 盛岡第一など上位校の挑戦ライン（660点満点上の素点）',
      practicalPlus1: '実技の「3→4」は、5教科の「3→4」の1.5倍の価値！'
    },
    // 2026-08-01: 4項目をWebSearchで個別に裏取り。①②③は一次情報・prefectures.tsと一致し正確。
    // ④は事実誤りと判明（実際の「A選考・B選考・C選考」は推薦入試の区分ではなく、一般選抜における
    // 学力検査:調査書+面接の比率タイプ(A=5:5均等/B=3:7調査書面接重視/C=7:3学力重視)を指す。
    // また従来の校長推薦が必要な「推薦入試」自体、令和7(2025)年度入試から通学区域の制限がない
    // 「特色選抜」に変更されており、原文の「通学区域内が対象のA選考」という説明は現行制度と
    // 矛盾する）ため、現行制度に合わせて修正。
    pitfalls: {
      title: '岩手県の公立高校入試の複雑な内申点計算',
      items: [
        ...
        '<strong>「660点満点→500点満点」への圧縮：</strong>複雑な計算で出した内申点（660点満点）は、最終的に500点満点に圧縮されます（令和7年度以降。令和6年度以前は440点満点でした）。学力検査500点・調査書（圧縮後）500点に、学校ごとに実施される学校独自検査（面接・小論文・作文・実技等のうち1〜2項目、0〜100点）を加えた1000〜1100点満点で一般選抜が実施されます。受験生は、自分の素点（660点満点）だけでなく、この換算後の点数も意識して志望校のボーダーラインと比較する必要があります。',
        '<strong>「A選考・B選考・C選考」は学力:調査書の比率タイプ：</strong>各高校は学力検査と調査書＋面接の比率をA選考(5:5・均等型)、B選考(3:7・調査書面接重視型)、C選考(7:3・学力検査重視型)から選びます。また、従来の中学校長推薦が必要だった「推薦入試」は令和7(2025)年度入試から、通学区域の制限がなく誰でも出願できる「特色選抜」に変わりました。'
      ]
    },
    faq: [
      {
        question: '岩手県の内申点計算が複雑すぎます。簡単に教えてください。',
        answer: 'はい、承知いたしました。まず、各学年の9教科の評定（5段階）を用意します。次に、<strong>【5教科の合計点×2】</strong>と<strong>【実技4教科の合計点×3】</strong>を計算します。これを各学年で行い、最後に<strong>【中1の合計点×1】＋【中2の合計点×2】＋【中3の合計点×3】</strong>を全て足したものがあなたの内申点（660点満点）です。実際の合否判定では、これを500点満点に圧縮した点数（令和7年度以降。令和6年度以前は440点満点でした）が使われます。'
      },
      {
  ...
  miyagi: {
    ...
    // 高校・学科・コースごとに共通選抜50〜90%・特色選抜10〜50%(体育・美術系学科は10〜90%)と
    // 幅がある）ため修正。④は具体的な比率(3:7)の一次情報を複数確認したが情報源間で数値が
    // 一致しない(6:4とする情報源もあった)ため、確定できない数値を明記せず一般的な傾向のみ
    // 記述するよう修正。⑤の「特色選抜において重視される傾向が強まっている」という断定は
    // 裏付けが取れなかったため仕組みの説明に留めた。
    pitfalls: {
      title: '宮城県の公立高校入試の重要ポイントと戦略',
      items: [
        ...
        '<strong>「共通選抜」と「特色選抜」の募集割合は高校ごとに違う：</strong>すべての高校・学科で「共通選抜」と「特色選抜」の2つの枠で合否判定が行われますが、募集割合は固定ではなく高校・学科・コースごとに共通選抜50〜90%・特色選抜10〜50%(体育・美術系学科は10〜90%)の範囲で異なります。「共通選抜」は学力検査と調査書の合計点で決まりますが、「特色選抜」は高校・学科の特色に応じて、面接・実技・作文の得点や調査書の特定の記録（部活動など）も加えて総合的に審査されます。志望校の募集割合を必ず確認しましょう。',
        '<strong>トップ校は当日点重視の傾向：</strong>仙台第一、仙台第二、宮城第一といったトップ校では、学力検査の比重を高めに設定する傾向があります（具体的な比率は高校ごとに異なるため募集要項で確認が必要）。内申点で高得点を取るのは当然の前提として、その上で学力検査でライバルと差をつけるハイレベルな学力が求められます。',
        '<strong>「主体的に学習に取り組む態度」の評価：</strong>調査書における「各教科の学習の記録」は「知識・技能」「思考・判断・表現」「主体的に学習に取り組む態度」の3観点で評価され、後者は主に授業態度が反映されます。授業中の積極的な発言や、質の高い提出物などが、数字の評定以上に大切になる可能性があります。'
      ]
    },
    faq: [
      {
      ...
      },
      {
        ...
        answer: '「共通選抜」は、これまで通り「学力検査点」と「調査書点」の合計点で合格者を決定するものです。一方、「特色選抜」は各高校が求める生徒像（例：理数系の探究活動に優れた生徒、スポーツで全国レベルの実績を持つ生徒など）に基づいて、調査書の内容をより深く評価したり、面接や実技試験を課したりして選抜します。募集定員に占める割合は高校・学科ごとに共通選抜50〜90%・特色選抜10〜50%と異なるため、志望校の割合を必ず確認しましょう。'
      },
      {
        question: '仙台第一高校や仙台第二高校に合格するには、どれくらいの内申点が必要ですか？',
        answer: 'これらのトップ校に合格するには、内申点は最低でも160点以上、安全圏を目指すなら170点以上（平均4.4〜4.6以上）が目安となります。しかし、これらの高校は学力検査の比重を高めに設定する傾向があるため、内申点のアドバンテージは限定的です（具体的な比率は高校ごとに異なるため募集要項で確認が必要）。内申点でライバルに差をつけられない分、当日の学力検査で500点満点中420〜430点以上を得点する高度な学力が不可欠です。'
      }
    ],
    sources: [
      {
...
/**
 * `pitfalls`（物語調の詳しい解説）をUIに表示してよいと事実確認済みの都道府県コード。
 *
 * `prefectureGuides`に手書きデータがある14県のうち、各項目を個別にWebSearchで裏取りした県
 * だけをここに追加する（PREFECTURE_TRAPSのtopicタグ方式と同じ「検証済みのみ解禁」の考え方）。
 * 2026-08-01: 東京都の5項目を検証（ESAT-J加点/実技2倍/自校作成問題校は一次情報と一致・
 ...
 * 現行の一次選抜一本化・6:2:2比重に修正）。愛知県も検証（「3タイプ（I型・II型・III型）」が
 * 事実誤りで実際は5つの計算式Ⅰ〜Ⅴがあり3傾向に分類される制度だったため修正。原文中で直後に
 * 「V型」を引用する内部矛盾もあった）。青森県も検証（「学校ごとに学力:調査書の比率を選べる
 * (例:7:3や5:5)」が事実誤りで、実際は学力検査500点+調査書135点の単純合計635点で判定する
 * 固定方式(山形・鳥取等の変動比率制度を誤って適用した記述)だったため修正）。岩手県も検証
 * （「推薦入試のA選考(通学区域内)/B選考(文化・スポーツ)」が事実誤りで、実際のA/B/C選考は
 * 一般選抜の学力:調査書比率タイプであり、従来の推薦入試自体も令和7年度から通学区域制限のない
 * 「特色選抜」に変わっていたため修正）。宮城県も検証（「共通選抜70%・特色選抜30%」の固定比率が
 * 事実誤りで実際は高校ごとに50〜90%/10〜50%と幅がある、「トップ校は3:7」という具体的比率も
 * 情報源間で数値が一致せず確定できないため修正）。**これでprefectureGuidesに存在する全14県の
 * 検証・topicタグ付けが完了した**。
 */
export const VERIFIED_PITFALLS_PREFECTURE_CODES = new Set(['tokyo', 'kanagawa', 'osaka', 'saitama', 'chiba', 'hokkaido', 'fukuoka', 'shizuoka', 'hyogo', 'hiroshima', 'aichi', 'aomori', 'iwate', 'miyagi']);
