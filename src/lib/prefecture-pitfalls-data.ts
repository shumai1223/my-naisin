...
// 2026-08-21: 上記の数値突合では検出できない「手続き・制度面」の事実誤りをkanagawaで1件発見・
// 修正（「面接が全校で実施される」→実際は令和6年度から各校判断に移行し原則廃止）。数値フィールド
// だけでなく、prefecture-guides.tsのような既にWebSearch裏取り済みの文章と突き合わせる横断チェックも
// 定期的に行う価値がある。
export const PREFECTURE_PITFALLS: Record<string, { title: string; items: string[] }> = {
  ...
  kanagawa: {
    ...
    items: [
      ...
      '中3の成績は2倍で計算される',
      'S値（学力検査・内申点・特色検査の合計得点）の比率は学校ごとに異なる',
      '面接は令和6(2024)年度入試から各校判断での実施に移行（一部の特色検査実施校を除き原則は学力検査と調査書のみで判定）',
  ...
  saga: {
    ...
    items: [
      '中1〜中3の3年間すべての成績が対象（学年差なし均等評価）',
      '9教科×5段階×3学年＝評定の合計135点というオーソドックスな数え方',
      '入試での調査書の配点は学校ごとに異なる（実施要項の付表で学校別に公表）。県共通の配点はない',
      '実技4教科への傾斜（重み付け）はなく主要教科と同倍率',
      '中1からの成績の積み重ねがそのまま内申点になる',
      '早い学年からの継続的な取り組みが重要'
    ]
