...
export const DATA_LICENSE_LEDGER: Record<string, DataLicenseLedgerEntry> = {
  ...
  nagano: {
    ...
    sourceHost: '.lg.jp',
    ...UNKNOWN(
      '【2026-09-01追記】初回問い合わせ(2026-08-28送信)への回答を受領(2026-09-01 05:48・Gmailスレッド' +
        '1a05b8350ec641fa): 「本県教育委員会が公表している資料につきましては、出典を明記していただく' +
        'ことにより、参考資料としてご活用いただくことは差し支えありません」＝一般的な引用利用は許諾。' +
        '「県の公式ホームページから...ウェブサイトへのリンク掲載や参照につきましては...原則として' +
        '行っておりません」＝被リンクは不可(mie/ishikawa/gifu/kagawaと同型)。' +
        '⚠️この回答はまだ「応募状況（募集人員・応募者数・応募倍率等）データ」を名指しした具体的な' +
        '再配布許諾ではない（一般的な「資料の活用」への言及に留まる）。gifu/mie/ishikawa/kagawaと' +
        '同型の絞り込み再質問（応募状況データの出典明記のうえでの掲載可否）を次回送信すること'
    ),
  },
  ibaraki: {
    prefecture: 'ibaraki',
