...
export const PREFECTURES: PrefectureConfig[] = [
  ...
  // 関東
  {
    ...
    maxScore: 135,
    description: '中1〜中3の9教科×5段階（135点満点）',
    note: 'この計算は募集の大半を占める「共通選抜」の方式です。「特色選抜」の一部の高校では、これとは異なる独自の換算方式が用いられる場合があります。',
  ...
  },
  {
    ...
    practicalMultiplier: 1,
    maxScore: 180,
    description: '中1・中2は各45点、中3は2倍で90点（180点満点）',
    sourceUrl: 'https://www.pref.ishikawa.lg.jp/kyoiku/gakkou/senbatu/documents/r8bosyuyoko.pdf',
    sourceUrl2: 'https://www.deskstyle.info/eria/ishikawa/juken.html',
  ...
  },
  {
    ...
    lastVerified: '2026-07-16',
    fiscalYear: '2026',
    // ⚠️2026-08-31再確認(質問ノート08-31 07:14の疑義に対応・詳細はsource-history.tsのnagasaki
    // 2026-08-31エントリ参照): 令和8年度実施要領PDF(01・1757565086.pdf)は
    // 「各高校でそれぞれの比重を定めて選抜を行う」という学校裁量制の記述のみでmaxScore=135の直接の
    // 裏付けが見当たらず、PDFのテキスト層もpdftotext/WebFetch双方で抽出不能(独自フォント埋め込みで
    // 機械的な原文確認が不可能と判明)。そのため独立した第三者情報源3件(jyuku-online.com直接WebFetch・
    // WebSearch要約が集約したedu-netz/eishinkan等・axis-kobetsu.jp)で135(中1〜中3均等9教科×5段階=
    // 45点×3学年)の記載を確認。「315点(学年傾斜)」という対立仮説は08-31時点のWebSearchで再検証した
    // ところ北海道の情報との混同と判明し当県には該当しない。axis側は「調査書は高校により異なる」とも
    // 言及しており、135は代表値(学校裁量の範囲内の標準パターン)という位置づけで従来の実装方針と整合。
    // ⚠️2026-08-21修正: examMaxScore(500点)は正しいが、totalMaxScoreが他県と同じ汎用値
