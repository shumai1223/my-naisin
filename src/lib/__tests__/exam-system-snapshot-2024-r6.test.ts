...
describe('2024-r6 exam-system snapshot（T-Y11 Task C 収集中スナップショットの整合性）', () => {
  ...
  test('shizuoka: 2025-r7・2026-r8と2024-r6で制度の核となる数値が一致する(実測で確認済みの「変更なし」)', () => {
    const s2024 = snapshot.entries.find((e) => e.code === 'shizuoka')!;
    const s2025 = snapshot2025.entries.find((e) => e.code === 'shizuoka')!;
    ...
    expect(s2024.gradeMultipliers).toEqual(s2026.gradeMultipliers);
  ...
  test('shiga: 2025-r7・2026-r8と2024-r6で制度の核となる数値が一致する(実測で確認済みの「変更なし」)', () => {
    const s2024 = snapshot.entries.find((e) => e.code === 'shiga')!;
    const s2025 = snapshot2025.entries.find((e) => e.code === 'shiga')!;
  ...
  test('kanagawa: 2025-r7・2026-r8と2024-r6で制度の核となる数値が一致する(実測で確認済みの「変更なし」)', () => {
    const k2024 = snapshot.entries.find((e) => e.code === 'kanagawa')!;
    const k2025 = snapshot2025.entries.find((e) => e.code === 'kanagawa')!;
  ...
  test('fukuoka: 2025-r7・2026-r8と2024-r6で制度の核となる数値が一致する(実測で確認済みの「変更なし」)', () => {
    const f2024 = snapshot.entries.find((e) => e.code === 'fukuoka')!;
    const f2025 = snapshot2025.entries.find((e) => e.code === 'fukuoka')!;
    const f2026 = snapshot2026.entries.find((e) => e.code === 'fukuoka')!;
    expect(f2024.maxScore).toBe(f2025.maxScore);
    expect(f2024.maxScore).toBe(f2026.maxScore);
    expect(f2024.gradeMultipliers).toEqual(f2025.gradeMultipliers);
    expect(f2024.gradeMultipliers).toEqual(f2026.gradeMultipliers);
  });

  test('saitama: 2025-r7・2026-r8と2024-r6で制度の核となる数値が一致する(実測で確認済みの「変更なし」)', () => {
    const s2024 = snapshot.entries.find((e) => e.code === 'saitama')!;
    const s2025 = snapshot2025.entries.find((e) => e.code === 'saitama')!;
