...
import {
  ...
  PREFECTURE_PLACEMENT_LEAD_OVERRIDES,
  type LeadPlacement,
} from '../lead-config';
...
describe('PREFECTURE_PLACEMENT_LEAD_OVERRIDES（L-5：県×面コピー個別化の全面展開）', () => {
  test('47都道府県全てに prefecture面の見出し/本文が生成されている', () => {
    for (const pref of PREFECTURES) {
      const key = `${pref.code}:prefecture`;
      const override = PREFECTURE_PLACEMENT_LEAD_OVERRIDES[key];
      expect(override).toBeDefined();
      expect(override.heading).toContain(pref.name);
      expect(override.body).toContain(pref.name);
      // 送客先の差し替えはここでは行わない（EV判断は県/季節オーバーライドの責務のまま）。
      expect(override.affiliateId).toBeUndefined();
    }
  });

  test('生成コピーは県ごとに一意（コピペ重複防止・機械生成のname/満点/倍率差で保証）', () => {
    const bodies = PREFECTURES.map((pref) => PREFECTURE_PLACEMENT_LEAD_OVERRIDES[`${pref.code}:prefecture`]?.body);
    expect(new Set(bodies).size).toBe(PREFECTURES.length);
  });

  test('倍率差のある県（実技重視）と無い県で本文の訴求フレーズが異なる（データ由来の実差・捏造でない）', () => {
    const tokyo = PREFECTURE_PLACEMENT_LEAD_OVERRIDES['tokyo:prefecture']; // practicalMultiplier(2) > coreMultiplier(1)
    const hokkaido = PREFECTURE_PLACEMENT_LEAD_OVERRIDES['hokkaido:prefecture']; // practicalMultiplier === coreMultiplier
    expect(tokyo?.body).toContain('実技4教科の評定が2倍');
    expect(hokkaido?.body).toContain('9教科がそのまま反映される方式');
  });

  test('学年スコープが県ごとに正しく反映される（中3のみ／3学年など）', () => {
    const tokyo = PREFECTURE_PLACEMENT_LEAD_OVERRIDES['tokyo:prefecture']; // targetGrades: [3]
    const hokkaido = PREFECTURE_PLACEMENT_LEAD_OVERRIDES['hokkaido:prefecture']; // targetGrades: [1,2,3]
    expect(tokyo?.body).toContain('中3の');
    expect(hokkaido?.body).toContain('中1〜中3の3年間');
  });

  test('selectLeadOfferでprefecture面を解決すると、送客先は既存の県/季節オーバーライドを維持したまま見出し/本文だけ差し替わる', () => {
    const offer = selectLeadOffer({ prefectureCode: 'osaka', placement: 'prefecture', season: null });
    expect(offer.heading).toContain('大阪府');
    expect(isLiveAffiliate(offer.affiliateId)).toBe(true);
    expect(offer.note).toContain('PR'); // KANSAI_CAMPUSのnoteが維持される
  });
});
