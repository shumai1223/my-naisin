 ...
 * 見つかった前科があるため、既存の`/developers`ページと同じ21,548件をここでも独立に固定する）。
 */
describe('旺文社PoC納品物v0（T-Y11F §5順序#5）', () => {
  it('buildFullExportRecordsは配布可能レコード数（21,548件）と一致する', () => {
    ...
    expect(expectedTotal).toBe(21_548);
    expect(records).toHaveLength(21_548);
  });
  ...
  it('toCsvはヘッダ行＋レコード数分の行を返す', () => {
    ...
    expect(lines).toHaveLength(21_548 + 1);
    expect(lines[0]).toBe(
