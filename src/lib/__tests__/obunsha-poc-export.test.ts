 ...
 * 見つかった前科があるため、既存の`/developers`ページと同じ24,210件をここでも独立に固定する）。
 */
describe('旺文社PoC納品物v0（T-Y11F §5順序#5）', () => {
  it('buildFullExportRecordsは配布可能レコード数（24,210件）と一致する', () => {
    ...
    expect(expectedTotal).toBe(24_210);
    expect(records).toHaveLength(24_210);
  });
  ...
  it('toCsvはヘッダ行＋レコード数分の行を返す', () => {
    ...
    expect(lines).toHaveLength(24_210 + 1);
    expect(lines[0]).toBe(
