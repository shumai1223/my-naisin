...
describe('CONFIRMED_NO_CHANGE_CHECKS（掛-4・空振りの調査記録）', () => {
  ...
  test('徳島県は対象期間内の制度変更なしと確認済み(育成型選抜/一般選抜の2本立て構成・195点満点とも安定)', () => {
    const tokushima = CONFIRMED_NO_CHANGE_CHECKS.find((c) => c.prefCode === 'tokushima');
    expect(tokushima).toBeDefined();
    expect(tokushima?.note).toContain('育成型選抜');

    const tokushimaPref = PREFECTURES.find((p) => p.code === 'tokushima');
    expect(tokushimaPref?.maxScore).toBe(195);
    expect(tokushimaPref?.practicalMultiplier).toBe(2);
  ...
  test('高知県は対象期間内の制度変更なしと確認済み(特色化選抜/一般選抜の構成・195点満点とも安定)', () => {
    const kochi = CONFIRMED_NO_CHANGE_CHECKS.find((c) => c.prefCode === 'kochi');
    expect(kochi).toBeDefined();
    expect(kochi?.note).toContain('特色化選抜');

    const kochiPref = PREFECTURES.find((p) => p.code === 'kochi');
    expect(kochiPref?.maxScore).toBe(195);
    expect(kochiPref?.practicalMultiplier).toBe(2);
    expect(getPastSystemChangesByPrefecture('kochi')).toEqual([]);
  });
