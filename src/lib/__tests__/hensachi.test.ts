...
describe('buildHensachiWeeklyPlan（A-9：週次計画ジェネレータ）', () => {
  test('5週間で均等按分し、最終週は目標値ちょうどになる', () => {
    const plan = buildHensachiWeeklyPlan(50, 60, 5);
    expect(plan).toHaveLength(5);
    expect(plan[0]).toEqual({ week: 1, targetHensachi: 52 });
    expect(plan[4]).toEqual({ week: 5, targetHensachi: 60 });
  });

  test('週数を指定できる（3週間）', () => {
    const plan = buildHensachiWeeklyPlan(50, 53, 3);
    expect(plan).toHaveLength(3);
    expect(plan[2].targetHensachi).toBe(53);
  });

  test('target <= current（既に到達）なら空配列', () => {
    expect(buildHensachiWeeklyPlan(60, 60, 5)).toEqual([]);
    expect(buildHensachiWeeklyPlan(60, 55, 5)).toEqual([]);
  });

  test('不正な週数・非有限値は空配列', () => {
    expect(buildHensachiWeeklyPlan(50, 60, 0)).toEqual([]);
    expect(buildHensachiWeeklyPlan(NaN, 60, 5)).toEqual([]);
    expect(buildHensachiWeeklyPlan(50, NaN, 5)).toEqual([]);
  });
});
