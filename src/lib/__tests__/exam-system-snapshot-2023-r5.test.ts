...
describe('2023-r5 exam-system snapshot（T-Y11 Task C 収集中スナップショットの整合性）', () => {
  ...
  test('niigata: 2024-r6・2025-r7・2026-r8と2023-r5で制度の核となる数値が一致する(実測で確認済みの「変更なし」)', () => {
    const ni2023 = snapshot.entries.find((e) => e.code === 'niigata')!;
    const ni2024 = snapshot2024.entries.find((e) => e.code === 'niigata')!;
    const ni2025 = snapshot2025.entries.find((e) => e.code === 'niigata')!;
    const ni2026 = snapshot2026.entries.find((e) => e.code === 'niigata')!;
    expect(ni2023.maxScore).toBe(135);
    expect(ni2023.maxScore).toBe(ni2024.maxScore);
    expect(ni2023.maxScore).toBe(ni2025.maxScore);
    expect(ni2023.maxScore).toBe(ni2026.maxScore);
    expect(ni2023.gradeMultipliers).toEqual({ '1': 1, '2': 1, '3': 1 });
  ...
  test('fukui: 2024-r6・2025-r7・2026-r8と2023-r5で制度の核となる数値が一致する(実測で確認済みの「変更なし」)', () => {
    const fk2023 = snapshot.entries.find((e) => e.code === 'fukui')!;
    const fk2024 = snapshot2024.entries.find((e) => e.code === 'fukui')!;
    const fk2025 = snapshot2025.entries.find((e) => e.code === 'fukui')!;
    const fk2026 = snapshot2026.entries.find((e) => e.code === 'fukui')!;
    expect(fk2023.maxScore).toBe(45);
    expect(fk2023.maxScore).toBe(fk2024.maxScore);
    expect(fk2023.maxScore).toBe(fk2025.maxScore);
    expect(fk2023.maxScore).toBe(fk2026.maxScore);
    expect(fk2023.gradeMultipliers).toEqual({ '1': 0, '2': 0, '3': 1 });
    expect(fk2023.gradeMultipliers).toEqual(fk2024.gradeMultipliers);
    expect(fk2023.reverseCalc).toBeNull();
  });
});
