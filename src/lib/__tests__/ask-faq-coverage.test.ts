...
describe('buildPrefectureFormulaFaqs（S-4④・軸拡張3本目=計算式）', () => {
  const faqs = buildPrefectureFormulaFaqs();
  ...
  it('すべての回答が空でなく、質問文に都道府県名を含む（formula.summaryは県名非依存の汎用文のため回答文自体には県名を含まない仕様）', () => {
    for (let i = 0; i < faqs.length; i++) {
      const p = PREFECTURES[i];
      expect(faqs[i].answer.length).toBeGreaterThan(0);
