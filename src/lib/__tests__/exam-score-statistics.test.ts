...
import { EXAM_SCORE_STATISTICS_TOYAMA } from '@/data/exam-score-statistics/toyama';
...
describe('EXAM_SCORE_STATISTICS_KAGOSHIMA(パイロット実データ・2つの独立PDFで5年度が重複検証済み)', () => {
  it('7年度分(令和2〜8年度)が収録されている', () => {
    expect(EXAM_SCORE_STATISTICS_KAGOSHIMA.years).toHaveLength(7);
  });

  it('各教科90点満点(他県に無い特有配点・5教科合計450点満点)・test-takersで統一されている', () => {
    for (const year of EXAM_SCORE_STATISTICS_KAGOSHIMA.years) {
      expect(year.averageType).toBe('test-takers');
      expect(year.subjects).toHaveLength(5);
      for (const s of year.subjects) expect(s.maxScore).toBe(90);
      expect(year.totalMaxScore).toBe(450);
    }
  });
...
describe('EXAM_SCORE_STATISTICS_OKINAWA(パイロット実データ・1年度分は内部矛盾検知で除外)', () => {
  it('7年度分(平成30・31年度、令和2・3・5・6・7年度)が収録されている(令和4年度は内部矛盾により除外)', () => {
    expect(EXAM_SCORE_STATISTICS_OKINAWA.years).toHaveLength(7);
    expect(EXAM_SCORE_STATISTICS_OKINAWA.years.map((y) => y.fiscalYearLabel)).not.toContain('令和4年度');
  });

  it('各教科60点満点(5教科合計300点満点体系)・test-takersで統一されている', () => {
    for (const year of EXAM_SCORE_STATISTICS_OKINAWA.years) {
      expect(year.averageType).toBe('test-takers');
      expect(year.subjects).toHaveLength(5);
      for (const s of year.subjects) expect(s.maxScore).toBe(60);
      expect(year.totalMaxScore).toBe(300);
    }
  });

  it('全年度で教科別平均点の合計がtotalAverageと妥当な範囲で一致する(令和4年度の1.0点乖離を除外済みのため)', () => {
    for (const year of EXAM_SCORE_STATISTICS_OKINAWA.years) {
      expect(isPlausibleSubjectSum(year)).toBe(true);
    }
  });
...
describe('EXAM_SCORE_STATISTICS_ISHIKAWA(パイロット実データ・1年度分は内部矛盾検知で除外)', () => {
  ...
  it('各教科100点満点(5教科合計500点満点体系)・全年度passersで統一されている', () => {
  ...
  it('全年度で教科別平均点の合計がtotalAverageと妥当な範囲で一致する(令和2年度の0.7点乖離を除外済みのため)', () => {
    for (const year of EXAM_SCORE_STATISTICS_ISHIKAWA.years) {
      expect(isPlausibleSubjectSum(year)).toBe(true);
    }
  });
...
describe('EXAM_SCORE_STATISTICS_SAGA(パイロット実データ・2年度が独立記事とクロス検証済み)', () => {
  ...
  it('各教科50点満点(5教科合計250点満点体系)・test-takersで統一されている', () => {
    for (const year of EXAM_SCORE_STATISTICS_SAGA.years) {
      expect(year.averageType).toBe('test-takers');
      expect(year.subjects).toHaveLength(5);
      for (const s of year.subjects) expect(s.maxScore).toBe(50);
    }
  ...
  it('令和5・6年度分は独立記事(英進館)のsourceを持つ(クロス検証済みの印)', () => {
    const r5 = EXAM_SCORE_STATISTICS_SAGA.years.find((y) => y.fiscalYearLabel === '令和5年度');
...
describe('EXAM_SCORE_STATISTICS_TOYAMA(パイロット実データ・教育長談話の参考表・WebSearch要約と完全一致でクロス検証済み)', () => {
  it('2年度分(令和7〜8年度)が収録されている', () => {
    expect(EXAM_SCORE_STATISTICS_TOYAMA.years).toHaveLength(2);
  });

  it('各教科100点満点・5教科の内訳を持ちtest-takersで統一されている', () => {
    for (const year of EXAM_SCORE_STATISTICS_TOYAMA.years) {
      expect(year.averageType).toBe('test-takers');
      expect(year.subjects).toHaveLength(5);
      for (const s of year.subjects) expect(s.maxScore).toBe(100);
    }
  ...
  it('原資料の「総平均点」は5教科の相加平均(合計ではない)のためtotalAverageは未設定', () => {
    for (const year of EXAM_SCORE_STATISTICS_TOYAMA.years) {
      expect(year.totalAverage).toBeUndefined();
      expect(isPlausibleSubjectSum(year)).toBe(true);
    }
...
describe('exam-score-statistics index', () => {
  it('の41県が集約されている', () => {
    ...
    expect(EXAM_SCORE_STATISTICS_FILES).toHaveLength(41);
  });
});
