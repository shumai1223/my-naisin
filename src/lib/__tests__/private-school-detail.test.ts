...
describe('PRIVATE_SCHOOL_DETAIL_OSAKA(大都市圏5県の3県目・育伸社募集要項PDF1ページ目+公式サイト個別確認で48校を収録・スキップ5校・進行中・参照台帳107校)', () => {
  it('収録した学校は全てcourses合計とtotalCapacityが一致する', () => {
  ...
  it('収録48校・スキップ5校で参照台帳107校のうち残り54校は未着手(重複なし)', () => {
    ...
    expect(result.duplicates).toEqual([]);
    expect(result.missing).toHaveLength(54);
    expect(PRIVATE_SCHOOL_DETAIL_OSAKA.schools.length).toBe(48);
    expect(PRIVATE_SCHOOL_DETAIL_OSAKA.skipped.length).toBe(5);
  });
});
