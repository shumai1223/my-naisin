...
describe('PRIVATE_SCHOOL_DETAIL_OSAKA(大都市圏5県の3県目・育伸社募集要項PDF1ページ目+公式サイト個別確認で63校を収録・スキップ9校・進行中・参照台帳107校)', () => {
  it('収録した学校は全てcourses合計とtotalCapacityが一致する', () => {
  ...
  it('収録63校・スキップ9校で参照台帳107校のうち残り35校は未着手(重複なし)', () => {
    ...
    expect(result.duplicates).toEqual([]);
    expect(result.missing).toHaveLength(35);
    expect(PRIVATE_SCHOOL_DETAIL_OSAKA.schools.length).toBe(63);
