...
describe('PRIVATE_SCHOOL_DETAIL_TOKYO(大都市圏5県の最後・育伸社募集要項PDF全20ページ処理完了+保留3校の再訪で241校中169校を収録・スキップ8校・残り64校は別チャネルで継続調査)', () => {
  it('収録した学校は全てcourses合計とtotalCapacityが一致する', () => {
  ...
  it('収録169校・スキップ8校で参照台帳241校のうち残り64校は未着手(重複なし)', () => {
    ...
    expect(result.duplicates).toEqual([]);
    expect(result.missing).toHaveLength(64);
    expect(PRIVATE_SCHOOL_DETAIL_TOKYO.schools.length).toBe(169);
    expect(PRIVATE_SCHOOL_DETAIL_TOKYO.skipped.length).toBe(8);
  });
});
