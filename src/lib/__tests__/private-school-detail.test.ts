...
describe('PRIVATE_SCHOOL_DETAIL_TOKYO(大都市圏5県の最後・育伸社募集要項PDF全20ページ処理完了+個別公式サイト調査11弾(廃校・募集停止校4校含む)+東京文華の公式PDF再訪で解決・241校を完全網羅達成・176校収録+スキップ65校)', () => {
  ...
  it('収録176校(掛-2の2024年度117校分含め293レコード)・スキップ65校で参照台帳241校を完全網羅(重複・欠落なし)', () => {
    ...
    expect(result.duplicates).toEqual([]);
    expect(result.missing).toHaveLength(0);
    expect(PRIVATE_SCHOOL_DETAIL_TOKYO.schools.length).toBe(293);
  ...
  it('掛-2(私立×多年度・1ページ目): 足立学園は2024→2026年度で総定員が変化', () => {
    const y2024 = PRIVATE_SCHOOL_DETAIL_TOKYO.schools.find(
      (s) => s.schoolCode === 'D113312100013' && s.fiscalYearLabel === '2024年度'
    );
    const yLatest = PRIVATE_SCHOOL_DETAIL_TOKYO.schools.find(
      (s) => s.schoolCode === 'D113312100013' && s.fiscalYearLabel !== '2024年度'
    );
  ...
  it('掛-2(私立×多年度・2〜3ページ目): 安部学院・岩倉・江戸川女子・大森学園・開成・かえつ有明・科学技術学園・学習院高等科・神田女学園・関東国際・関東第一の11校は2024年度と2026年度で総定員が完全一致', () => {
    const pairs: Array<[string, number]> = [
      ['D113311700019', 100],
      ['D113310600012', 420],
      ['D113312300020', 125],
  ...
  it('掛-2(私立×多年度・2〜3ページ目): 桜美林・北豊島の2校は2024→2026年度で総定員が変化', () => {
    const pairs: Array<[string, number, number]> = [
      ['D113320900036', 150, 180],
      ['D113311800027', 150, 140],
    ];
    for (const [code, before, after] of pairs) {
      const y2024 = PRIVATE_SCHOOL_DETAIL_TOKYO.schools.find(
        (s) => s.schoolCode === code && s.fiscalYearLabel === '2024年度'
  ...
  it('掛-2(私立×多年度・4ページ目): 共栄学園・錦城・錦城学園・国立音楽大学附属・慶應義塾女子・京華商業の6校は2024年度と2026年度で総定員が完全一致', () => {
    const pairs: Array<[string, number]> = [
      ['D113312200012', 160],
      ['D113321100014', 450],
      ['D113310100053', 240],
  ...
  it('掛-2(私立×多年度・4ページ目): 京華は2024→2026年度で総定員が変化', () => {
    const y2024 = PRIVATE_SCHOOL_DETAIL_TOKYO.schools.find(
      (s) => s.schoolCode === 'D113310500068' && s.fiscalYearLabel === '2024年度'
    );
    const yLatest = PRIVATE_SCHOOL_DETAIL_TOKYO.schools.find(
      (s) => s.schoolCode === 'D113310500068' && s.fiscalYearLabel !== '2024年度'
    );
  ...
  it('掛-2(私立×多年度・7ページ目): 品川学藝・芝浦工業大学附属・下北沢成徳・自由ヶ丘学園・自由学園・修徳の6校は2024年度と2026年度で総定員が完全一致', () => {
    const pairs: Array<[string, number]> = [
      ['D113310900073', 100],
      ['D113310800047', 50],
      ['D113311200130', 100],
  ...
  it('掛-2(私立×多年度・7〜8ページ目): 淑徳・松蔭大学附属松蔭・聖徳学園・城北の4校は2024年度と2026年度で総定員が完全一致', () => {
    const pairs: Array<[string, number]> = [
      ['D113311900017', 90],
      ['D113311200103', 80],
      ['D113320300014', 130],
  ...
  it('掛-2(私立×多年度・7〜8ページ目): 十文字・淑徳巣鴨・城西大学附属城西の3校は2024→2026年度で総定員が変化', () => {
    const pairs: Array<[string, number, number]> = [
  ...
  it('掛-2(私立×多年度・15ページ目): 日本大学櫻丘・日本大学第一・日本大学第二・日本大学第三・日本大学鶴ヶ丘・日本大学豊山・日本大学豊山女子の7校(日大系列)は2024年度と2026年度で総定員が完全一致', () => {
    const pairs: Array<[string, number]> = [
      ['D113311200247', 450],
      ['D113310700011', 150],
      ['D113311500100', 210],
  ...
  it('掛-2(私立×多年度・15ページ目): 日本女子体育大学附属二階堂は総定員80は一致するがコース間で再配分', () => {
    const y2024 = PRIVATE_SCHOOL_DETAIL_TOKYO.schools.find(
      (s) => s.schoolCode === 'D113311200229' && s.fiscalYearLabel === '2024年度'
    );
    const yLatest = PRIVATE_SCHOOL_DETAIL_TOKYO.schools.find(
      (s) => s.schoolCode === 'D113311200229' && s.fiscalYearLabel !== '2024年度'
    );
    expect(y2024?.totalCapacity).toBe(80);
    expect(yLatest?.totalCapacity).toBe(80);
  });

  it('掛-2(私立×多年度・16ページ目): 八王子実践・広尾学園・フェリシア・文華女子(現・東京文華)の4校は2024年度と2026年度で総定員が完全一致', () => {
    const pairs: Array<[string, number]> = [
      ['D113320100070', 530],
      ['D113310300042', 25],
      ['D113320900054', 280],
