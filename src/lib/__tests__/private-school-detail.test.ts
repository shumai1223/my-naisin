...
describe('PRIVATE_SCHOOL_DETAIL_OSAKA(大都市圏5県の3県目・育伸社募集要項PDF1ページ目+公式サイト個別確認で45校を収録・スキップ4校・進行中・参照台帳107校)', () => {
  it('収録した学校は全てcourses合計とtotalCapacityが一致する', () => {
  ...
  it('収録45校・スキップ4校で参照台帳107校のうち残り58校は未着手(重複なし)', () => {
    ...
    expect(result.duplicates).toEqual([]);
    expect(result.missing).toHaveLength(58);
    expect(PRIVATE_SCHOOL_DETAIL_OSAKA.schools.length).toBe(45);
