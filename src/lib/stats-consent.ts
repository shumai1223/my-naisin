/**
 * 匿名統計オプトインの同意状態（S-1・旧TIER N-1）。
 *
 * A-12/S-1（一次データ堀）の入口。計算結果画面で「匿名で統計に協力する」への
 * 同意/撤回をlocalStorageに保持する純粋な状態管理のみをここに実装する。
 *
 * 送信先API（/api/stats/submit）・集計スキーマ（migration 0007）は実装・適用済み。
 * StatsOptInコンポーネントは2026-07-11時点で/hensachiに結線済み（他計算機面への展開は継続タスク）。
 *
