...
describe('bairitsu-ingest registry（T-Y11E E-1）', () => {
  ...
  it('sagaのパーサが登録されており、既存の手作業データと完全一致する結果を返す', () => {
    ...
    // T-Y11F §5順序#8: page/rowIndexは出典ロケータ用に追加されたフィールド。quota等の既存値は変えていない。
    // 位置ベース補完4件はpage/rowIndex両側ともundefinedのため引き続き一致する。
    expect(parsed).toEqual(
      expectedR8Records.map((e) => ({
        schoolName: e.schoolName,
        department: e.department,
  ...
  it('shigaのパーサが登録されており、既存の手作業データと完全一致する結果を返す', () => {
    ...
    // T-Y11F §5順序#8: page/rowIndexは出典ロケータ用に追加されたフィールド。quota等の既存値は変えていない。
    // 「両方の学科」合算レコードはpage/rowIndex両側ともundefinedのため引き続き一致する。
    expect(parsed).toEqual(
      expectedR8Records.map((e) => ({
        schoolName: e.schoolName,
        department: e.department,
  ...
  it('naganoのパーサが登録されており、既存の手作業データと多重集合一致する結果を返す（areaフィールド含む・ブロックoverride校を末尾追加するため順不同比較）', () => {
    const parser = getPrefectureParser('nagano');
    expect(parser).toBeDefined();
    const parsed = parser!(naganoR8Geometry as PdfPageGeometry[]) as unknown as NaganoParsedRow[];
  ...
  it('tochigiのパーサが登録されており、既存の手作業データと完全一致する結果を返す', () => {
    ...
    // T-Y11F §5順序#8: page/rowIndexは出典ロケータ用に追加されたフィールド。quota等の既存値は変えていない
    expect(parsed).toEqual(
      expectedR8Records.map((e) => ({ schoolName: e.schoolName, department: e.department, quota: e.quota, finalApplicants: e.finalApplicants, finalRate: e.finalRate, page: e.page, rowIndex: e.rowIndex }))
    );
  });
