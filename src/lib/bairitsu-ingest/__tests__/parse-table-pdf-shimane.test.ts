 ...
 * 決定不能問題）。既知の3件（松江商業・浜田商業「商業」/隠岐島前「普通」）を固定の
 * CONTINUATION_LABELSとして扱い、直前に登場した真の学校名へ後処理で書き戻す。
 * ⚠️2026-09-10(T-Y11F §5順序#7): 当初「情報科学」もこの4件目として同じ扱いをしていたが、
 * 島根県立情報科学高等学校（安来市）という実在の独立校であることが段階台帳作業で判明し、
 * CONTINUATION_LABELSから除外・独立校として扱うよう訂正した（parsers/shimane.tsのコメント参照）。
 *
