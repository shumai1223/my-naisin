...
export const EXAM_RATIO_DATA: ExamRatioConfig[] = [
  {
    ...
    generalExam: {
      ...
      totalMaxScore: 1020,
      subjects: 5,
      note: '換算内申300点満点＋学力検査700点満点＋ESAT-J20点＝1020点満点（total-score/tokyo.tsのTOKYO_TOTAL_SCORE_MAXと同じ）',
    },
    recommendedExam: {
      naishinRatio: 50,
      examRatio: 50,
  ...
  },
  {
    ...
    generalExam: {
      ...
      totalMaxScore: 900,
      subjects: 5,
      note: '学校選択問題実施校あり。内申点の学年比は標準1:1:2（180点満点）だが、高校により1:1:3・1:2:3などもある。総合点満点は高校ごとに異なり県一律の値は無いため、目安として500(学力)+400(調査書点の仮定)=900点前後（total-score/saitama.tsのSAITAMA_ASSUMED_TOTAL_CEILINGと同じ仮定）',
    },
    lastVerified: '2026-01-30',
  ...
  },
  {
    ...
    generalExam: {
      ...
      totalMaxScore: 635,
      subjects: 5,
      note: '2日間入試。調査書点(評定合計135点満点×K値)+学力検査500点+学校設定検査等(任意)。K値の標準値1.0の場合、調査書点135点+学力検査500点=635点満点（total-score/chiba.tsのcomputeChibaKValueと同じ前提）。K値は高校により0.5〜2.0で変動するため比率は目安。',
    },
    lastVerified: '2026-01-30',
  ...
  },
  {
    ...
    generalExam: {
      ...
      totalMaxScore: 615,
      subjects: 5,
      note: '学校裁量問題実施校あり。個人調査書の評定(315点満点)+学力検査(300点満点)=615点満点（total-score/hokkaido.tsのHOKKAIDO_TOTAL_SCORE_MAXと同じ）',
    },
    lastVerified: '2026-01-30',
