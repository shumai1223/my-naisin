...
// 表示される（getPrefectureTraps参照）。topicが無いエントリ（tokyo/kanagawa/osaka/fukuoka/
// hokkaido/saitama以外の6県）はまだ内容の再検証が済んでいないため、意図的に非表示のまま
// 据え置く（1県ずつ検証してから解禁する方針）。神奈川県は2026-08-01に再検証し、「特色検査の
// 影響」の未検証だった割合（約半数）を削除、「2次選考の存在」（一部学校で面接・作文）を
// 事実誤り（第2次選考は全校対象・判定材料は「主体的に学習に取り組む態度」の観点別評価）と
// 判明し訂正、「主体的態度の評価」は訂正後の内容と重複するため削除した。大阪府も同日に
// 再検証し、「A方式とB方式の選択」を事実誤り（実際は学力:内申=7:3〜3:7の5段階「タイプⅠ〜Ⅴ」）
// と判明し訂正した。福岡県も同日に再検証し、「実技4教科は2倍計算」と「3年間が対象」の**2件**が
// prefectures.tsの実データ（practicalMultiplier=1・targetGrades=[3]）と直接矛盾する事実誤りと
// 判明し訂正した（東京都・大阪府の同名エントリからのコピペ由来の誤りと推測）。北海道も同日に
// 再検証し、「特色検査の導入」を事実誤り（"特色検査"は神奈川県の制度名。北海道はむしろ2022年度
// から学校裁量問題を廃止する方向で記述と実態が逆行）と判明し、実際の制度（3段階選抜）に書き換えた。
 ...
 * （2026-08-01: 東京都・神奈川県・大阪府・福岡県・北海道・埼玉県を再検証・topicタグ付けを実施。他県は今後1県ずつ検証してから解禁する）。
 */
