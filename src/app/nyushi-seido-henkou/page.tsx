...
const UPCOMING_CHANGES: UpcomingChange[] = [
  {
    ...
    headline: '前期選抜と中期選抜を一本化し「独自枠」「共通枠」を新設する予定',
    detail:
      '京都府は令和9年度入試から、現行の前期選抜・中期選抜（・後期選抜）という区分を前期選抜に一本化する予定です。公式ページでは新制度に「独自枠」（各校が独自の検査項目・配点で選抜）という名称が使われていることを確認しています。令和8年度（2026年度）入試は従来どおりの方式で実施されます。前期・中期で内申点の満点・配点方式が異なる現行制度が変わるため、対象学年の生徒・保護者は今後の正式発表（学校別の検査項目・配点を含む詳細資料）に注意が必要です。',
    sourceUrl: 'https://www.kyoto-be.ne.jp/koukyou/cms/?p=7529',
    sourceTitle: '京都府教育委員会「京都府公立高等学校の新しい入学者選抜制度について」',
