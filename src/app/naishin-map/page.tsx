...
const FAQS = [
  {
    ...
    answer:
      'はい。実際の都道府県境の形状をもとにしたSVG地図です（沖縄県のみ本州から距離が離れているため、一般的な日本地図の慣習に倣い左下に別枠で表示しています）。ただし詳細な離島・境界線までは再現していない簡略化データのため、正確な行政区域図としてではなく、データを比較しやすくするための可視化としてご利用ください。',
  },
  {
    question: '地図の色は何を表していますか？',
    answer:
      '選んだ指標（実技教科の傾斜倍率／中3の重み／内申点の満点）について、47都道府県の実測値を5段階に色分けしています。色が濃いほど値が大きいことを表します。正確な数値は各都道府県にカーソルを合わせる（タップする）か、下の「表で見る」から確認できます。',
  },
  {
...
export const metadata: Metadata = {
  title: '内申点の日本地図【都道府県別に色分け比較】| My Naishin',
  description:
    '47都道府県の内申点の「実技教科の傾斜倍率」「中3の重み」「満点」を、都道府県境の形をした日本地図で一目比較。各都道府県教育委員会の公式データに基づく可視化です。',
  keywords: ['内申点 地図', '内申点 都道府県 比較 地図', '内申点 全国マップ', '内申点 都道府県 違い 可視化'],
  alternates: { canonical: `${SITE_URL}/naishin-map` },
  openGraph: {
    title: '内申点の日本地図【都道府県別に比較】| My Naishin',
    description: '47都道府県の内申点の重み付けを、色分けされた日本地図で一目比較。',
        ...
        <div className="mx-auto max-w-3xl px-4 py-6 md:py-10">
          ...
          <header className="mb-8 text-center">
            ...
            <p className="mx-auto mt-4 max-w-xl leading-relaxed text-slate-600">
              47都道府県の内申点の重み付けを、色分けされた日本地図で一目比較。
              各都道府県をタップすると、その県の詳しい解説に移動します。
            </p>
          </header>
          ...
          <section className="mb-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="text-sm leading-relaxed text-slate-600">
              ...
              地図は実際の都道府県境の形をもとにしていますが、離島・境界線の細部までは再現していない簡略化データです。
