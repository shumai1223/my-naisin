...
const DATASET_EXEMPT_ROUTES: Record<string, string> = {
  ...
  '/juku/matching/dashboard': 'noindex・旗付きUI・招待トークン必須の内部ツール（データセットの提供ではない・Λ-7）',
  '/juku/matching': 'noindex・旗付きUIの公開送客導線（提携塾一覧・データセットの提供ではない・Λ-7残作業）',
  ...
  '/naishin-type-shindan': 'ZZ-5c：内申点タイプ診断（利用者入力の決定論分類でありデータセットの提供ではない）',
  ...
  '/glossary/[term]': '用語個別ページ（定義・具体例の解説でありデータセットの提供ではない）',
  ...
  '/hyotei-heikin/gakushu-seiseki/yokuaru-machigai': 'よくある誤りの解説記事（自作の具体例による検証でデータセットの提供ではない・T-C4）',
  '/hyotei-heikin/gakushu-seiseki/tousei-han-i': '到達可能範囲チェックUI（単発計算でありデータセットの提供ではない・T-C4）',
  '/kakomon-jisaiten': '過去問自己採点×総合得点計算UI（単発計算でありデータセットの提供ではない・T-C6）',
  ...
  '/jikosaiten': '自己採点のやり方解説ページ（手順/HowToでありデータセットの提供ではない）',
  ...
  '/naishin-oru': 'オール3/4/5内申点ハブページ（下位ページへの導線のみ・データセットの提供なし）',
  '/naishin-oru/[grade]': 'オール3/4/5内申点の計算例ページ（単発の計算例一覧でありデータセットの提供ではない）',
  '/nyushi-seido-henkou': '入試制度変更点まとめページ（ニュース速報一覧であり構造化データセットの提供ではない）',
  ...
  '/shutsugan-junbi/kounai-senkou': '校内選考の解説ページ（データセットの提供なし）',
