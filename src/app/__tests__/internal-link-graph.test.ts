...
const NO_INBOUND_LINK_EXEMPT_ROUTES: Record<string, string> = {
  ...
  '/admin/community-posts': '認証必須のnoindex内部ツール（意図的にリンクしない設計・Λ-14 build-not-launch）',
  ...
  '/juku/matching/dashboard': '招待トークン必須のnoindex内部ツール（admin/juku-matchingが発行するURLを提携塾に個別に渡す運用のため文脈的リンクは意図的に無い・Λ-7）',
