...
const PREFECTURE_META_OVERRIDES: Record<string, { title: string; description: string }> = {
  ...
  // 2026-08-02判明: 旧nara設定(中1除外・135点満点)は2026年3月17日発表の制度改定で既に廃止されており、
  // prefectures.tsは既にmaxScore=144/gradeMultipliers{1:0.6,2:0.6,3:2}へ更新済み(lastVerified: 2026-07-17)
  // だったが、本overrideの文言だけが古いまま取り残されていた(note フィールドが「旧制度の中1除外・135点満点とは
  // 別物です」と明記する制度そのもの)。SERPのtitle/descriptionが実際の制度と矛盾する状態だったため実データに合わせて修正。
  nara: {
    title: '奈良県の内申点 計算｜中1・中2は「態度」評価で27点ずつ・144点満点を30秒で自動計算【2026】 | My Naishin',
    description:
      '【無料】奈良県の内申点は2026年3月の制度改定で、中1・中2「態度」評価(各27点)＋中3評定×2倍(90点)＝144点満点が標準に。学校により234/198/180点のパターンも。評定を入れて30秒で自動算出。2026年度入試対応。',
  },
  miyagi: {
        ...
        <div className="mx-auto max-w-4xl px-4 py-8 md:py-12">
          ...
          <div className="space-y-8">
            ...
            <RelatedToolsSection
              heading="次の一手・関連ツール"
              links={[
                { href: '/hensachi', title: '偏差値計算サイト（5教科）', desc: '内申点とあわせて学力の立ち位置も把握' },
                ...
                ...(ALTERNATIVE_TRACK_PREFECTURE_CODES.includes(prefectureCode)
                  ? [
                      {
                        href: `/${prefectureCode}/teiji-tsushin`,
                        title: `${prefecture.name}の定時制・通信制高校一覧`,
                        desc: '全日制以外に公表されている選抜区分を、募集人員・倍率とあわせて確認',
                      },
                    ]
                  : []),
              ]}
