...
/**
 * T-P1 P1-3 DoD検証（徳島県・定時制課程6校9レコード）: 手入力した合計が、
 * 徳島県教育委員会公表の「合計」（officialSubtotals）と一致することを機械的に突合する。
 */
describe('徳島県 定時制課程 倍率パイプライン（T-P1 P1-3）', () => {
  const { records, officialSubtotals } = TOKUSHIMA_TEIJI_COMPETITION_RATES;

  it('取り込み件数は9レコード（6校・複数学科/時間帯を持つ学校は別レコード）', () => {
  ...
  it('全レコードの合計が公式「合計」と一致する', () => {
    ...
    expect(sumRecords(records).quota).toBe(210);
    expect(sumRecords(records).finalApplicants).toBe(112);
  });
