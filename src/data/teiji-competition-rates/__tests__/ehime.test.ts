...
/**
 * T-Y11F §5順序#4 DoD検証（愛媛県・定時制課程11レコード）: 手入力した合計が、
 * 愛媛県教育委員会公表の「合計」（officialSubtotals）と一致することを機械的に突合する。
 */
describe('愛媛県 定時制課程 倍率パイプライン（T-Y11F §5順序#4）', () => {
  const { records, officialSubtotals } = EHIME_TEIJI_COMPETITION_RATES;

  it('取り込み件数は11レコード（10校・松山南は普通/商業の2レコード）', () => {
  ...
  it('全レコードの合計が公式「合計」と一致する', () => {
    ...
    expect(sumRecords(records).finalApplicants).toBe(166);
  });
