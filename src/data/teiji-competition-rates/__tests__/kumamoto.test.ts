...
/**
 * T-P1 P1-3 DoD検証（熊本県・定時制課程8校12レコード）: 手入力した合計が、
 * 熊本県教育委員会公表の「計」（officialSubtotals）と一致することを機械的に突合する。
 */
describe('熊本県 定時制課程 倍率パイプライン（T-P1 P1-3）', () => {
  const { records, officialSubtotals } = KUMAMOTO_TEIJI_COMPETITION_RATES;
  ...
  it('全レコードの合計が公式「計」と一致する', () => {
    const subtotal = officialSubtotals.find((s) => s.label === '計');
    if (!subtotal) throw new Error('officialSubtotals に "計" が見つかりません');
    const result = checkAgainstSubtotal(records, subtotal, () => true);
    expect(result.matches).toBe(true);
    expect(sumRecords(records).quota).toBe(440);
    expect(sumRecords(records).finalApplicants).toBe(136);
  });
