 ...
 * 胆振地区の定時制3校＝coverage='partial'）: ページ内に公式「計」行が無いため、個票の機械集計値
 * が自己検算どおりであることを検証する。北海道は15地域区分あるうち3区分のみ着手済みで、
 * coverage.status==='partial'であることも明示的に検証する（次回セッションが誤って「完了」と
 * 誤認しないため）。
 */
describe('北海道 定時制・有朋単位制 倍率パイプライン（T-P1 P1-3・partial・石狩+後志+胆振地区のみ）', () => {
  ...
  const ishikariTeiji = records.filter(
    (r) => !yuho.includes(r) && !shiribeshiTeiji.includes(r) && !iburiTeiji.includes(r)
  );

  it('coverage.statusはpartial（15地域区分中、石狩+後志+胆振の3区分のみ着手のため）', () => {
    expect(coverage.status).toBe('partial');
    expect(coverage.pendingDepartments.length).toBe(12);
  });

  it('取り込み件数は石狩定時制13＋有朋単位制2＋後志定時制4＋胆振定時制3=計22レコード', () => {
  ...
  it('石狩定時制は10校・後志定時制は4校・胆振定時制は3校（複数学科を持つ学校は学科ごとに別レコード）', () => {
