...
// 園部＋農芸は「前期選抜独自枠等募集要項（後半）.pdf」(`wp-content/uploads/2026/08/`
// 配下・全82頁)の頁1-39(印刷頁79-117)から収録。この文書は前半PDFの続き(印刷頁が
// 連番)で、印刷頁79(=東宇治)が物理的に前半の最終頁(79頁)を超えるため後半ファイルへ
// 分割されている。この範囲は頁1校=1頁(まれに1校2-4頁)の個別ページのみで構成され、
// 1頁の学校名が本文中に明記される曖昧性ゼロの情報源のため一覧表との突合は不要
// だった。京都八幡は北キャンパス(普通科総合選択制)と南キャンパス(介護福祉科・人間
// 科学科)が資料上も別ページ・別学科構成のため別学校として収録。京都八幡南キャンパス
// の「各学科15人」・田辺の「デザイン科・機械科・電気科・自動車科 各学科15人」はいずれも
// 学科ごとにレコードを分割。北桑田美山分校(頁108・農業科/家政科)は定時制(昼間)課程
// のため清明・京都奏和と同様に対象外。次の須知(頁118〜)以降は今回未収録。
//
...
export const KYOTO_SCHOOL_SELECTION_METHOD: PrefectureSchoolSelectionMethod = {
  ...
  status: 'structured',
  coverageNote:
    '「京都市・乙訓」学区26校(58レコード)＋「城陽」学区8校(18レコード)＋「相楽」学区3校(13レコード)＋北桑田(本校・2レコード)＋亀岡(4レコード)＋南丹(3レコード)＋園部(1レコード)＋農芸(1レコード)の計42校100レコードを収録。京都市・乙訓学区の山城〜東稜13校は一覧表(頁14-18)と個別学校ページ(頁21-51)の両方で行の帰属を突合できた学校、洛水〜開建13校は個別学校ページ(頁52-77)のみを直接転記(一覧表側の該当ページに文字化けがあったため)。城陽学区以降(東宇治〜農芸)は後半PDF頁1-39(印刷頁79-117)の個別学校ページのみで構成され一覧表との突合は不要だった。結合セルの帰属が一意に確定できない他学区・須知以降(口丹通学圏等)は今回見送り。清明高校(単位制による定時制・昼間二部)・京都奏和(単位制による定時制・昼間四部制)・北桑田美山分校(定時制・昼間課程)は本表の対象外(定時制は別掲)',
  source: {
    url: 'https://www.kyoto-be.ne.jp/koukyou/cms/wp-content/uploads/2026/09/前期選抜独自枠等募集要項（1前半）.pdf',
    docTitle: '京都府教育委員会「令和9年度京都府公立高等学校入学者選抜 前期選抜独自枠等募集要項」前半(頁14-18・頁21-77)＋後半(https://www.kyoto-be.ne.jp/koukyou/cms/wp-content/uploads/2026/08/前期選抜独自枠等募集要項（後半）.pdf・頁1-39=印刷頁79-117)',
