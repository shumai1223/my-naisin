...
// は「前期選抜独自枠等募集要項（後半）.pdf」(`wp-content/uploads/2026/08/`配下・
// 全82頁)の頁1-37(印刷頁79-115)から収録。この文書は前半PDFの続き(印刷頁が連番)で、
...
// のため清明・京都奏和と同様に対象外。次の園部(頁116〜)以降は今回未収録。
//
...
export const KYOTO_SCHOOL_SELECTION_METHOD: PrefectureSchoolSelectionMethod = {
  ...
  status: 'structured',
  coverageNote:
    '「京都市・乙訓」学区26校(58レコード)＋「城陽」学区8校(18レコード)＋「相楽」学区3校(13レコード)＋北桑田(本校・2レコード)＋亀岡(4レコード)＋南丹(3レコード)の計40校98レコードを収録。京都市・乙訓学区の山城〜東稜13校は一覧表(頁14-18)と個別学校ページ(頁21-51)の両方で行の帰属を突合できた学校、洛水〜開建13校は個別学校ページ(頁52-77)のみを直接転記(一覧表側の該当ページに文字化けがあったため)。城陽学区8校・相楽学区3校・北桑田本校・亀岡・南丹は後半PDF頁1-37(印刷頁79-115)の個別学校ページのみで構成され一覧表との突合は不要だった。結合セルの帰属が一意に確定できない他学区・園部以降(口丹通学圏等)は今回見送り。清明高校(単位制による定時制・昼間二部)・京都奏和(単位制による定時制・昼間四部制)・北桑田美山分校(定時制・昼間課程)は本表の対象外(定時制は別掲)',
  source: {
    url: 'https://www.kyoto-be.ne.jp/koukyou/cms/wp-content/uploads/2026/09/前期選抜独自枠等募集要項（1前半）.pdf',
    docTitle: '京都府教育委員会「令和9年度京都府公立高等学校入学者選抜 前期選抜独自枠等募集要項」前半(頁14-18・頁21-77)＋後半(https://www.kyoto-be.ne.jp/koukyou/cms/wp-content/uploads/2026/08/前期選抜独自枠等募集要項（後半）.pdf・頁1-37=印刷頁79-115)',
