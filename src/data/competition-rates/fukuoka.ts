 ...
 * ＋PDF4ページ目（最終ページ）全11校（田川・東鷹・田川科学技術・稲築志耕館・嘉穂・嘉穂東・
 * 嘉穂総合・鞍手・直方・筑豊・鞍手竜徳）＝機械的カウントで89校169レコードを確定済み。
 * 2ページ目は宗像から始まる。
 *
 ...
 * 完全一致することを確認した上で採用した。
 *
 * ⚠️2026-07-25判明（グランドトータル突合の不一致・未解決）: recordsを機械的に集計すると
 * 89校169レコード・quota合計21,960・applicants合計22,578（倍率1.028）となり、PDF末尾の
 * グランドトータル（90校・quota22,200・applicants22,854・倍率1.03）と一致しない
 * （差分: 1校・quota240・applicants276）。4ページとも各校を漏れなく転記したはずだが、
 * どこかで1校を読み落としている可能性が高い。差分の大きさ（quota240=6学科相当の中規模校、
 * またはquota240の単科校1つ分）から、次回セッションは①各ページを再度通読し校数を
 * 数え直す（27+30+21+11=89校という前提自体が誤っている可能性）②市組合立側に混入していない
 * か確認、の両方を試すこと。この不一致が解消するまでcoverage.statusは'complete'にしない。
 *
...
export const FUKUOKA_COMPETITION_RATES: PrefectureCompetitionRateFile = {
  ...
  coverage: {
    ...
    includedDepartments: [
      ...
      '県立全日制（PDF4ページ目=最終ページ・田川〜鞍手竜徳の11校全て。うち10校は英進館筑豊地区記事で外部裏取り、田川科学技術はΣ子学科=計行の自己検算で採用）',
    ],
    pendingDepartments: [
      '県立全日制（グランドトータル不一致・89校169レコードの機械集計がPDF末尾の90校/quota22,200/applicants22,854と一致せず、差分1校・quota240・applicants276が未解決。全4ページを転記したはずだが1校の見落としが疑われる）',
      '市組合立全日制（別PDF・uploaded/life/806459_62802784_misc.pdf・未着手。南筑（久留米市立）はこちらに含まれる見込み）',
    ],
    note: '福岡県は資料が複数ページ＋県立/市組合立の別PDFに分かれており、今回はPDF1〜4ページ目の全89校169レコードを高確信度で確定した（4ページとも各校を漏れなく転記したつもり）。PDF4ページ目末尾に県立合計行（90校・定員22,200・確定志願者22,854・倍率1.03）があり、これはリセモム記事の数値と完全一致する信頼できる値だが、本ファイルの機械集計（89校・quota21,960・applicants22,578）とは一致しない（差分1校・quota240・applicants276）。原因は未特定（1校の見落とし、または既存レコードのどこかに転記ミスがある可能性）で、次回セッションが優先的に解決すべき課題として記録する。',
  },
  officialSubtotals: [
