 ...
 * ⚠️未解決の小さな差異: 専門学科+総合学科の合算applicants(6,708)＋普通科計(35,530)=42,238だが、
 * 第1弾で記録した総括表(01_teisei_2)のapplicants合計は42,236で2件の差がある（quotaは30,825で完全一致）。
 * 本弾のPDF3内の全17小計は個別に完全一致しており本弾側の転記誤りとは考えにくいため、原因は
 * 第1弾が総括表を読んだ際の転記（または訂正前後の混同）の可能性がある。次にtokyoへ戻る回で
 * 01_teisei_2を再取得し総括表のapplicants合計を再確認すること。
 */
