 ...
 * 記載済みで一定の下地あり）。
 *
 * ⚠️対象範囲=現時点でPDF2ページ目（全日制・東灘〜宝塚東、43校）のみを高い確信度で確定済み。
 * 全体は10ページ・全日制127校（県立115校・市立12校）＋定時制19校（県立15校・市立4校）の
 * 大規模資料と判明済み（1ページ目のグランドトータルで確認: 全日制定員21,150・確定志願者
 * 20,567・倍率0.97）。残り8ページ（3〜10ページ目）は未転記のまま次回に持ち越し。
 *
...
export const HYOGO_COMPETITION_RATES: PrefectureCompetitionRateFile = {
  ...
  coverage: {
    status: 'partial',
    includedDepartments: ['全日制（PDF2ページ目・東灘〜宝塚東の43校）'],
    pendingDepartments: [
      '全日制（PDF3〜10ページ目、残り約84校。127校中43校のみ着手済み）',
      '定時制（全日制の外側の別課程のため東京都・神奈川県・千葉県・埼玉県・福岡県と同じ理由で意図的にスコープ外）',
    ],
    note:
      '兵庫県は全日制127校（県立115校・市立12校）が県立/市立を区別せず1つのPDF（全10ページ）に' +
      '収録されている大規模資料。今回はPDF2ページ目の43校のみを高確信度で確定（1ページ目の' +
      'グランドトータル: 全日制定員21,150・確定志願者20,567・倍率0.97は確認済みだが、43校のみでの' +
      '部分突合はまだ行っていない）。残り8ページは次回以降のセッションで継続する。',
  },
  officialSubtotals: [],
  records: [
