 ...
 * +留萌(全日制)+宗谷(全日制)+オホーツク(全日制)+十勝(全日制)+釧路(全日制)）。
 *
 ...
 * 分に着手した（詳細はhokkaido.tsのコメント参照）。各行「finalApplicants ÷ quota ≒ finalRate」の
 * 内部整合性のみを検証する（公式グランドトータル行は原資料に印字されておらず突合対象が無いため）。
 */
describe('北海道 倍率パイプラインα（Y-6・空知29+石狩31+札幌市9+後志6+胆振11+日高7+渡島29+檜山4+上川37+留萌7+宗谷8+オホーツク31+十勝28+釧路21=258レコード・coverage=partial）', () => {
  ...
  it('258レコードが収録されている', () => {
    expect(records.length).toBe(258);
  });
