 ...
 */
describe('北海道 倍率パイプラインα（Y-6・R8=323レコード＋掛-1(空知+石狩+札幌市):R7=97レコード＝420レコード・coverage=partial・全14管内着手済み）', () => {
  ...
  it('420レコードが収録されている(R8年度323+R7年度97)', () => {
    expect(records.length).toBe(420);
    expect(records.filter((r) => r.fiscalYear === '令和7年度（2025年度）').length).toBe(97);
