 ...
 */
describe('北海道 倍率パイプラインα（Y-6・R8=323レコード＋掛-1(全14管内R7完走):R7=325レコード＋掛-1(14年度目・空知+石狩+札幌市+後志+胆振+日高+渡島+檜山+上川+留萌+宗谷+オホーツク+十勝+釧路):R6=318レコード＝966レコード・coverage=partial）', () => {
  ...
  it('966レコードが収録されている(R8年度323+R7年度325+R6年度318(空知32+石狩57+札幌市9+後志17+胆振29+日高7+渡島30+檜山4+上川38+留萌7+宗谷8+オホーツク31+十勝28+釧路21))', () => {
    ...
    expect(records.filter((r) => r.fiscalYear === '令和6年度（2024年度）').length).toBe(318);
  });
