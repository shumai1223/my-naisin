 ...
 */
describe('北海道 倍率パイプラインα（Y-6・R8=323レコード＋掛-1(空知+石狩+札幌市+後志+胆振+日高+渡島+檜山+上川+留萌+宗谷):R7=234レコード＝557レコード・coverage=partial・全14管内着手済み）', () => {
  ...
  it('557レコードが収録されている(R8年度323+R7年度234)', () => {
    expect(records.length).toBe(557);
    expect(records.filter((r) => r.fiscalYear === '令和7年度（2025年度）').length).toBe(234);
