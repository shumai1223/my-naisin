...
/**
 * Y-6 DoD検証（北海道・coverage='partial'・空知地区+石狩地区(全日制普通科)）。
 *
 * 北海道は2026-08-06までΛ-4・Y-6いずれも恒久ブロック扱いだったが、教委公式ページの
 * 別シリーズ資料を発見し空知地区・石狩地区(普通科)分に着手した（詳細はhokkaido.tsのコメント参照）。
 * 各行「finalApplicants ÷ quota ≒ finalRate」の内部整合性のみを検証する
 * （公式グランドトータル行は原資料に印字されておらず突合対象が無いため）。
 */
describe('北海道 倍率パイプラインα（Y-6・空知29+石狩31=60レコード・coverage=partial）', () => {
