 ...
 */
describe('北海道 倍率パイプラインα（Y-6・R8=323レコード＋掛-1(全14管内R7完走):R7=325レコード＋掛-1(4年度目・空知+石狩+札幌市+後志):R6=115レコード＝763レコード・coverage=partial）', () => {
  ...
  it('763レコードが収録されている(R8年度323+R7年度325+R6年度115(空知32+石狩57+札幌市9+後志17))', () => {
    expect(records.length).toBe(763);
    expect(records.filter((r) => r.fiscalYear === '令和7年度（2025年度）').length).toBe(325);
    expect(records.filter((r) => !r.fiscalYear).length).toBe(323);
    expect(records.filter((r) => r.fiscalYear === '令和6年度（2024年度）').length).toBe(115);
  });
