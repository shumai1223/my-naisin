...
describe('宮崎県 倍率パイプラインα（Y-6・全日制34校104レコードの完全収録テスト）', () => {
  ...
  it('掛-1(学校別×多年度): 令和6年度(R6)分レコードが104件・34校収録され、公式「全日制合計」3,948/3,190と完全一致する。R7と学校名+学科名の組み合わせが完全一致する(学校再編なし・飯野「普通」もR6時点は改称前)', () => {
    const r6 = records.filter((r) => r.fiscalYear === '令和6年度（2024年度）');
    expect(r6.length).toBe(104);
    ...
    expect(sumQuota).toBe(3948);
    expect(sumApplicants).toBe(3190);
  ...
  it('掛-1(学校別×多年度): 令和5年度(R5)分レコードが104件・34校収録され、公式「全日制合計」4,106/3,514と完全一致する。R6と学校名+学科名の組み合わせが完全一致する(学校再編なし・飯野「普通」もR5時点は改称前)', () => {
    ...
    expect(sumQuota).toBe(4106);
    expect(sumApplicants).toBe(3514);
