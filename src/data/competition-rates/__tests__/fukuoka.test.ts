 ...
 * 全27校（青豊〜遠賀）＋2ページ目27校（宗像〜糸島）の計54校・84レコードのみを対象とした
 * 正直な部分収録。2ページ目のうち筑紫丘〜糸島の15校、および1ページ目の北筑は、PDF自体の
 * 視覚読み取りが試行のたびに食い違ったため、外部の学習塾サイト記事から引用しrate整合性で
 * 裏取りした。県レベルの公式合計との突合はまだできない（残りページ未読のため）ので、代わりに
 * 複数学科を持つ学校について、学校単位でPDFに印字された「計」行との完全一致を検証する。
 */
describe('福岡県 倍率パイプラインα（Y-2・PDF1ページ目+2ページ目 計54校の部分収録テスト）', () => {
  ...
  it('84レコードが収録されている（1ページ目27校+2ページ目27校=計54校）', () => {
    expect(records.length).toBe(84);
  });
