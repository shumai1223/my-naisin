 ...
 * 全21校＋2ページ目冒頭5校（宗像・光陵・水産・玄界・新宮）の計26校・41レコードのみを
 * 対象とした正直な部分収録。県レベルの公式合計との突合はまだできない（残りページ未読の
 * ため）ので、代わりに複数学科を持つ学校について、学校単位でPDFに印字された「計」行、
 * または外部の学習塾サイトの入試速報記事で裏取りした値との完全一致を検証する。
 */
describe('福岡県 倍率パイプラインα（Y-2・PDF1ページ目+2ページ目冒頭 計26校の部分収録テスト）', () => {
  ...
  it('複数学科を持つ8校すべてで、学科別内訳の合計がPDF記載の学校単位「計」行（または外部裏取り値）と完全一致する', () => {
  ...
  it('41レコードが収録されている（1ページ目21校+2ページ目冒頭5校=計26校）', () => {
