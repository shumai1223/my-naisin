 ...
 * 全27校（青豊〜遠賀）＋2ページ目30校（宗像〜福岡魁誠等12校+筑紫丘〜糸島15校+福岡工業/
 * 福岡農業/糸島農業3校）の計57校・100レコードのみを対象とした正直な部分収録（PDF2ページ目
 * は完了）。2ページ目のうち筑紫丘〜糸島の15校・福岡工業/福岡農業/糸島農業の3校、および
 * 1ページ目の北筑は、PDF自体の視覚読み取りが試行のたびに食い違ったため、外部の学習塾
 * サイト記事から引用しrate整合性で裏取りした。県レベルの公式合計との突合はまだできない
 * （残りページ未読のため）ので、代わりに複数学科を持つ学校について、学校単位でPDFに
 * 印字された「計」行との完全一致を検証する。
 */
describe('福岡県 倍率パイプラインα（Y-2・PDF1ページ目+2ページ目 計57校の部分収録テスト）', () => {
  ...
  it('複数学科を持つ13校すべてで、学科別内訳の合計がPDF記載の学校単位「計」行（または外部裏取り値）と完全一致する', () => {
  ...
  it('100レコードが収録されている（1ページ目27校+2ページ目30校=計57校）', () => {
