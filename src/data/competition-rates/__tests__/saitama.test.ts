...
describe('埼玉県 倍率パイプラインα（Y-2・全日制の突合テスト）', () => {
  ...
  it('総合学科の合計が公式値と完全一致する', () => {
    const result = checkAgainstSubtotal(records, findSubtotal('総合学科計'), (r) => r.department === '総合学科' && !r.fiscalYear);
    expect(result.matches).toBe(true);
  });
  ...
  it('工業に関する学科（13校46レコード）の合計が公式値と完全一致する', () => {
    ...
    const result = checkAgainstSubtotal(
      ...
      (r) => kougyouSchools.has(r.schoolName) && kougyouDepartments.has(r.department) && !r.fiscalYear
    );
    expect(result.matches).toBe(true);
  });
  ...
  it('専門学科の各小科群（商業・家庭・看護・外国語・美術・音楽・書道・体育・理数・情報・福祉・人文・国際関係・映像芸術・舞台芸術・生物環境系）が公式値と完全一致する', () => {
    ...
    for (const [label, predicate] of checks) {
      const result = checkAgainstSubtotal(records, findSubtotal(label), (r) => predicate(r) && !r.fiscalYear);
      expect(result.matches).toBe(true);
    }
  });
  ...
  it('商業に関する学科の合計が公式値と完全一致する（狭山経済・情報処理科の転記ミス修正済み）', () => {
    ...
    const result = checkAgainstSubtotal(
      ...
      (r) => shougyouSchools.has(r.schoolName) && shougyouDepartments.has(r.department) && !r.fiscalYear
    );
    expect(result.matches).toBe(true);
  });
  ...
  it('掛-1(学校別×多年度): 令和7年度(R7)分レコードが240件収録され(1〜8頁目完了・全日制が完結)、区市町村+学校名+学科の重複が無い', () => {
    ...
    expect(sumQuota).toBe(35001);
    expect(sumApplicants).toBe(38587);
