...
describe('鹿児島県 倍率パイプラインα（Y-6・全日制68校156レコードの完全収録テスト）', () => {
  ...
  it('掛-1(学校別×多年度): 令和4年度(R4)分レコードが153件・68校収録され、「全日制合計」(quota11,187・applicants9,155)と完全一致する。7学区すべての学区合計行(鹿児島3822/4004・南薩1105/777・北薩1603/1085・姶良伊佐1719/1357・大隅1554/1078・熊毛398/223・大島986/631)とも完全一致する。学校名+学科名のキー集合はR5と完全一致(差分0件・統廃合なし)。喜界(商業)がR4時点でも最終出願者数0だった', () => {
    const r4 = records.filter((r) => r.fiscalYear === '令和4年度（2022年度）');
    const r5 = records.filter((r) => r.fiscalYear === '令和5年度（2023年度）');
    expect(r4.length).toBe(153);
    const distinctSchools = new Set(r4.map((r) => r.schoolName));
    expect(distinctSchools.size).toBe(68);
    expect(r4.reduce((a, r) => a + r.quota, 0)).toBe(11187);
    expect(r4.reduce((a, r) => a + r.finalApplicants, 0)).toBe(9155);
    ...
    const expectedDistrictTotals: Record<string, { quota: number; applicants: number }> = {
      鹿児島: { quota: 3822, applicants: 4004 },
      南薩: { quota: 1105, applicants: 777 },
      北薩: { quota: 1603, applicants: 1085 },
      姶良伊佐: { quota: 1719, applicants: 1357 },
      大隅: { quota: 1554, applicants: 1078 },
      熊毛: { quota: 398, applicants: 223 },
      大島: { quota: 986, applicants: 631 },
    };
    for (const [district, schoolNames] of Object.entries(districts)) {
      const recs = r4.filter((r) => schoolNames.includes(r.schoolName));
      const exp = expectedDistrictTotals[district];
      expect(recs.reduce((a, r) => a + r.quota, 0)).toBe(exp.quota);
      expect(recs.reduce((a, r) => a + r.finalApplicants, 0)).toBe(exp.applicants);
    }

    expect(r4.find((r) => r.schoolName === '喜界' && r.department === '商業')).toMatchObject({ finalApplicants: 0 });
