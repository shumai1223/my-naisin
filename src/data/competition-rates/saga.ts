 ...
 *
 * ⚠️T-Y11 B-1（2026-09-01・R6追加を試みたが取得不能）: 令和6年度(2024年度)の「一般選抜志願状況
 * （志願変更後）」告知ページ(`kiji003101127`)を発見したが、現行サイトは404・Wayback CDXでは
 * 2件のスナップショット(2024-04-13/2024-08-15)が確認できたものの、取得しようとした時点で
 * Internet Archive自体がサービス一時停止（`Temporarily Offline`）しており複数回リトライしても
 * 本文（PDFリンク）を取得できなかった。推測で埋めず、R6は本ファイルへ未収録のまま「取得不能」
 * として記録する。次に触れるセッションはWaybackの復旧後に上記2スナップショットへの再アクセスを
 * 試すこと（URLは判明済みなので探索からやり直す必要はない）。
 */
