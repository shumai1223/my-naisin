// 鳥取県: 県立高校(全日制普通科)の学区制度は平成19年度（2007年度）に廃止され、現在は「全県一区」
...
// pdftoppmで目視確認)に「H19　群馬、山梨、鳥取」と明記(gunma/yamanashi実装時に同じ行で既に
// 目視確認済み)。WebSearchの二次資料でも「2007年(平成19年)4月から全日制普通科高等学校の学区が
// 廃止され全県一学区となった」という一致した記述を確認した。
//
...
export const TOTTORI_SCHOOL_DISTRICT: PrefectureSchoolDistrict = {
  ...
  outOfDistrictCondition:
    '学区制度自体が無いため、現在は居住地に関わらずどの県立高校にも出願可能(全日制普通科が対象)。廃止前の学区数・名称・区割りは今回一次資料・二次資料とも見つけられず未確認',
  source: {
  ...
  note: '廃止年度は同一PDF行(H19・群馬/山梨/鳥取)でgunma/yamanashi実装時に一次確認済み。廃止理由等の詳細は鳥取県教育委員会の一次資料には到達できず、WebSearchの二次資料要約のみで確認。これでT-Y15は47/47都道府県の構造化データが完成',
};
