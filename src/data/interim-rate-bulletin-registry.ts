 ...
 * 調査を完走した。confirmed-multistage 33県・presumed-multistage 1県（佐賀）・unconfirmed 12県
 * （島根・青森・宮城・岡山・長崎の5県＋interimIncludesRate等が未確定のまま残る県を含む、詳細は各エントリの
 * note参照）。unconfirmed/presumed止まりの県は値を推測で埋めておらず、今後の追加調査（一次資料の直接確認）
 * で更新されるべき対象として明示している（Y-0憲法③「機械可読不能・未確認は正直にスキップ」の精神）。
...
export const INTERIM_BULLETIN_REGISTRY: InterimBulletinPrefectureEntry[] = [
  ...
  },
  {
    ...
    confidence: 'medium',
    note:
      '進研ゼミに一般選抜の「倍率速報(志願変更前)」(2026年度0815)と「倍率速報(志願変更後)」(2025年度' +
      '1569458_5863/1055)の別記事・別URLが複数年度にわたり存在し2段階構造を確認。ただしczemi個別ページへの' +
      'WebFetchが403で拒否されたため、速報段階の学校別倍率の具体的な数値は今回直接確認できず' +
      '(interimIncludesRateは保留)。リセマムの確定記事は城西(生産技術)1.44倍等の確定値のみ。要追加調査。',
    investigatedAt: '2026-08-22',
  },
  {
  ...
  },
  {
    ...
    confidence: 'low',
    note:
      '進研ゼミには「一般選抜 倍率速報」(0821)と「一般選抜 倍率速報(高校別・学科別)」(1062)の2記事が' +
      '存在するが、両者は時期(志願変更前/後)でなく粒度(県計/学校別)の違いとみられ、他県のような時系列的な' +
      '別公表物とは性質が異なる。長崎県公式サイトへのWebFetchはコンテンツ空白で確認できず、WebSearchでは' +
      '「志願状況(確定)を2/9発表」という1回分の発表しか見つからなかった。先行速報の実在を断定できないため' +
      '無理に「confirmed」にせず「unconfirmed」のまま正直に記録する(捏造ゼロ原則)。要追加調査' +
      '(公式サイト一次情報の直接確認)。',
    investigatedAt: '2026-08-22',
  },
  {
  ...
  },
  {
    ...
    note:
      ...
      '報じており、要約中の「鹿児島南(体育)1.50倍」等との整合性も未確認)。confidenceは中位に留め、' +
      '速報段階の学校別倍率の実数値は要追加調査(一次情報の直接確認)とする。',
    investigatedAt: '2026-08-22',
  },
];
