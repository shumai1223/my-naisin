 ...
 * 本ファイルはこれを都道府県別に構造化する（2026-08-22時点でα+第2+第3バッチの計30県まで進捗・残り17県）。
 *
 * ## スコープと除外
 ...
 *
 * ## 次への引き継ぎ
 * 1. unconfirmedの17県（千葉・埼玉・福岡・北海道・静岡・青森・岩手・宮城・秋田・福島・茨城・群馬・石川・
 *    山梨・岐阜・三重・滋賀）の一次ソース再調査。群馬・岐阜・滋賀は制度名・金額の手がかり（Google索引や
 *    第三者PDF）が既にあるため、正しいURLの再探索から始めると効率的。
 * 2. confidence:mediumの兵庫（金額）・奈良（金額・別添PDF）を一次ソースで直接確認する。
 * 3. 京都のB/C所得区分（590万〜910万円未満相当）の金額を追加確認する。
 * 4. 残り17県（栃木を除く47県からこれまでの30県を引いた残り）への横展開（本ファイルの型・調査手法をそのまま
 *    踏襲できる）。
