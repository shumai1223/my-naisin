 ...
 *
 * ## 次への引き継ぎ
 * 1. ✅2026-08-23一部完了: confidence:mediumだった5県（兵庫・奈良・鳥取・広島・徳島）のリーフレットPDFを
 *    直接開いて確認した結果、**鳥取・広島の2県はconfidence:highへ格上げ**（県独自上乗せ分の具体的な金額
 *    がグラフ・表で明記されていた）。**兵庫・奈良・徳島の3県は依然としてmediumのまま**（兵庫・徳島は
 *    リーフレットに合算後の上限額のみでグラフに数値ラベルが無く内訳不明・奈良は報道記事に「最大63万円」
 *    とあるが公式ページ本文には非掲載）。残る3県はさらに別の一次資料（交付要綱PDF等）を探す余地がある。
...
export const PREFECTURE_TUITION_SUBSIDY_REGISTRY: PrefectureTuitionSubsidyEntry[] = [
  ...
  },
  {
    ...
    subsidyAmountNote:
      '2026-08-23にPDFリーフレット（r8sienseido1_zennitisei.pdf・私立全日制高校向け）を直接開いて確認した' +
      'ところ、「就学支援金」（国制度＋県上乗せの合算値とみられる）の支援限度額（年額）は457,200円と明記' +
      'されていたが、これは私立高校授業料の全国共通上限額と同一の数値であり、県独自上乗せ分のみを分離した' +
      '所得区分別金額（64,000円/120,000円/60,000円等・WebSearch要約由来）はこのリーフレットには記載が無く、' +
      '依然として独立確認できていない。金額（区分別内訳）は未確認情報のまま扱う。',
    source: {
    ...
    note:
      '制度の実在自体は公式ページで確認済み。2026-08-23にPDFリーフレット（documents/r8sienseido1_zennitisei.pdf）' +
      'を直接開いたが、記載は合算後の支援限度額457,200円のみで、県独自上乗せ分の所得区分別内訳は確認できず' +
      'confidence: mediumのまま維持する（捏造回避）。',
  ...
  },
  {
    ...
    eligibilityNote:
      ...
      '撤廃したと公式ページに明記。',
    subsidyAmountNote:
      '公式ページ本文には所得区分別の具体的な円額の記載がなく、「対象校の授業料等一覧」という別添PDF' +
      '（documents/5387/r7jyugyouryou_1.pdf・学校ごとの授業料一覧表であり補助金額表ではないと2026-08-23に' +
      '確認済み）を参照する案内のみだったため、金額は未転記（捏造回避）。日本経済新聞・リセマム等の報道記事は' +
      '「全世帯に最大63万円を支援」と報じているが、この金額は公式ページ本文には出現せず（2026-08-23に' +
      '再度WebFetchで確認）、報道のみを根拠に金額を確定させることは避け、未確認情報として扱う。',
    source: {
    ...
    note:
      '制度の実在・対象要件（所得制限撤廃）は公式ページで確認済み。2026-08-23に別添PDFと公式ページ本文を' +
      '再確認したが、具体的な補助金額（最大63万円という報道はあるが公式ページには非掲載）は依然として' +
      '一次ソースで直接確認できず、confidence: mediumのまま維持する（捏造回避）。',
    investigatedAt: '2026-08-23',
  },
  {
  ...
  },
  {
    ...
    targetCourse: 'private',
    eligibilityNote:
      '鳥取県内の私立高等学校または専修学校高等課程に在学し、鳥取県で国の高等学校等就学支援金を受給して' +
      'いる方が対象。保護者等の年収目安が270万円以上（4人家族モデル）の場合は支給されない（公式リーフレット' +
      'に明記）。支給額は「市町村民税の課税標準額×6％−市町村民税の調整控除の額」という算出式で判定。',
    subsidyAmountNote:
      '2026-08-23にPDFリーフレット（R8sougousienkin.pdf）を直接開いて確認済み。算出額が100円未満（年収目安' +
      '270万円未満相当）の世帯は月額3,600円、生活保護受給世帯は月額7,200円（いずれも「各校で定める額の' +
      'いずれか低い額」）。国の就学支援金（月額38,100円）への上乗せという位置付けで、県制度単独の金額が' +
      'グラフ付きで明記されていた。',
    source: {
      url: 'https://www.pref.tottori.lg.jp/secure/1312397/R8sougousienkin.pdf',
      docTitle: '鳥取県「鳥取県私立高等学校等総合支援金」リーフレット（令和8年6月）',
      lastChecked: '2026-08-23',
    },
    note:
      '2026-08-23にWebFetch+ローカルpdftoppmでPDFリーフレットを画像化し目視確認済み。県独自上乗せ分の' +
      '金額（月額3,600円/7,200円）が算出式・グラフとともに明記されておりconfidence: highへ格上げした。',
    investigatedAt: '2026-08-23',
  },
  {
  ...
  },
  {
    ...
    targetCourse: 'private',
    eligibilityNote:
      '生活保護受給世帯、または保護者どちらか一方が働き子供2人（高校生・中学生）の世帯モデルで年収目安' +
      '350万円未満の世帯が対象（公式リーフレットに明記）。県が国の高等学校等就学支援金に上乗せして助成する' +
      '制度で、学校が受け取り授業料等から差し引く仕組み。',
    subsidyAmountNote:
      '2026-08-23にPDFリーフレット（664134.pdf）を直接開いて確認済み。授業料等の上限年額600,000円' +
      '（月額50,000円・国の就学支援金との合計額）、入学時納入金は180,000円が上限。国の就学支援金単独の' +
      '上限（年額457,200円）と対比する形で図示されており、県独自上乗せ分を含む合算上限額として明記されて' +
      'いる。',
    source: {
      url: 'https://www.pref.hiroshima.lg.jp/uploaded/attachment/664134.pdf',
      docTitle: '広島県「生徒・保護者の皆様へ（私立高等学校等の教育費負担軽減制度の御案内）」リーフレット',
      lastChecked: '2026-08-23',
    },
    note:
      '2026-08-23にWebFetch+ローカルpdftoppmでPDFリーフレットを画像化し目視確認済み。通常課程（全日制等）' +
      '向けの具体的な合算上限額（年額600,000円）が明記されておりconfidence: highへ格上げした。',
    investigatedAt: '2026-08-23',
  },
  {
  ...
  },
  {
    ...
    subsidyAmountNote:
      '2026-08-23にPDFリーフレット（1063049.pdf・私立高校における授業料等軽減の補助について）を直接開いて' +
      '確認済み。令和8年度より所得制限が撤廃され支給上限額も引き上げられたことは図示されているが、県独自' +
      '上乗せ分の具体的な円額は棒グラフの色分けのみで数値ラベルが無く読み取れなかった（国の就学支援金部分' +
      '＝457,200円/年は明記）。したがって所得区分別の具体的な金額は依然として未転記（捏造回避）。',
    source: {
    ...
    note:
      '制度の実在・学校経由の補填の仕組みは公式ページで確認済み。2026-08-23にPDFリーフレット' +
      '（1063049.pdf）を画像化して確認したが、県独自上乗せ分は令和8年度に所得制限撤廃・上限引き上げが' +
      'あったことは分かったものの、具体的な円額はグラフに数値ラベルが無く読み取れず、confidence: medium' +
      'のまま維持する（捏造回避）。',
    investigatedAt: '2026-08-23',
  },
  {
