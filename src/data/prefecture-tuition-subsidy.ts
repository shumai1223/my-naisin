 ...
 * 本ファイルはこれを都道府県別に構造化する（2026-08-22時点でα+第2+第3+第4バッチの計40県まで進捗・残り7県）。
 *
 * ## スコープと除外
 ...
 *
 * ## 次への引き継ぎ
 * 1. unconfirmedの21県（千葉・埼玉・福岡・北海道・静岡・青森・岩手・宮城・秋田・福島・茨城・群馬・石川・
 *    山梨・岐阜・三重・滋賀・和歌山・山口・愛媛・高知）の一次ソース再調査。群馬・岐阜・滋賀は制度名・
 *    金額の手がかり（Google索引や第三者PDF）が既にあるため、正しいURLの再探索から始めると効率的。
 * 2. confidence:mediumの兵庫・奈良・鳥取・広島・徳島（いずれも金額がリーフレットPDF参照のみ）を、
 *    可能であればPDFの直接確認まで踏み込んで金額を補完する。
 * 3. 京都のB/C所得区分（590万〜910万円未満相当）の金額を追加確認する。
 * 4. 残り7県（栃木を除く47県からこれまでの40県を引いた残り: 大分・佐賀・長崎・熊本・宮崎・鹿児島・沖縄）
 *    への横展開（本ファイルの型・調査手法をそのまま踏襲できる）。
 * 5. `/kyouiku-hi`・`/shougakukin`群との結線（フロントエンド表示）は残り県数がある程度揃ってから。
 */
