...
describe('KAGAWA_EXAM_SCHEDULE', () => {
  it('has at least one fiscal year with events', () => {
    expect(KAGAWA_EXAM_SCHEDULE.prefectureCode).toBe('kagawa');
    expect(KAGAWA_EXAM_SCHEDULE.years.length).toBeGreaterThan(0);
    for (const year of KAGAWA_EXAM_SCHEDULE.years) {
      expect(year.events.length).toBeGreaterThan(0);
      expect(year.sourceUrl).toMatch(/^https:\/\/www\.pref\.kagawa\.lg\.jp\//);
    }
  });
  ...
  it('finds the main (一般選抜) exam day and result date by label', () => {
    const exam = findScheduleEvent(KAGAWA_EXAM_SCHEDULE, '令和8年度（2026年度）', '一般選抜 学力検査');
    expect(exam?.startDate).toBe('2026-03-10');

    const result = findScheduleEvent(KAGAWA_EXAM_SCHEDULE, '令和8年度（2026年度）', '一般選抜 合格発表');
    expect(result?.startDate).toBe('2026-03-19');
  });

  it('covers both 自己推薦選抜 and 一般選抜 tracks', () => {
    const labels = KAGAWA_EXAM_SCHEDULE.years[0].events.map((e) => e.label);
    expect(labels.some((l) => l.startsWith('自己推薦選抜'))).toBe(true);
