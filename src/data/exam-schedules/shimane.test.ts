...
describe('SHIMANE_EXAM_SCHEDULE', () => {
  ...
  it('has 令和9年度 entries matching the published R9 calendar (no fabricated application periods)', () => {
    const exam = findScheduleEvent(SHIMANE_EXAM_SCHEDULE, '令和9年度（2027年度）', '一般選抜 学力検査');
    expect(exam?.startDate).toBe('2027-03-03');

    const result = findScheduleEvent(SHIMANE_EXAM_SCHEDULE, '令和9年度（2027年度）', '一般選抜等 合格発表');
    expect(result?.startDate).toBe('2027-03-12');

    const labels = SHIMANE_EXAM_SCHEDULE.years[1].events.map((e) => e.label);
