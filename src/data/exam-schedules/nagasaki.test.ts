...
describe('NAGASAKI_EXAM_SCHEDULE', () => {
  it('has at least one fiscal year with events', () => {
    expect(NAGASAKI_EXAM_SCHEDULE.prefectureCode).toBe('nagasaki');
    expect(NAGASAKI_EXAM_SCHEDULE.years.length).toBeGreaterThan(0);
    for (const year of NAGASAKI_EXAM_SCHEDULE.years) {
      expect(year.events.length).toBeGreaterThan(0);
      expect(year.sourceUrl).toMatch(/^https:\/\/www\.pref\.nagasaki\.jp\//);
    }
  });
  ...
  it('finds the main (一般選抜) exam day and result date by label', () => {
    const exam = findScheduleEvent(NAGASAKI_EXAM_SCHEDULE, '令和8年度（2026年度）', '一般選抜 学力検査');
    expect(exam?.startDate).toBe('2026-02-17');
    expect(exam?.endDate).toBe('2026-02-18');

    const result = findScheduleEvent(NAGASAKI_EXAM_SCHEDULE, '令和8年度（2026年度）', '一般選抜 合格者発表');
    expect(result?.startDate).toBe('2026-03-05');
  });

  it('covers 特別選抜・一般選抜・チャレンジ選抜 tracks', () => {
    const labels = NAGASAKI_EXAM_SCHEDULE.years[0].events.map((e) => e.label);
    expect(labels.some((l) => l.startsWith('特別選抜'))).toBe(true);
    expect(labels.some((l) => l.startsWith('一般選抜'))).toBe(true);
    expect(labels.some((l) => l.startsWith('チャレンジ選抜'))).toBe(true);
  });
