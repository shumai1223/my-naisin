...
/**
 * 北海道 段階台帳（T-Y11F §5順序#7・28県目・全日制coverage='partial'・空知地区29レコードで
 * 着手・全14管内中の1管内目）。
 *
...
export const HOKKAIDO_STAGE_LEDGER: PrefectureStageLedgerFile = {
  ...
  coverage: {
    status: 'partial',
    includedDepartments: ['全日制・空知地区（普通教育を主とする学科12レコード＋専門教育を主とする学科及び総合学科17レコード＝29レコード）'],
    pendingDepartments: [
      '全日制・石狩地区（札幌市を含む道内最大地区）',
      '全日制・後志地区',
      '全日制・胆振地区',
      '全日制・日高地区',
      '全日制・渡島地区',
    ...
    note: '全14管内のうち空知地区（29レコード）のみ着手。quota・applicantsConfirmedは既存パイプライン`competition-rates/hokkaido.ts`の空知地区29レコードをそのまま再利用し、testTakersConfirmed（第1次受検者数＋第2次受検者数）・finalPassers（入学者数＝第1次合格者数＋第2次合格者数）を「§3学校別受検者数及び合格者数」p.9から新規転記した。推薦枠は一般枠と完全に独立したクオータ（秋田のような推薦落選者の一般転入は無い）のためスコープ外。既知の例外5件（第2次募集による新規応募者分でtestTakersConfirmedがapplicantsConfirmedを僅かに上回る4件、追加合格者と推測される岩見沢緑陵「普通」1件）はいずれも+1の小差。本資料はさらに13管内分（全320レコード規模）を残しており、既存パイプラインと同じく1管内ずつ段階的に追加する。',
  },
  records: [
