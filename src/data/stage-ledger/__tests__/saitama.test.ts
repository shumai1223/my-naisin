 ...
 * 「農業に関する学科」18レコードの2区分・計120レコードを完全収録）: ①レコードの不変条件
 * ②quota/applicantsConfirmedは既存の倍率パイプライン（competition-rates/saitama.ts）と
 * 同一値であること ③公表資料の「普通科 計」「農業科 計」との完全突合（quota/
 * testTakersConfirmed/finalPassersの3つ・applicantsConfirmedはこの資料に印字が
 * 無いため対象外）。
 */
describe('埼玉県 段階台帳（T-Y11F §5順序#7・2県目・全日制普通科＋農業に関する学科は完全収録）', () => {
  const { records, officialSubtotals } = SAITAMA_STAGE_LEDGER;
  // 農業に関する学科18レコードは1〜2頁目（普通科102レコード）の後に3頁目分として
  // 追記されているため、配列順で分割する（department名だけでは「情報コース」等の
  // 普通科内特別コースと区別できない）。
  const normalRecords = records.slice(0, 102);
  const agricultureRecords = records.slice(102);

  it('取り込み件数は普通科102レコード＋農業に関する学科18レコード（計120レコード）', () => {
