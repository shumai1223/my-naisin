 ...
 * 107件中77件と極めて高頻度（推薦選抜の未消化枠繰り上げが原因と推測）のため他県のような
 * 個別例外列挙はせず、代わりにfinalPassers<=applicantsConfirmedの逆側の不変条件のみ検証する
 * ④公式小計（区部計・多摩部計・コース単位制以外計・島しょ計）との完全一致。
 */
describe('東京都 段階台帳（T-Y11F §5順序#7・7県目・「普通科（コース・単位制以外）」107レコード）', () => {
  const { records, officialSubtotals } = TOKYO_STAGE_LEDGER;

  it('取り込み件数は区部57校＋多摩部44校＋島しょ6校＝107レコード', () => {
  ...
  it('quota・applicantsConfirmedは既存の倍率パイプライン（competition-rates/tokyo.ts）と全件完全一致する（本資料に志願者数列が無いため両方とも再利用）', () => {
    ...
    const r8Records = tokyoCompetitionFile.records.filter((r) => r.fiscalYear === undefined && r.department === '普通科');

    let matched = 0;
    for (const stageRecord of records) {
      const counterpart = r8Records.find((r) => r.schoolName === stageRecord.schoolName);
  ...
  it('107レコード全数の機械集計が資料本文の4段階の公式小計（区部計・多摩部計・コース単位制以外計・島しょ計）とquota/testTakersConfirmed/finalPassersの3系列とも完全一致する', () => {
    ...
    const kubuRecords = records.slice(0, 57);
    const tamaRecords = records.slice(57, 101);
    const shimaRecords = records.slice(101);
