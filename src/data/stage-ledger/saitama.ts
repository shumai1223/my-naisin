 ...
 * レコードに収録漏れ・重複が一件も無いことを機械的に証明できた。
 *
 * 9〜10頁目を確認済み: 9頁目は「定時制」課程（普通科・専門学科（工業・商業）・総合学科）で、
 * **本ファイルのスコープ外**（別ファイル`teiji-competition-rates/saitama.ts`が既に別の
 * 一次資料で定時制課程をカバー済みのため、schema設計上も一本化しない）。10頁目は本資料
 * 自体の「訂正版」総括表で、【全日制課程】の内訳（普通科25,703/専門学科7,420/総合学科
 * 1,720/計34,843・入試募集人員計34,603/受検者数計35,872/入学許可候補者数計32,399）が
 * 本ファイルで独立に積み上げた3段階検算の結果と**完全に一致**することを、資料自体の公式
 * 総括表からも二重に裏付けられた（さらに【定時制課程】計1,976/1,920/1,043/1,027・
 * 全日制+定時制の総計36,819/36,523/36,915/33,426も併記されており、9頁目末尾の個別集計と
 * 完全に整合）。10頁目の注6により、受検者数・入学許可候補者数の一部括弧書き（例:
 * B35,872の脇に(37)）は帰国生徒特別選抜の内数であり、既に本ファイルの数値に含まれている
 * （二重計上ではない）。これで本資料（全10頁）の精査が完了し、埼玉県「全日制」の段階台帳
 * 241レコードは確定。
 */
