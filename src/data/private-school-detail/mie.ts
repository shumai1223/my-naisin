 ...
 * 三重県私学協会が公表する「令和9年度(2027年度)私立高等学校生徒募集定員」PDF(全日制/通信制の
 * 2分冊)から、全日制13校を1周回で完全収録。令和8年度分の一覧は協会サイトから既に削除されており
 * (令和8年度入試は既に終了済み)、現在公表されている最新版である令和9年度(2027年度入学)の値を
 * fiscalYearLabelにそのまま記録する。13校のcourses合計を積み上げた総計が原資料の合計欄
 * 「3,760(13校)」と完全一致することを検算済み(佐賀/富山/栃木/静岡/福岡/長野/岐阜に続く完全収録
 * パターン)。通信制8校(大橋学園/徳風/一志学園/英心(本校+桔梗が丘校)/代々木/神村学園高等部
 * 伊賀分校/みえ大台おおぞら高等学校)は今回は対象外とし未着手のまま残す。
 */
