 ...
 * **東京文華の再訪で解決(2026-07-31)**: 育伸社PDF段階で「推薦120(含スポーツコース)・一般120
 * (含スポーツコース)が完全同一の説明付きで反復」のため2つの独立プールか1つの反復記述か確証が
 * 持てず見送っていたが、公式サイトの生徒募集要項PDF(bunkagakuen.ac.jp)を直接確認したところ、
 * [推薦入試]表では「募集人数120名」がA推薦(単願)・B推薦(併願)の2列を横断する結合セルとして
 * 明記され、[一般入試]表でも同様に「募集人数120名」が第一志望・併願優遇・一般の3列を横断する
 * 結合セルとして明記されていた(1年次はコース分けなし、2年次からコース分岐)。これは推薦ブロック
 * 内部の共有(120)と一般ブロック内部の共有(120)がそれぞれ独立しているという4頁目の基本ルールに
...
export const PRIVATE_SCHOOL_DETAIL_TOKYO: PrivateSchoolDetailFile = {
  ...
  schools: [
    ...
    },
    {
      ...
      fiscalYearLabel: '2026年度',
      courses: [
        { courseName: '推薦(A推薦・B推薦共通枠、1年次はコース分けなし)', capacity: 120 },
        { courseName: '一般(第一志望・併願優遇・一般共通枠、1年次はコース分けなし)', capacity: 120 },
      ],
      totalCapacity: 240,
      source: {
  ...
  skipped: [
    ...
    },
    {
      ...
      schoolName: 'ドルトン東京学園高等部',
      reason: '完全中高一貫校で中学受験時のみ入学を受け入れており高校からの外部募集を実施していないため(みんなの高校情報「高校受験時の偏差値はございません」の明記による確認・公式サイトの入試関連リンクも「2月一般入試」「帰国生入試・転編入試」のみで通常の高校入試は無し)',
    },
  ],
};
