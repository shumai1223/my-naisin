...
/**
 * Λ-4（多年度アーカイブ・大阪府）DoD検証: 令和8・令和7年度の「全体合計」行の数値を
 * 一次資料(xlsx・Node標準zlibで自前パース)の固定値で確認する。
 */
describe('大阪府 多年度アーカイブ（Λ-4・令和8/令和7の2年度分・grand-total-only）', () => {
  it('2年度分（令和8年度・令和7年度）を収録している', () => {
    expect(OSAKA_COMPETITION_RATE_HISTORY.years).toHaveLength(2);
