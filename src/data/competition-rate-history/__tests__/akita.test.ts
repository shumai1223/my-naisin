 ...
 */
describe('秋田県 多年度アーカイブ（Λ-4・令和8〜令和2の7年度分・grand-total-only）', () => {
  it('7年度分（令和8年度〜令和2年度）を収録している', () => {
    expect(AKITA_COMPETITION_RATE_HISTORY.years).toHaveLength(7);
  ...
  it('令和8年度の県合計はY-6既存確定値と一致する(募集定員6,268・志願5,237・倍率0.84)', () => {
    const r8 = AKITA_COMPETITION_RATE_HISTORY.years[0];
    expect(r8.grandTotal.quota).toBe(6268);
    expect(r8.grandTotal.applicants).toBe(5237);
    expect(r8.grandTotal.rate).toBeCloseTo(0.84, 2);
  });

  it('令和7年度の県合計は一次資料と一致する(募集定員6,495・志願5,587・倍率0.86)', () => {
    const r7 = AKITA_COMPETITION_RATE_HISTORY.years[1];
  ...
  it('令和6年度の県合計は一次資料と一致する(募集定員6,604・志願5,753・倍率0.87)', () => {
    const r6 = AKITA_COMPETITION_RATE_HISTORY.years[2];
  ...
  it('令和5年度の県合計は一次資料と一致する(募集定員6,752・志願5,912・倍率0.88)', () => {
    const r5 = AKITA_COMPETITION_RATE_HISTORY.years[3];
  ...
  it('令和4年度の県合計は一次資料と一致する(前期選抜後の募集人数5,597・志願4,959・倍率0.89)', () => {
    const r4 = AKITA_COMPETITION_RATE_HISTORY.years[4];
  ...
  it('令和3年度の県合計は一次資料と一致する(前期選抜後の募集人数5,675・志願4,933・倍率0.87)', () => {
    const r3 = AKITA_COMPETITION_RATE_HISTORY.years[5];
  ...
  it('令和2年度の県合計はリセモム確定記事と一致する(募集人数5,692・志願5,196・倍率0.91)', () => {
    const r2 = AKITA_COMPETITION_RATE_HISTORY.years[6];
    expect(r2.grandTotal.quota).toBe(5692);
    expect(r2.grandTotal.applicants).toBe(5196);
    expect(r2.grandTotal.rate).toBeCloseTo(0.91, 2);
  });
});
