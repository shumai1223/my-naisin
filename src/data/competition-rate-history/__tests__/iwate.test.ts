...
describe('岩手県 多年度アーカイブ（Λ-4・令和7/令和6/令和5/令和4/令和3/令和2の6年度分・grand-total-only）', () => {
  it('6年度分（令和7年度・令和6年度・令和5年度・令和4年度・令和3年度・令和2年度）を収録している', () => {
    expect(IWATE_COMPETITION_RATE_HISTORY.years).toHaveLength(7);
  ...
  it('令和8年度はY-6既存確定値と一致する(合計（59校113学科）・募集8,215・志願6,574・倍率0.80)', () => {
    const r8 = IWATE_COMPETITION_RATE_HISTORY.years[0];
    expect(r8.grandTotal.quota).toBe(8215);
    expect(r8.grandTotal.applicants).toBe(6574);
    expect(r8.grandTotal.rate).toBeCloseTo(0.8, 2);
  });

  it('令和7年度の合計は一次資料と一致する(実質定員8,382・調整後志願6,684・倍率0.80)', () => {
    const r7 = IWATE_COMPETITION_RATE_HISTORY.years[1];
  ...
  it('令和6年度の合計は一次資料と一致する(実質定員7,862・調整後志願6,281・倍率0.80)', () => {
    const r6 = IWATE_COMPETITION_RATE_HISTORY.years[2];
  ...
  it('令和5年度の合計はリセモム記事と一致する(実質定員7,881・確定志願6,424・倍率0.82)', () => {
    const r5 = IWATE_COMPETITION_RATE_HISTORY.years[3];
  ...
  it('令和4年度の合計はリセモム記事と一致する(実質定員8,049・確定志願6,836・倍率0.85)', () => {
    const r4 = IWATE_COMPETITION_RATE_HISTORY.years[4];
  ...
  it('令和3年度の合計はリセモム記事と一致する(実質定員8,068・志願6,590・倍率0.82)', () => {
    const r3 = IWATE_COMPETITION_RATE_HISTORY.years[5];
  ...
  it('令和2年度の合計はリセモム記事と一致する(実質定員8,115・志願7,088・倍率0.87)', () => {
    const r2 = IWATE_COMPETITION_RATE_HISTORY.years[6];
