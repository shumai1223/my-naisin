...
describe('京都府 多年度アーカイブ（Λ-4・令和8〜令和3の6年度分・grand-total-only）', () => {
  ...
  it('令和8年度は一次資料と一致する(中期選抜募集人員6,048・志願5,160・倍率0.85)', () => {
    const r8 = KYOTO_COMPETITION_RATE_HISTORY.years[0];
    expect(r8.grandTotal.quota).toBe(6048);
    expect(r8.grandTotal.applicants).toBe(5160);
  ...
  it('令和7年度は一次資料と一致する(中期選抜募集人員6,006・志願5,635・倍率0.94)', () => {
    const r7 = KYOTO_COMPETITION_RATE_HISTORY.years[1];
  ...
  it('令和6年度は一次資料と一致する(中期選抜募集人員6,108・志願6,027・倍率0.99)', () => {
    const r6 = KYOTO_COMPETITION_RATE_HISTORY.years[2];
  ...
  it('令和5年度は一次資料と一致する(中期選抜募集人員6,096・志願5,935・倍率0.97)', () => {
    const r5 = KYOTO_COMPETITION_RATE_HISTORY.years[3];
  ...
  it('令和4年度は一次資料と一致する(中期選抜募集人員6,424・志願6,414・倍率1.00)', () => {
    const r4 = KYOTO_COMPETITION_RATE_HISTORY.years[4];
    expect(r4.grandTotal.quota).toBe(6424);
    expect(r4.grandTotal.applicants).toBe(6414);
    expect(r4.grandTotal.rate).toBeCloseTo(1.0, 2);
  });

  it('令和3年度は一次資料と一致する(中期選抜募集人員6,535・志願6,314・倍率0.97)', () => {
    const r3 = KYOTO_COMPETITION_RATE_HISTORY.years[5];
