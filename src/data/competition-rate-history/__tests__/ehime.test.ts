 ...
 */
describe('愛媛県 多年度アーカイブ（Λ-4・令和7/令和6/令和5/令和4/令和3/令和2の6年度分・grand-total-only）', () => {
  it('6年度分（令和7年度・令和6年度・令和5年度・令和4年度・令和3年度・令和2年度）を収録している', () => {
    expect(EHIME_COMPETITION_RATE_HISTORY.years).toHaveLength(7);
  ...
  it('令和8年度はY-6既存確定値と一致する(全日制計・募集8,370・志願7,468・倍率0.89)', () => {
    const r8 = EHIME_COMPETITION_RATE_HISTORY.years[0];
    expect(r8.grandTotal.quota).toBe(8370);
    expect(r8.grandTotal.applicants).toBe(7468);
    expect(r8.grandTotal.rate).toBeCloseTo(0.89, 2);
  });

  it('令和7年度の合計は一次資料と一致する(定員8,590・志願7,898・倍率0.92)', () => {
    const r7 = EHIME_COMPETITION_RATE_HISTORY.years[1];
  ...
  it('令和6年度の合計は一次資料と一致する(定員8,765・志願7,619・倍率0.87)', () => {
    const r6 = EHIME_COMPETITION_RATE_HISTORY.years[2];
  ...
  it('令和5年度の合計は一次資料と一致する(定員8,965・志願7,941・倍率0.89)', () => {
    const r5 = EHIME_COMPETITION_RATE_HISTORY.years[3];
  ...
  it('令和4年度の合計は一次資料と一致する(定員9,025・志願7,980・倍率0.88)', () => {
    const r4 = EHIME_COMPETITION_RATE_HISTORY.years[4];
  ...
  it('令和3年度の合計はリセモム記事と一致する(定員9,145・志願7,554・倍率0.83)', () => {
    const r3 = EHIME_COMPETITION_RATE_HISTORY.years[5];
  ...
  it('令和2年度の合計はリセモム記事と一致する(定員9,185・志願8,030・倍率0.87)', () => {
    const r2 = EHIME_COMPETITION_RATE_HISTORY.years[6];
    expect(r2.grandTotal.quota).toBe(9185);
    expect(r2.grandTotal.applicants).toBe(8030);
