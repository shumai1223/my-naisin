...
describe('熊本県 多年度アーカイブ（Λ-4・令和7/令和6/令和5/令和4/令和3/令和2の6年度分・grand-total-only）', () => {
  it('6年度分（令和7年度・令和6年度・令和5年度・令和4年度・令和3年度・令和2年度）を収録している', () => {
    expect(KUMAMOTO_COMPETITION_RATE_HISTORY.years).toHaveLength(7);
  ...
  it('令和8年度はY-6既存確定値と一致する(全日制（後期・一般選抜）計・募集8,322・志願7,295・倍率0.88)', () => {
    const r8 = KUMAMOTO_COMPETITION_RATE_HISTORY.years[0];
    expect(r8.grandTotal.quota).toBe(8322);
    expect(r8.grandTotal.applicants).toBe(7295);
    expect(r8.grandTotal.rate).toBeCloseTo(0.88, 2);
  });

  it('令和7年度の全日制課程・後期(一般)選抜の合計は一次資料の「計」行と一致する(募集8,258・出願7,585・倍率0.92)', () => {
    const r7 = KUMAMOTO_COMPETITION_RATE_HISTORY.years[1];
  ...
  it('令和6年度の全日制課程・後期(一般)選抜の合計は一次資料の「計」行と一致する(募集8,250・出願7,760・倍率0.94)', () => {
    const r6 = KUMAMOTO_COMPETITION_RATE_HISTORY.years[2];
  ...
  it('令和5年度の全日制課程・後期(一般)選抜の合計は一次資料の「計」行と一致する(募集8,362・出願7,985・倍率0.95)', () => {
    const r5 = KUMAMOTO_COMPETITION_RATE_HISTORY.years[3];
  ...
  it('令和4年度の全日制課程・後期(一般)選抜の合計は一次資料の「計」行と一致する(募集8,569・出願7,691・倍率0.90)', () => {
    const r4 = KUMAMOTO_COMPETITION_RATE_HISTORY.years[4];
  ...
  it('令和3年度の全日制課程・後期(一般)選抜の合計は一次資料の「計」行と一致する(募集8,785・出願7,411・倍率0.84)', () => {
    const r3 = KUMAMOTO_COMPETITION_RATE_HISTORY.years[5];
  ...
  it('令和2年度の全日制課程・後期(一般)選抜の合計はリセモム記事と一致する(募集8,743・出願8,041・倍率0.92)', () => {
    const r2 = KUMAMOTO_COMPETITION_RATE_HISTORY.years[6];
