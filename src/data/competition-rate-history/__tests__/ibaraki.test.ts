...
/**
 * Λ-4（多年度アーカイブ・茨城県）DoD検証: 令和7・令和6年度の「全日制計」行の数値を
 * 一次資料（茨城県教育委員会の志願者数等PDF・志願先変更後）の固定値で確認する。
 */
describe('茨城県 多年度アーカイブ（Λ-4・令和7/令和6の2年度分・grand-total-only）', () => {
  it('2年度分（令和7年度・令和6年度）を収録している', () => {
    expect(IBARAKI_COMPETITION_RATE_HISTORY.years).toHaveLength(2);
