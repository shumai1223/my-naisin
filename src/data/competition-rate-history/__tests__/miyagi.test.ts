 ...
 */
describe('宮城県 多年度アーカイブ（Λ-4・令和7/令和6/令和5/令和4/令和3/令和2の6年度分・grand-total-only）', () => {
  it('6年度分（令和7年度・令和6年度・令和5年度・令和4年度・令和3年度・令和2年度）を収録している', () => {
    expect(MIYAGI_COMPETITION_RATE_HISTORY.years).toHaveLength(7);
  ...
  it('令和8年度はY-6既存確定値と一致する(全日制合計（第一次募集）・募集13,400・志願12,516・倍率0.93)', () => {
    const r8 = MIYAGI_COMPETITION_RATE_HISTORY.years[0];
    expect(r8.grandTotal.quota).toBe(13400);
    expect(r8.grandTotal.applicants).toBe(12516);
    expect(r8.grandTotal.rate).toBeCloseTo(0.93, 2);
  });

  it('令和7年度の全日制課程・第一次募集は一次資料の「総括」表と一致する(募集13,440・出願13,349・倍率0.99)', () => {
    const r7 = MIYAGI_COMPETITION_RATE_HISTORY.years[1];
  ...
  it('令和6年度の全日制課程・第一次募集は一次資料の前年度列と一致する(募集13,640・出願13,609・倍率1.00)', () => {
    const r6 = MIYAGI_COMPETITION_RATE_HISTORY.years[2];
  ...
  it('令和5年度の全日制課程・第一次募集は一次資料(令和6年度「結果について」発表の前年度列)と一致する(募集13,760・出願14,095・倍率1.02)', () => {
    const r5 = MIYAGI_COMPETITION_RATE_HISTORY.years[3];
  ...
  it('令和4年度の全日制課程・第一次募集はリセモム確定記事と一致する(募集13,880・出願14,005・倍率1.01)', () => {
    const r4 = MIYAGI_COMPETITION_RATE_HISTORY.years[4];
  ...
  it('令和3年度の全日制課程・第一次募集はリセモム確定記事と一致する(募集14,200・出願13,685・倍率0.96)', () => {
    const r3 = MIYAGI_COMPETITION_RATE_HISTORY.years[5];
    expect(r3.grandTotal.quota).toBe(14200);
    expect(r3.grandTotal.applicants).toBe(13685);
    expect(r3.grandTotal.rate).toBeCloseTo(0.96, 2);
  });

  it('令和2年度の全日制課程・第一次募集はリセモム確定記事と一致する(募集14,280・出願14,648・倍率1.03)', () => {
    const r2 = MIYAGI_COMPETITION_RATE_HISTORY.years[6];
    expect(r2.grandTotal.quota).toBe(14280);
    expect(r2.grandTotal.applicants).toBe(14648);
