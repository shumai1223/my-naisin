...
/**
 * Λ-4（多年度アーカイブ・群馬県）DoD検証: 令和7・令和6年度の「公立全日制・フレックス
 * スクール合計」行の数値を一次資料（群馬県教育委員会の志願先変更後志願状況PDF）の固定値で確認する。
 */
describe('群馬県 多年度アーカイブ（Λ-4・令和7/令和6の2年度分・grand-total-only）', () => {
  it('2年度分（令和7年度・令和6年度）を収録している', () => {
    expect(GUNMA_COMPETITION_RATE_HISTORY.years).toHaveLength(2);
  ...
  it('令和7年度の合計は一次資料と一致する(募集定員11,435・志願11,525・倍率1.01)', () => {
    const r7 = GUNMA_COMPETITION_RATE_HISTORY.years[0];
  ...
  it('令和6年度の合計は一次資料と一致する(募集定員11,757・志願11,744・倍率1.00)', () => {
    const r6 = GUNMA_COMPETITION_RATE_HISTORY.years[1];
