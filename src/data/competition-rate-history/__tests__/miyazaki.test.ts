...
describe('宮崎県 多年度アーカイブ（Λ-4・令和8〜令和3の6年度分・grand-total-only）', () => {
  ...
  it('令和8年度は一次資料と一致する(募集人員3,873・志願者数2,767・倍率0.71)', () => {
    const r8 = MIYAZAKI_COMPETITION_RATE_HISTORY.years[0];
    expect(r8.grandTotal.quota).toBe(3873);
    expect(r8.grandTotal.applicants).toBe(2767);
    expect(r8.grandTotal.rate).toBeCloseTo(0.71, 2);
  });
  ...
  it('令和7年度は一次資料と一致する(募集人員3,862・志願者数3,159・倍率0.82)', () => {
    const r7 = MIYAZAKI_COMPETITION_RATE_HISTORY.years[1];
  ...
  it('令和6年度はリセモム記事と一致する(募集人員3,948・志願者数3,190・倍率0.81)', () => {
    const r6 = MIYAZAKI_COMPETITION_RATE_HISTORY.years[2];
  ...
  it('令和5年度はリセモム記事と一致する(募集人員4,106・志願者数3,514・倍率0.86)', () => {
    const r5 = MIYAZAKI_COMPETITION_RATE_HISTORY.years[3];
  ...
  it('令和4年度は教委公式PDFと一致する(募集人員4,301・志願者数3,521・倍率0.82)', () => {
    const r4 = MIYAZAKI_COMPETITION_RATE_HISTORY.years[4];
    expect(r4.grandTotal.quota).toBe(4301);
    expect(r4.grandTotal.applicants).toBe(3521);
    expect(r4.grandTotal.rate).toBeCloseTo(0.82, 2);
  });

  it('令和3年度はリセモム記事と一致する(募集人員5,273・志願者数4,332・倍率0.82)', () => {
    const r3 = MIYAZAKI_COMPETITION_RATE_HISTORY.years[5];
