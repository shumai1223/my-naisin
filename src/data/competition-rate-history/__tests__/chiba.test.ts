...
describe('千葉県 多年度アーカイブ（Λ-4・令和8/令和7/令和6/令和5の4年度分・grand-total-only）', () => {
  ...
  it('令和8年度の全日制合計は教委公式PDF(確定数)と一致する(募集28,880・確定志願32,008・倍率1.11)', () => {
    const r8 = CHIBA_COMPETITION_RATE_HISTORY.years[0];
    expect(r8.grandTotal.quota).toBe(28880);
    expect(r8.grandTotal.applicants).toBe(32008);
  ...
  it('令和7年度の全日制合計は報道記事と一致する(募集29,720・志願33,854・倍率1.14)', () => {
    const r7 = CHIBA_COMPETITION_RATE_HISTORY.years[1];
  ...
  it('令和6年度の全日制合計は独立した2記事(令和7年度記事の前年度比較・令和6年度発表時記事)双方と一致する(募集30,680・志願34,478・倍率1.12)', () => {
    const r6 = CHIBA_COMPETITION_RATE_HISTORY.years[2];
  ...
  it('令和5年度の全日制合計は教委公式ページ2件(一次発表・結果まとめページ)双方と一致する(募集30,960・志願34,793・倍率1.12)', () => {
    const r5 = CHIBA_COMPETITION_RATE_HISTORY.years[3];
