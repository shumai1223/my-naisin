 ...
 */
describe('静岡県 多年度アーカイブ（Λ-4・令和8〜令和2の7年度分・grand-total-only）', () => {
  it('7年度分（令和8年度〜令和2年度）を収録している', () => {
    expect(SHIZUOKA_COMPETITION_RATE_HISTORY.years).toHaveLength(7);
  ...
  it('令和4年度の公立合計はリセモム記事と一致する(募集定員18,874・志願者数19,289・倍率1.02)', () => {
    const r4 = SHIZUOKA_COMPETITION_RATE_HISTORY.years[4];
    expect(r4.grandTotal.quota).toBe(18874);
    expect(r4.grandTotal.applicants).toBe(19289);
    expect(r4.grandTotal.rate).toBeCloseTo(1.02, 2);
  });

  it('令和3年度の公立合計はリセモム記事と一致する(募集定員19,015・志願者数18,868・倍率0.99)', () => {
    const r3 = SHIZUOKA_COMPETITION_RATE_HISTORY.years[5];
    expect(r3.grandTotal.quota).toBe(19015);
    expect(r3.grandTotal.applicants).toBe(18868);
    expect(r3.grandTotal.rate).toBeCloseTo(0.99, 2);
  });

  it('令和2年度の公立合計はリセモム記事と一致する(募集定員20,140・志願者数20,571・倍率1.02)', () => {
    const r2 = SHIZUOKA_COMPETITION_RATE_HISTORY.years[6];
    expect(r2.grandTotal.quota).toBe(20140);
    expect(r2.grandTotal.applicants).toBe(20571);
