...
/**
 * ⚠️2026-08-24調査(令和2年度・見送り): リセモム記事(2020-02-20付「【高校受験2020】香川県公立高入試
 * の出願状況・倍率（2/18時点）」https://resemom.jp/article/2020/02/20/54876.html)で
 * 「全日制課程合計の募集人員5,177人に対し、出願者数が6,098人、倍率が1.18倍」を発見したが、これは
 * **2/18時点の志願変更前**の速報値であり、既存R3-R8がいずれも採用する「志願変更締切後」の確定値
 * とは基準が揃わない。確定値を報じた後続記事(2/27発表を引用したbest-kobetsu.co.jp記事等で
 * 高松の倍率が1.17倍に変動したことは断片的に確認できたが、県全体の確定合計を記載した記事は
 * 2/28前後の記事ID帯（同日はコロナ休校報道が優勢だったためリセモムの通常記事が少ない）を含め
 * 発見できなかった。志願変更前後で倍率が動く実例（高松1.19→1.17）がある以上、速報値を確定値の
 * 代わりに使うのは適切でないためR2追加を見送る**(捏造ゼロ原則)。
 */
