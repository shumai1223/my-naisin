 ...
 *
 * **令和4年度（2022年度）は恒久的に見送り（2026-08-05調査済み・再挑戦不要）**: 一次資料
 * （attachment/471930.pdf「選抜（Ⅱ）等の志願状況」）を確認したところ、当時は推薦選抜等の
 * 「選抜（Ⅰ）」を経た後に一般入試「選抜（Ⅱ）」を実施する旧2段階制度で、「選抜（Ⅱ）定員」は
 * 入学定員から選抜（Ⅰ）合格者数を大きく差し引いた値（例: 普通科は入学定員8,840→選抜Ⅱ定員
 * 6,972）になっていた。令和5年度以降は推薦選抜等が廃止され入学定員のほぼ全てを単一の「一次
 * 選抜」で争う制度に改編されており、R5-R8の`quota`は入学定員とほぼ同義（連携型中高一貫の
 * 既確定者のみを除いた値）。したがって令和4年度の「選抜（Ⅱ）定員」とR5-R8の「一次選抜定員」は
 * 母数の性質が根本的に異なり単純に並べられない（akita/gunma/nagasakiと同型の制度改編による
 * スコープ不整合）。akitaのように「印字済み倍率と整合する列を採用」する救済も、令和4年度の
 * 母数自体が他年度と異質なため成立しない。gunma/nagasakiと同じ理由で恒久見送りとする。
 */
