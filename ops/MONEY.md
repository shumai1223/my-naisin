...
> **2026-08-24訂正（`ops/CORRECTIONS.md` C-7）**: 上記の trusted=39 は `referer LIKE 'https://my-naishin.com%'`
> （アンダースコア無し）で数えており、root_only（パス無し）を人間に数える既知の罠が混入している。
> `ops/COEFFICIENTS.md`§2が2026-08-15時点で確定していた採用値は**幅0.198%（下限）〜0.537%（上限）・
> 下流は必ず0.198%を使う**。**以降のC5計算の基準係数は0.198%とする**（本節の0.455%は参考値として
> 打ち消し線のまま残す）。
...
|---|---|---|---|---|
| **`moshimo-manecafe`** | ¥11,500 | **完全に未配置**。`lead-config.ts` の `PROGRAM_PRESET`（note/ctaText）と `affiliate-economics` に載っているだけで、`PLACEMENT_LEAD_OVERRIDES`・`PLACEMENT_SECONDARY_OFFER`・全コンポーネントのどこからも参照されない | `PLACEMENT_LEAD_OVERRIDES`・`PLACEMENT_SECONDARY_OFFER`・全`.tsx`から参照0（`grep -rn moshimo-manecafe src` 全5件はカタログ定義のみ＝`affiliate-economics.ts:93`／`affiliates.ts:44,424,425`（型コメント・カタログオブジェクト）／`lead-config.ts:108`。2026-08-15訂正: 引用符無しgrepだと5件・元の「全2件」は引用符付きgrepの過小報告だった＝`ops/CORRECTIONS.md` C-4） | **未配置＝在庫死蔵**（結論不変） |
...
| 遷移率（GSCクリック→アフィリクリック） | ~~0.455%~~ **0.198%**（2026-08-24訂正・`ops/CORRECTIONS.md` C-7） | ~~実測：D1 trusted 39 ÷ GSC 8,565（同一28日窓）~~ `ops/COEFFICIENTS.md`§2の採用値（下限。root_only混入を除いた値） |
...
**2026-08-24訂正（C-7）: 月間アフィリクリック = 10,667 × 0.198% = 21.1 クリック/月**

| シナリオ | EV/click | ~~月次EV(0.455%基準)~~ | **月次EV(0.198%基準・2026-08-24訂正)** | 現在との差(訂正後) |
|---|---|---|---|---|
| **現在のミックス**（trusted 39件の構成） | ¥148.2 | ~~¥7,199/月~~ | **¥3,127/月** | — |
| **最高CPA案件（`moshimo-minhoken` ¥17,000）に全寄せ** | ¥153.0 | ~~¥7,431/月~~ | **¥3,228/月** | **+¥101/月** |
| 全国対応の最良案件（`sora-juku-text` ¥10,000）に全寄せ | ¥240.0 | ~~¥11,664/月~~ | **¥5,064/月** | **+¥1,937/月** |
| **最高EV案件（`morijuku-text`/`campus-text` ¥15,000）に全寄せ**（理論上限・地盤13県のみ有効なので実現不能） | ¥360.0 | ~~¥17,486/月~~ | **¥7,596/月** | **+¥4,469/月** |

> 訂正の計算根拠: `node -e`で`10667*0.00198`→21.1（四捨五入）・各EV/click×21.1を四捨五入。
> 「最高CPA案件への全寄せは小さいレバー、全国最良案件への切替の方が大きい」という**結論の順序自体は
> 0.455%基準・0.198%基準のどちらでも変わらない**（絶対額のみ約43%に縮小）。

> **この計算が示す最重要の含意：**
> **「最高CPA案件に寄せる」は ~~+¥232/月~~ **+¥101/月（2026-08-24訂正・C-7）** にしかならない。** サイト自身の `convRateLow` を使うと、CPA¥17,000のFP相談（発生率1.5%）は CPA¥10,000の塾体験（発生率4.0%）に EV で負ける。
> **単価ミックスの損失は最大でも ~~¥4,465/月~~ **¥1,937/月（2026-08-24訂正・C-7・全国最良案件シナリオ基準）**（現実的な全国案件基準）であり、ボトルネックは単価ではなく遷移率の側にある。**
> **2026-08-15訂正**: この基準値**1.17%は誤り（正しくは0.43%）**（§3-a末尾の※1参照）。`ops/CORRECTIONS.md` C-1に
> 既に再計算済みだったと判明（2026-08-10作成・本書側への反映が漏れていた）ため、ここに正しい値を転記する。
> ~~参考：遷移率をトップページ実測の1.17%までサイト平均で引き上げると、ミックスを一切変えなくても
> 48.6→124.8クリック/月・¥18,495/月（+¥11,296/月）。単価ミックス最適化の2.5倍のレバー。~~
> **訂正後（2026-08-15時点・旧基準0.455%比）**: 10,667×0.43%＝45.4クリック/月（当時の基準48.6より減少）。
> EV＝45.4×¥148.2＝¥6,728/月＝当時比−¥471/月。
> ⚠️ **2026-08-24再訂正の必要性（C-7・未着手のまま残す）**: この「−¥471/月」という結論は
> **サイト平均を0.455%とした場合の比較でしか成立しない**。C-7が確定した正しいサイト平均は
> **0.198%（下限）〜0.537%（上限）の幅**であり、トップページの0.43%はこの幅の**内側**に位置する
> （0.198% < 0.43% < 0.537%）。下限0.198%と比べれば0.43%は依然として**上回る**——つまり
> 「引き上げが成立しない」という結論は下限基準では**逆転しうる**。この比較（トップページ実測値 vs
> サイト平均のレンジ）をどちらの基準点で行うべきかは値の意味が異なり単純な置換では済まないため、
> **次回セッションでの慎重な再検討に持ち越す**（今回はここまでの単純な基準係数置換のみ実施）。
> ⚠️比較に使う「サイト平均遷移率0.455%」は`ops/COEFFICIENTS.md`§2が不採用とした値（root_onlyを人間に数える
> 定義の副作用）。採用値は0.198%（下限）〜0.537%（上限）で、この節の再計算は「1.17%がどれだけ過大だったか」の
> 対照としてのみ読み、絶対額として引用しないこと（`ops/CORRECTIONS.md`の同じ注記を踏襲）。
...
| **推定影響** | ~~トップページ実測遷移率1.17%が出れば21.7クリック/月×EV¥240=¥5,208/月→出血約¥5,200/月~~ **2026-08-15訂正（`ops/CORRECTIONS.md` C-1転記）**: ブログ月間クリック≈1,853×**0.43%**＝**7.9クリック/月**×¥240＝**約¥1,893/月**（元の36%。仮置き：遷移率は自社実測の最高値を上限とした。同一`ParentLeadCTA`コンポーネントを使っているため到達可能と判断） |
...
| **推定影響** | ~~月間クリック≈1,287。トップページ実測遷移率1.17%なら15.1クリック/月×EV¥240(そら塾／関東関西は¥360)=¥3,616〜¥5,424/月~~ **2026-08-15訂正（`ops/CORRECTIONS.md` C-1転記）**: naishin月間クリック≈1,287×**0.43%**＝**5.5クリック/月**→**¥1,315〜¥1,972/月**（元の36%）。現状¥0 |
...
| | |
|---|---|
| **症状** | live なのに`PLACEMENT_LEAD_OVERRIDES`・`PLACEMENT_SECONDARY_OFFER`・全`.tsx`コンポーネントのどこからも参照されない。どの面にも副にも載っていない |
| **証拠** | `grep -rn moshimo-manecafe src` → 5ヒット（`affiliate-economics.ts:93`／`affiliates.ts:44,424,425`＝型コメント・カタログ定義のみ／`lead-config.ts:108`＝文言プリセットのみ）。**2026-08-15訂正**: 元の「引用符付きgrepで2ヒット」は過小報告（`ops/CORRECTIONS.md` C-4）。実配置0件という結論は不変 |
