// T-Y14 nara: data.mjs(目視転記)から src/data/school-selection-methods/nara.ts を生成する。使い方: node ops/baselines/nara-r9/(令和8年度版の目視転記は ops/baselines/nara-transcription/)gen.mjs
import fs from 'fs';
...
for (const e of N2) {
  const ratioType = `一次選抜学力検査(3教科)${e.gaku}:調査書${e.chosho}`;
  const note = `【二次選抜一覧・${e.course}】一次選抜で合格者数が募集人員に満たなかった学科(コース)で実施。一次選抜の国語・数学・英語の3教科の学力検査の得点を用いる(満点${e.gaku}点・学科によって加重配点)。令和9年度は各高校での二次選抜の検査(面接等)を実施しない(令和8年度は面接あり)。調査書:学習成績の取扱いパターン${e.pattern}=${PAT[e.pattern]}、加重配点:${e.kajuu || 'なし'}、調査書成績の満点${e.chosho}点。第2(第3)志望の取扱いは資料の別表(22〜23頁)を参照(未収録)。`;
  out += `    {
      schoolName: ${q(e.name)},
      department: ${q(e.dept + sfx(e.course))},
...
const ts = `// 奈良県: 令和9年度奈良県立高等学校入学者選抜概要「一次選抜一覧【第1希望校・第2希望校として出願する場合】」「二次選抜一覧」。
//
// 一次ソース: 奈良県教育委員会「令和9年度奈良県立高等学校入学者選抜概要」
// (\`https://www.pref.nara.lg.jp/documents/24281/31_r9gaiyou_ichiji_ichiran_dai1.pdf\`・5頁・県ページ
// \`https://www.pref.nara.lg.jp/n167/p122015.html\`・令和9年度版は2026-09-20取得・pdftotext -bboxの座標で令和8年度版(目視転記)と突合し変更点のみ更新。調査書の取扱いパターン①〜④は
// 同概要「4 調査書の取扱い」(\`24_r9gaiyou_chousasho.pdf\`)で確認)。転記データと生成スクリプトは ops/baselines/nara-r9/(令和8年度版の目視転記は ops/baselines/nara-transcription/) に保存。
//
...
// 検査成績の満点=学力検査+独自問題+作文+面接+実技検査となることを全レコードで検算済み(テスト)。第2希望校は第1希望校の3教科学力検査の得点を使い、二次選抜は令和9年度は各高校での検査を実施せず一次選抜の3教科の得点と調査書で選考する(各別PDF: 32_r9gaiyou_ichiji_ichiran_dai2.pdf/33_r9gaiyou_niji_ichiran.pdf)。
...
export const NARA_SCHOOL_SELECTION_METHOD: PrefectureSchoolSelectionMethod = {
  ...
  status: 'structured',
  coverageNote:
    ${q(`一次選抜一覧【第1希望校・第2希望校として出願する場合】と二次選抜一覧(県立全${names.size - 3}校+参考の市立3校・${D.length}学科・3種の選抜区分)を完全収録。各校の学科別概要(25頁〜)・第2(第3)志望の別表は未収録。合格人数枠は令和8年10月頃に発表される正式な募集人員で変更されることがある`)},
  schools: [
${out.replace(/\n$/, '')}
  ],
  source: {
    url: 'https://www.pref.nara.lg.jp/n167/p122015.html',
    docTitle: '令和9年度奈良県立高等学校入学者選抜概要（一次選抜一覧・調査書の取扱い）（奈良県教育委員会）',
