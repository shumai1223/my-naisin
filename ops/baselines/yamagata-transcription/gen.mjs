...
for (const e of D) {
  ...
  const day = e.day === 'A' ? 'A日程(令和8年1月20日)' : 'B日程(令和8年2月3日)';
  const zk = `【前期(特色)選抜】検査日程:${day}。募集人員(定員の比率):${e.quota}。検査方法:${methods(e)}。${e.detail}県外志願者受入れ:${e.outF ? 'あり(○)' : 'なし'}。`;
...
const ts = `// 山形県: 令和8年度山形県公立高等学校入学者選抜における前期(特色)選抜及び後期(一般)選抜の概要。
//
// 一次ソース: 山形県教育委員会
...
${out.replace(/\n$/, '')}
  ...
  source: {
    url: 'https://www.pref.yamagata.jp/documents/42443/r8senbatsugaikyo.pdf',
    docTitle: '令和8年度山形県公立高等学校入学者選抜における前期（特色）選抜及び後期（一般）選抜の概要（山形県教育局高校教育課）',
    lastChecked: '${new Date().toISOString().slice(0, 10)}',
  },
  note: '前期(特色)選抜のinterviewRequiredは個人面接または集団面接がある学科(集団討論・口頭試問・発表のみの学科は含めない)。後期(一般)選抜のratioTypeは「調査書の評定:学力検査の成績」の比。検査日程のAは令和8年1月20日・Bは2月3日。山形市立商業高校の前期募集人員は総合ビジネス科25%程度・情報科10%程度・経済科10%程度(概要表の注※5)。',
};
`;
