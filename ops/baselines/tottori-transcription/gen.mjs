...
const base = (e) => `【令和8年度 募集生徒数及び入学者選抜検査内容等一覧表・${e.course}】募集生徒数${e.teiin}名`;
...
for (const e of Z) {
  ...
  if (e.tokuN != null) {
    push(e.school, e.dept, e.tokuName ? `特色入学者選抜(${e.tokuName})` : '特色入学者選抜', /面接/.test(e.tokuTest), '', `${base(e)}。特色入学者選抜: 募集人員${e.tokuN}人以内${e.tokuOut != null ? '(うち県外生徒' + e.tokuOut + '人程度)' : ''}、実施検査内容:${e.tokuTest}。出願要件・選抜方法・出願する際の評定の目安は資料本文にあり本DBには未収録。`);
  }
}
for (const e of T) {
...
const ts = `// 鳥取県: 令和8年度鳥取県立高等学校募集生徒数及び入学者選抜検査内容等一覧表(全日制・定時制)。
//
// 一次ソース: 鳥取県教育委員会「令和8年度鳥取県立高等学校募集生徒数及び入学者選抜検査内容等一覧表」
// (\`https://www.pref.tottori.lg.jp/secure/1393541/20250602_reiwa8nendo_kensanaiyoutou.pdf\`・全10頁・
// 県ページ \`https://www.pref.tottori.lg.jp/322543.htm\`・2026-09-19 pdftoppm 110dpiで目視転記)。転記データと生成スクリプトは ops/baselines/tottori-transcription/ に保存。
//
...
// 検算: 全日制の募集生徒数(3,728)・特色募集人員(945)・うち県外生徒(98)と、定時制の募集生徒数(220)・特色募集人員(18)が資料の小計行と一致。
...
export const TOTTORI_SCHOOL_SELECTION_METHOD: PrefectureSchoolSelectionMethod = {
  ...
  status: 'structured',
  coverageNote:
    ${q(`全日制22校(募集生徒数3,728名)・定時制4校(220名)の一般入学者選抜(比率)と特色入学者選抜(募集人員・検査内容)を完全収録(${names.size}校・${recs}レコード)。特色入学者選抜の出願要件と通信制課程は未収録`)},
  schools: [
${out.replace(/\n$/, '')}
  ],
  source: {
    url: 'https://www.pref.tottori.lg.jp/322543.htm',
    docTitle: '令和8年度鳥取県立高等学校募集生徒数及び入学者選抜検査内容等一覧表（鳥取県教育委員会）',
