// T-Y14 mie: data-b4.mjs(別表4の目視転記)から src/data/school-selection-methods/mie.ts を生成する。使い方: node ops/baselines/mie-transcription/gen.mjs
import fs from 'fs';
...
for (const r of B4) {
  ...
  const note =
    '【令和8年度 後期選抜の選抜資料及び最終段階における「特に重視する選抜資料等」(別表4)・' + r.course + '】' +
...
const names = new Set(B4.map((r) => r.school.replace(/\((木本|紀南|度会)校舎\)/, '')));
const zen = B4.filter((r) => r.course === '全日制').length;
const tei = B4.filter((r) => r.course === '定時制').length;
const ts = `// 三重県: 令和8年度三重県立高等学校入学者選抜における各高等学校別後期選抜の選抜資料及び選抜方法の最終段階における「特に重視する選抜資料等」一覧(別表4)。
//
...
// 未収録: 前期選抜(別表2・66頁)・後期選抜の配点比率と学校別の実施要項(別表1)・スポーツ特別枠(別表5・6)・海外帰国生徒等(別表7)・通信制課程。
...
export const MIE_SCHOOL_SELECTION_METHOD: PrefectureSchoolSelectionMethod = {
  ...
  status: 'structured',
  coverageNote:
    ${q('後期選抜の選抜資料と最終段階で特に重視する選抜資料(別表4)を全日制' + zen + 'レコード・定時制' + tei + 'レコードで完全収録(' + names.size + '校・' + B4.length + 'レコード)。前期選抜・配点比率・スポーツ特別枠・帰国等・通信制は未収録')},
  schools: [
${out.replace(/\n$/, '')}
  ],
  source: {
    url: 'https://www.pref.mie.lg.jp/KOKOKYO/HP/m0204200379.htm',
    docTitle: '令和8年度三重県立高等学校入学者選抜実施要項 別表4（三重県教育委員会）',
...
console.log('schools', names.size, 'records', B4.length, '全日制', zen, '定時制', tei);
