// T-Y14 chiba: 千葉県公式ページ「令和8年度千葉県公立高等学校「一般入学者選抜」の検査の内容等（全日制の課程）」
// (https://www.pref.chiba.lg.jp/kyouiku/shidou/nyuushi/koukou/r8/r8zennichi.html・2026-09-19 curl取得)の
// HTML表(学校番号1〜121+市立高校)を解析して src/data/school-selection-methods/chiba.ts を生成する。
// 使い方: node ops/baselines/chiba-transcription/gen.mjs  (r8zennichi.html は同ディレクトリに保存済み)
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const dir = path.dirname(fileURLToPath(import.meta.url));
const html = fs.readFileSync(path.join(dir, 'r8zennichi.html'), 'utf8');
...
const ts = `// 千葉県: 令和8年度千葉県公立高等学校「一般入学者選抜」の検査の内容等（全日制の課程）。
//
