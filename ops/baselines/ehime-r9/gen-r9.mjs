// T-Y14 ehime: data-r9.mjs(座標抽出)から src/data/school-selection-methods/ehime.ts を生成する。使い方: node ops/baselines/ehime-r9/gen-r9.mjs
import fs from 'fs';
...
const ts = `// 愛媛県: 令和9年度愛媛県県立高等学校入学者選抜 特色入学者選抜 各校の出願資格及び検査項目等(募集割合・募集人数・検査項目等の比重)。
//
...
export const EHIME_SCHOOL_SELECTION_METHOD: PrefectureSchoolSelectionMethod = {
  ...
  status: 'structured',
  coverageNote:
    ${q('特色入学者選抜の募集割合・募集人数・検査項目等の比重(文化・スポーツ活動を重視した選抜を含む)を全' + names.size + '校・' + recs + 'レコードで完全収録(資料67頁中の各校表)。出願資格と検査概要の本文、一般入学者選抜の配点は未収録。募集定員は令和8年10月頃に正式公表予定の見込み値')},
  schools: [
${out.replace(/\n$/, '')}
  ],
  source: {
