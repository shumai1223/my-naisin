...
// 全県共通の制度は『令和9年度沖縄県立高等学校入学者選抜実施要項(全日制・定時制課程)』(\`https://www.pref.okinawa.jp/_res/projects/default_project/_page_/001/041/122/09yoko_h.pdf\`・全14頁・2026-09-22取得・テキスト層あり)の第1〜2章から転記し、coverageNoteに記載した(学力検査5教科各60点・50分/特色選抜の学力検査成績は記述式除く各50点/特色選抜の募集人員は普通科20%以内・その他40%以内/調査書と学力検査等の比重は原則5:5)。
// 未収録: 各校の特色選抜の配点・検査内容(各校の募集要項・県ページに一括公表なし)・学力検査の学校別の教科別配点。
...
export const OKINAWA_SCHOOL_SELECTION_METHOD: PrefectureSchoolSelectionMethod = {
  ...
  status: 'structured',
  coverageNote:
    ${q(`全日制・定時制の全${names.size}校${rows.length}行(定員合計${total.toLocaleString('en-US')}名)の特別枠・比重一覧を完全収録。全県共通の制度(令和9年度実施要項): 学力検査は国語・社会・数学・理科・英語の5教科で各教科50分・各60点(英語は聞き取り検査あり)・学校の特色に応じて教科ごとの配点を変えられる・特色選抜の募集人員は普通科20%以内/普通科以外40%以内・特色選抜の学力検査成績は一般選抜の各60点のうち思考力等を問う記述式問題を除いた各教科50点で取り扱う。各校の特色選抜の配点・検査内容は未収録`)},
  schools: [
${out.replace(/\n$/, '')}
  ],
  source: {
