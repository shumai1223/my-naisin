...
for (const e of ROWS) {
  ...
  const note = `【令和8年度 一般入学者選抜 募集定員及び検査内容・${e.course}】定員${e.teiin}名。${subjText}、面接${e.itv}点(${e.itvType}面接)${tekText}、調査書${e.cho}点、合計${e.kei}点${memo}。推薦・連携型入学者選抜は別レコードとして収録。`;
...
for (const e of JIKO) {
  pushSuisen(e.school, e.course === '定時制' ? e.dept + '(定時制)' : e.dept, '推薦入学者選抜(自己推薦方式)', e, '【令和8年度 推薦・連携型入学者選抜 募集人員割合及び検査内容・' + e.course + '】定員' + e.teiin + '名、募集人員は定員の' + e.pct + '%(' + e.nin + '名)', e.memo);
}
for (const e of SPORTS) {
  const head = '【令和8年度 推薦・連携型入学者選抜 募集人員割合及び検査内容・全日制】指定部活動「' + e.act + '」の募集人員は' + e.nin + '人(自己推薦方式の募集人員には含まれない)';
...
for (const e of RENKEI) pushSuisen(e.school, e.dept, '連携型入学者選抜', e, '【令和8年度 推薦・連携型入学者選抜 募集人員割合及び検査内容・全日制】定員' + e.teiin + '名', e.memo);
...
export const MIYAZAKI_SCHOOL_SELECTION_METHOD: PrefectureSchoolSelectionMethod = {
  ...
  status: 'structured',
  coverageNote:
    ${q(`一般入学者選抜の全日制(定員${zen}名)・定時制(${tei}名)の学科別配点(学力検査・面接・調査書・適性検査等)を完全収録(${names.size}校・${recCount}レコード)。推薦入学者選抜(自己推薦方式114・スポーツ推薦方式46)と連携型(福島)も収録。通信制課程と出願要件等の本文は未収録`)},
  schools: [
${out.replace(/\n$/, '')}
  ],
  source: {
