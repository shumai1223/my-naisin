...
// 行 = { n:校名, cs:'あり'|'なし', cho:調査書点, men:個人面接点 | null, merged:個人面接・集団討論の結合セル点 | null, rt:'作文'|'小論文', rp:点, sc:自校作成問題か, m2:第二次募集の個人面接点 | null }
import fs from 'fs';
export function build(rows) {
  ...
  for (const r of rows) {
    ...
    ins += `    {\n      schoolName: '${r.n}',\n      department: '普通科',\n      selectionCategory: '推薦に基づく選抜',\n${r.merged != null ? '' : '      interviewRequired: true,\n'}      ratioType: '推薦枠割合20%',\n      note: '文化・スポーツ等特別推薦の実施${r.cs}。調査書の観点別学習状況の評価は活用せず評定のみ活用。満点は調査書${r.cho}点+${menDesc}+${r.rt}${r.rp}点(合計${tot}点)${kekka}。${r.merged != null ? '実技検査・学校設定検査の実施なし' : '集団討論・実技検査・学校設定検査の実施なし'}',\n    },\n`;
    ...
    ins += `    {\n      schoolName: '${r.n}',\n      department: '普通科',\n      selectionCategory: '第二次募集',\n      interviewRequired: ${r.m2 != null ? 'true' : 'false'},\n      ratioType: '学力検査6:調査書4(600点:400点)${r.m2 != null ? '+個人面接' + r.m2 + '点' : ''}',\n      note: '学力検査は国数英の3教科。学力検査を実施する教科の評定は1倍・実施しない教科の評定は2倍に換算。${r.m2 != null ? '満点は学力検査600点+調査書400点+個人面接' + r.m2 + '点(この学校のみ第二次募集で個人面接あり)。小論文・実技検査の実施なし' : '面接・小論文・実技検査の実施なし'}',\n    },\n`;
  }
  return ins;
}
