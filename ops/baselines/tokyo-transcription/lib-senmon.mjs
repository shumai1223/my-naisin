...
//       f1?/f1n?:第一次のratio/note, f2?/f2n?:第二次のratio/note, f1j?:第一次の実技検査点, f2j?:第二次の実技検査点, m2?:第二次の面接点, m2t?:'集団', sc?:自校作成, keisha?:傾斜配点の説明, cat?: 追加note }
...
export function buildSenmon(group, rows) {
  ...
  for (const r of rows) {
    ...
    ins += rec(r.n, dept, '第一次募集', false, (r.f1 || '学力検査7:調査書3(700点:300点)+ESAT-J20点') + (r.f1j ? `+実技検査${r.f1j}点` : ''), r.f1n || `学力検査は国数英社理の5教科${sc}${k}。学力検査を実施する教科の評定は1倍・実施しない教科(社理)の評定は2倍に換算。${r.f1j ? '実技検査' + r.f1j + '点を課す。' : ''}面接・小論文の実施なし`.replace('。。', '。'));
    const m2 = r.m2 != null;
    const m2t = r.m2t || '個人';
    ins += rec(r.n, dept, '第二次募集', m2, (r.f2 || '学力検査6:調査書4(600点:400点)') + (m2 ? `+${m2t}面接${r.m2}点` : '') + (r.f2j ? `+実技検査${r.f2j}点` : ''), (r.f2n || `学力検査は国数英の3教科${k}。学力検査を実施する教科の評定は1倍・実施しない教科の評定は2倍に換算。`) + (m2 ? `満点は学力検査600点+調査書400点+${m2t}面接${r.m2}点${r.f2j ? '+実技検査' + r.f2j + '点' : ''}。` : '') + (r.f2j ? '' : m2 ? '小論文・実技検査の実施なし' : '面接・小論文・実技検査の実施なし'));
  }
  return ins;
}
export { append };
