...
for(const r of rows.slice(+process.argv[4]||0,(+process.argv[4]||0)+(+process.argv[5]||45)))console.log(Math.round(r.y),r.w.sort((a,b)=>a.x-b.x).map(w=>Math.round(w.x)+':'+w.t).join(' ').slice(0,260));
