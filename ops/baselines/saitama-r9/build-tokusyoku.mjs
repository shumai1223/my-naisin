...
const pages = fs.readFileSync(path.join(dir, '2.bbox.html'), 'utf8').split('<page ').slice(1);
const out = [];
pages.forEach((pg, pi) => {
  const pageRows = [];
  ...
  const mains = ws.filter((w) => w.x >= 2050 && w.x < 2330 && /^\d+%$/.test(w.t)).sort((a, b) => a.y - b.y);
  for (const m of mains) {
    ...
    const nameToks = win.filter((w) => w.x < 1100 && Math.abs(w.y - m.y) <= 60).sort((a, b) => a.y - b.y);
    ...
    const deptToks = ws.filter((w) => w.x >= 1250 && w.x < 2000 && w.y > hdrY && nearestMain(w.y) === m && !/^(全|定)$/.test(w.t) && !/%$/.test(w.t)).sort((a, b) => a.y - b.y || a.x - b.x);
    ...
    const r = {
      ...
      pct1: m.t, pct2: near(2330, 2700, 30).find((w) => /%$/.test(w.t))?.t || null,
      keisha: win.filter((w) => w.x >= 2700 && w.x < 3450 && Math.abs(w.y - m.y) <= 130).sort((a, b) => a.y - b.y || a.x - b.x).map((w) => w.t).join(''),
      gakkoSentaku: near(3450, 3700, 80).some((w) => w.t === '○'),
      ratio1: ratio(3750, 4180), ch1: (() => { const t = near(4180, 4400, 30).find((w) => /^\d+$/.test(w.t)); return t ? +t.t : null; })(),
      ratio2: ratio(4400, 4900), ch2: (() => { const t = near(4900, 5150, 30).find((w) => /^\d+$/.test(w.t)); return t ? +t.t : null; })(),
      itvType: near(5200, 5450, 30).map((w) => w.t).join(''),
      itvOwn: near(5500, 5700, 60).map((w) => w.t).join(''),
      tokusyokuKensa: near(5700, 6150, 60).map((w) => w.t).join(''),
      s1: { gaku: num(6200, 6450), cho: num(6450, 6750), itv: num(6750, 7000), toku: num(7000, 7250), total: num(7300, 7520) },
      s2: { gaku: num(7600, 7850), cho: num(7850, 8150), itv: num(8150, 8400), toku: num(8400, 8600), total: num(8650, 8900) },
      second: win.filter((w) => w.x >= 9100 && w.x < 9900 && Math.abs(w.y - m.y) <= 130).sort((a, b) => a.y - b.y || a.x - b.x).map((w) => w.t).join(''),
  ...
  const hdrBottom = Math.max(0, ...ws.filter((w) => /^(1年|2年|３年|3年|合計)$/.test(w.t) && w.x > 3000 && w.x < 9000 && w.y < 1500).map((w) => w.y)) + 20;
  const names = [];
...
fs.writeFileSync(path.join(dir, process.argv[3] || 'rows-tokusyoku.json'), JSON.stringify(out, null, 1));
