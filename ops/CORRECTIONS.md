...
- **✅2026-08-15追記**: 本書のC-1〜C-6全項目を`ops/MONEY.md`/`ops/DISTANCE.md`/`ops/BAR.md`/`ops/DEADWIRE.md`本体へ
  転記済み（取り消し線+訂正値の形で該当箇所に直接反映・元の記述も保持して差分が追える形にした）。C-1/C-4のみ
  `node scripts/d1q.mjs`で、C-5は`ops/raw/revenue-distance.json`を、C-6a/C-6cはそれぞれファイル走査/`mcp__ga4__ga4_run_report`で
  独立に再検証したうえで転記し、全て本書の値と一致した。**以後は各成果物ファイル本体を読めば訂正済みの値が
  直接わかる**（本書はまだ経緯の記録として有用だが、数値を読むためだけに本書と成果物を往復する必要はなくなった）。
...
| **C-7** | `MONEY.md:19,286,291,302,367,376` — サイト全体の遷移率として**「0.455%」を単独引用**している全箇所（`ops/BAR.md:371,379,387,398,401,410`にも伝播） | 「サイト全体の実遷移率＝0.455%」を以降の全計算の基準係数として使用 | `ops/COEFFICIENTS.md`§2が既に0.455%を**不採用値**と確定し、採用値を**幅0.198%（下限）〜0.537%（上限）・下流は必ず0.198%を使う**と定めていた（2026-08-15時点で確定済みだったが、`MONEY.md`/`BAR.md`本体は`:309`の注記1本を除き未反映のまま）。**単純な基準係数の置換（0.455%→0.198%）に限れば**: `MONEY.md:291`「10,667×0.455%=48.6クリック/月」→**10,667×0.198%=21.1クリック/月**（`PHASE0_FINDINGS.md`出血1が同型の計算を提示済み・21.2件/月で概ね一致・独立算出で確認）。`MONEY.md:19`の結論文自体も「0.455%」から「0.198%（下限）〜0.537%（上限）」に置き換える | `ops/COEFFICIENTS.md:43,45,48`（既存の確定計算をそのまま参照。今回新たなD1再クエリはしていない） | **§5-c・出血1/3/5/6/7の個別ドル換算（`MONEY.md:302,367,376`等）は、C-1（トップページ固有0.43%）と本項目（サイト全体0.198%〜0.537%）のどちらを使うべきかが行ごとに異なり、既存のCORRECTIONS表（C-1〜C-6g）ほどの精度で1件ずつ再計算するには追加の時間が要る。この行は「基準係数の置換ルール」だけを確定し、`MONEY.md`/`BAR.md`本体の個別ドル数値の書き換えは次回セッションの持ち越しとする（2026-08-24・loopが起票）** |

> ✅ **2026-08-25 追記（loopが解決）— 「C-1と本項目のどちらを使うべきか」自体に答えが出た**:
> 両者は**比較不能な別基準**だった。C-1の0.43%は`referer='https://my-naishin.com/'`の**完全一致**
> （root_only）で数えており、本項目C-7が確定した是正済み`TRUSTED_CLAUSE`（`LIKE '.../_%'`＝
> **root_only除外**）を適用していない。しかも`ops/PHASE0_FINDINGS.md`2章項目2の追記が示す通り、
> **トップページ自身のURLはパス無しのため、トップページ発クリックのrefererは是正済み基準下では
> 構造的に常にroot_only**——つまり「トップページ固有のtrusted遷移率」は是正済み基準では
> **原理的に測定不能（常に0）**になる。「C-1とC-7のどちらが正しいか」という設問自体が、
> 集計基準が違う2つの数字を比較しようとしていたために不成立だったと判明した。
> **実務上の帰結**: `MONEY.md`§5-c・出血1/3/5/6/7の各行で「トップページの遷移率をX%とみなす」
> という試算は、C-1（0.43%）・C-7（0.198%〜0.537%）のどちらを採用しても是正済み基準との
> 整合性が取れない。個別ドル数値の書き換えは優先度を下げてよい（唯一実害があった出血7は、
> この論点とは独立に**2026-08-24 `8ebad13`(S11-1)で既に対応済み**と確認・`MONEY.md`側に反映済み）。
