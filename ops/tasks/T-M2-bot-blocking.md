...
### M2-2 第3波の署名を防御に組み込む — 🟡2026-08-30 一部完了(loop)
...
- [ ] `persistClick()`を判定の後ろに移す、または判定結果を同じ行に書く
- [ ] ⚠️判定を厳しくしすぎない。実ユーザーを1人でも弾いたら¥117の損失。迷ったら通す
      （記録して後で分類できるようにする方を優先）
...
- [ ] 本番反映は👤ゲート → push待ち（このコミット自体は`src/app`+`src/lib`のためLOOP_CONTRACT方針どおり即push対象）
