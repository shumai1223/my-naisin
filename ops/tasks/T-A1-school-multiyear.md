...
## kill_criteria判定（2026-09-14・展開08-12から33日後・期日09-09から5日遅延して実施）

GSC `page`次元(`pageContains: '/school/'`)を28日窓（2026-08-13〜09-10・展開直後からの窓）で取得し
impression加重平均順位を機械算出した: **総impression 61,396・総click 1,821・加重平均順位 8.87位**
（rowLimit1000で全学校ページ中1000件・より小規模な学校ページは含まれない可能性あり）。

**判定: kill_criteriaは発動しない（順位は9.76位→8.87位へ改善）。** 展開後に「9.76位のまま」という
停滞条件は観測されなかったため、「順位の律速は権威側と確定・T-C1へ資源を寄せる」という撤退措置は
不要と結論する。ただし注意点として、baseline(9.76位)は08-02〜08-08の週(402ページ・impression
2,259件)という小規模サンプルであり、今回(1000ページ・61,396件)はサイト全体の自然なトラフィック
成長も混在するため、8.87位への改善が多年度化機能そのものの効果か全体成長の効果かは**因果分離
できていない**。それでも「改善しない」という具体的な停止条件には該当しないため、本タスクの
DoD(2026-08-12完了済み)はそのまま維持し追加の巻き戻し作業は行わない。この判定でT-A1の
未決事項は解消・以後の再判定は不要。
