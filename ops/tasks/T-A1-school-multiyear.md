...
| **45/47県**が3年度以上を保有（2026-08-25時点・`ops/BAR.md`で先行訂正済み。本行は2026-08-30に追随修正）。最多は東京の**5年度**（944レコード） | 同上。hokkaido/nara/niigataは2026-08-24に掛-1が3年度化を完走し45/47へ前進。残るoita/sagaの2県のみ一次ソースPDF自体が未アーカイブと確定済みで**本タスクの対象外**（`ops/PHASE0_FINDINGS.md`参照） |
...
5. 残り44県へ展開。**45/47県が対象**（2026-08-24にhokkaido/nara/niigataが3年度化されたため実装は無変更のまま自動的に対象拡大した。oita/sagaのみ対象外・スキップ台帳に理由を書く）
