...
- [x] ⚠️ **令和7年度・令和6年度のスナップショットにはハッシュが無い**（対話セッションが
      2026-09-03に確認・R8のみ47/47）。ここで過去年度分にもハッシュを入れられる
      → **2026-09-03時点の前提は既に古かった**: 実際にはT-N1-2(2025-r7)・T-Y11-winter Task C
      (2024-r6)が既に大部分を収集済みだった（本項目が「未対応」と書かれた後、別タスクが並行で
      前進していたが本ファイルへの反映が漏れていた・[[fable5-loop-protocol]]の「grep -cカウント
      だけでは個別精査を保証しない」教訓と同型）。2026-09-15に残っていた実際の欠落
      （2024-r6のishikawa・saga未収集）を埋めて **R7=45/47収集+2件unavailable(ishikawa/nagasaki)
      で47/47確定・R6=46/47収集+1件unavailable(nagasaki)で47/47確定** とした。ishikawaは
      Wayback経由でPDF実体を取得しpdftoppm+ビジョン解析で内容確認・pdfHash付きで収録。
      sagaは一次ソースが現行404・archive.orgにも一度もクロールされておらず物理的に取得不能
      だったため、第三者塾サイト2件の裏取りのみでcaveat付きpdfHashなしのまま収録（R7の
      同種caveatエントリと同じ扱い）。残る未収集はhokkaido(R6のみ・archive.org不安定で
      2026-09-07に見送り)のpdfHashのみ。tsc実exit0・jestフルスイート545suites7805tests green
