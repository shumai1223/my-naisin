...
- [x] 各県の**公表時期**（令和8年度実績の日付）を記録する。1月中旬〜2月下旬に散らばるはず →
      **✅2026-09-01（T-Y11B段階1）: 44/47県まで完了**（chiba/kanagawa/osaka/saitama/tokyoの
      既存5県＋新規39県を、既存ヘッダコメント／`sources[]`のdocTitleの転記で構造化
      （`src/lib/competition-rate-publication-notes.ts`・新規調査ではなく既存記述の構造化のみ）。
      残る3県（ehime/fukuoka/hokkaido）は一次資料に発表日の記述自体が見当たらず、推測せず
      `UNRESOLVED_PUBLICATION_DATE_PREFECTURES`に正直に記録した（次にこの3県のヘッダコメントを
      触る回で記述が見つかれば移す）。実測1月中旬〜3月下旬に分布（最速akita/gunma2月12日〜
      最遅fukushima3月19日）
...
- [x] 県ごとに `quota` が何を指すか（募集人員／入学許可予定者数）を台帳に明記する →
      **✅2026-09-01（T-Y11B段階1）: 47/47県完了**（既存3県＋新規44県を`src/lib/
      competition-rate-quota-definitions.ts`に構造化。既存ヘッダコメントの転記であり新規調査ではない）。
      確認された規則: **quotaは「その県が公表する倍率の分母と一致する列」を採用する設計**で全県一貫
      （埼玉=入学許可予定者数・千葉/東京=募集人員・多くの県は「内定者数等を控除した実質枠」パターン）。
      47/47固定の回帰テストを追加済み
- [x] **2026-09-01**: 定時制・通信制のスコープ外扱いを県ごとに明記する →
      **47県全ファイルに既に明記されていたと確認**（`src/data/competition-rates/__tests__/
      ...
      忘れるとテストが落ちるようにした）
- [x] 学科区分の名称は年度で変動する。**名寄せを機械的にやらない。年度ごとに素直に持つ**
      → ✅2026-09-07監査完了: `buildSchoolHistoryForPrefecture()`(`src/lib/school-page-data.ts`)は
      各年度レコードの`department`を生の文字列のまま`SchoolHistoryEntry`へ積むだけで、年度をまたぐ
      名前の突合・エイリアス処理は一切していない（学校名の突合には`nameAliases`があるが、学科名には
      同種の仕組みが存在しない＝設計として意図的に名寄せしていないと確認）。`groupSchoolHistoryByDepartment()`
      も`entry.department`の**完全一致**でグルーピングするだけなので、学科名が年度によって変わった
      場合は自動的に別グループとして扱われ、合算も上書きもされない。既存の`normalizeDepartmentText()`
      （`parse-table-pdf.ts`）は読点→中黒・半角→全角括弧という**プロジェクト内表記統一のみ**を行う
      もので、意味のある学科名の違いを消す正規化ではないため方針と矛盾しない。方針どおりに実装
      されていることを確認しただけでなく、`school-page-data.test.ts`に恒久回帰テスト（学科名が
      年度で変わるケースで2件のまま合算されないことを検証）を新規追加した。tsc実exit0・
      jest全体green（既存491suitesに新規テストケース追加）
...
- [x] `tsc` 実exit 0 / jest green → **2026-09-01時点で366 suites / 6410 tests green・tsc実exit0**。
      **これでA-5 DoD4項目全て達成。Task A（Y-11倍率速報パイプライン）は主要な骨格が完成し、
      ✅2026-09-01（T-Y11B段階1）: A-1(公表日44/47県)・A-3(quota定義47/47県)とも完了した。
      Task Aの「広げれば広げるほど良い」仕上げ項目は事実上完走。残るのはT-Y11B段階2（取り込み
      パーサの実装・着手前ゲートで判明したハイブリッド設計＝pdftotext試行→欠落自動検出→
      ビジョン解析フォールバック）のみ**
...
### B-1 oita・sagaのR6を追加する（2/47・小さい残作業） — ✅2026-09-14最終確定（取得不能で完了）

- [x] 大分県・佐賀県の令和6年度（2024年度）一次資料を探し、他45県と同じ形式
      （`fiscalYear: '令和6年度（2024年度）'`付きレコード）で追加する →
      **調査したが追加できなかった**。oita: 2026-09-01時点はInternet Archive一時停止で
      CDX検索自体が機能せず「見つからない」と誤認していたが、2026-09-14の復旧後の再検索で
      `r06ichijisaisyuu.html`（2024-04-22クロール）を発見・PDFリンク(`2207551.pdf`)を特定した。
      saga: 該当ページ(`kiji003101127`)のHTML自体は2026-09-14に取得成功（2スナップショットとも
      同一PDF`3_101127_308541_up_a4spvixc.pdf`を指す）。
- [x] 見つからない場合は他県と同じく「取得不能」と理由付きで明記する（推測で埋めない） →
      **✅2026-09-14最終確定**: 両県ともHTML案内ページはWaybackに存在するが、**リンク先PDF本体が
      両県ともWayback CDXに1件も存在しない**（HTMLはクロールされたがPDFはクロールされなかった）。
      現行サイトも両県ともPDF・ページとも404。oita.ts/saga.tsのヘッダコメントを最終確定版に更新
      済み。この経路は今後再試行不要（Internet Archive復旧待ちではなく、PDF自体が未アーカイブと
      確定したため）。残る手段は👤による県教委への直接問い合わせのみで、loop単独では完結しない。
- [x] 追加後は既存の自己検算パターン（グランドトータルとの一致）を適用する →
      R6データが確定的に取得不能なため対象外（該当なしで完了）
...
- [x] 取得不能県が理由付きで台帳に載っている → **✅2026-09-14最終確定**（B-1参照・oita/saga
      ともPDF本体がWayback CDX上に存在しないことを確認し、ヘッダコメントに理由を記録済み）
...
- [~] N1-2 と同じ形で `src/data/snapshots/2024-r6/` `2023-r5/` を作る →
      **2024-r6のみ着手・44/47件完了**(41件済み + 2026-09-01にkagawa/oita/tokyoを追加)。
      tokyoは実施要綱PDF本体(release20230928_01/1.pdf)がWayback未クロール(CDX0件)のため
      直接取得できなかったが、令和6年6月発行の学力検査結果調査報告書(現行サイトで取得成功・
      5教科×100点満点を確認)＋第三者塾サイトの2024年度明示の「7:3」記述で間接確認しcaveat付きで収録。
      **残り3県(ishikawa/nagasaki/saga)**はR7調査時点で一次ソース自体が404かつWayback Machineにも
      アーカイブ無しと確認済み(2026-09-01に再確認・状況変わらず)。取得困難な見込みが高く、
      次回もこの3県から着手するより先に他タスクへの配分を優先してよい
      **✅2026-09-14 nagasakiを取得不能として最終確定**: Internet Archive復旧後に再調査したところ
      令和6年度実施要領本体PDF自体は発見・全38頁確認できた（過去の「404かつアーカイブ無し」という
      判断は誤りだった＝復旧前の一時的な状況を恒久的な結論と誤認していた）。しかし内容を読んだ結果、
      長崎県は調査書・学力検査・面接の比重を**各高等学校が個別に定める学校裁量制**であり、
      県内で単一の計算式が存在しないと確定した（`exam-system.json`のスキーマ自体が前提とする
      「1県=1つの計算式」に構造的に合わない）。ishikawa/sagaは引き続き保留（低優先度のまま）。
      ...
      **2023-r5は未着手** → **✅2026-09-22着手**: `src/data/snapshots/2023-r5/`を新設し
      ...
      kagawa・hyogo・fukuoka・yamaguchi・shiga・naganoの26県を収録(26/47)。mieは単純URL置換でなくWebSearchで「令和５年度三重県
      ...
      ことを本文の参照箇所から特定し135点満点をR6引用文と一致で確定。kyotoはR6と同じファイル名
      パターン(senbatuyoukou.pdf)のR5専用ページ(?p=536)を発見し195点満点をR6引用文と一致で確定。
      fukushimaはR5専用ページ(r5koukounyushi.html)から実施要綱本体を取得し、一般選抜(後期選抜)の
      195点満点をR6引用文と一致で確定(同一文書内に前期選抜135点満点の別配点もあり要注意)。
      tokushimaは試行したがR5専用の議案書PDFが見つからず保留。okayamaはR5実施要項本体から
      ...
      45点満点を確定。okinawaはR5ページ404・Wayback不調で保留。shizuokaはproject IDが年度ごとに
      変わるURL構造をWebSearchで辿りR5専用発表資料ページを発見・45点満点をR6引用文と一致で確定。
      gifuは第三者サイト引用の直リンクが404だったがWayback CDX(availability APIは空だったがCDXには
      3件実在)で完全版PDFを回収・180点満点をR6引用文と一致で確定。toyama/iwate/nagano/shimaneは
      再試行したがCDXにも未アーカイブと確認、tottori/tokushima/akitaはR5専用PDFのURL特定に至らず
      保留、aomoriはWaybackに1件のみ発見したが1MBで内容切り詰めのため未確定。niigata/saitama/miyagi
      ...
      caveatあり135点満点を確定。naganoは当初保留だったがWayback CDXでディレクトリ全体検索し
      正しいファイル名(アンダースコア付き)を発見・45点満点をR6引用文と一致で確定(caveatなし)。
      ...
      **✅2026-09-23追記: saitama/kanagawa/oita/miyazaki/toyama/tokushima/niigata/
      kagoshima/fukuiを追加し36/47まで到達**(詳細は`src/data/snapshots/2023-r5/README.md`)。
      **⚠️練度が上がったため残り11県の状況を再点検した結果、2023-r5は事実上ここが実務上の
      天井と判断**: hokkaido/tokyo(R6自体がpdfHash null=恒久的取得不能)・aichi(令和5年度
      入試改革疑いのため意図的保留)・nagasaki(学校ごとの配点裁量制でexam-system.jsonの
      スキーマ自体が表現不能=構造的に対象外)・saga(同型の学校別裁量制を検出・👤レビュー待ち・
      loop-question-note記録済み)・akita/tottori/aomori/iwate/miyagi/okinawa(Wayback上に
      対象PDFが1件も存在しないか、存在しても1,048,576バイトに切り詰められた版が1件のみで
      shimane型の別ダイジェスト解決策が使えないことを複数セッションで独立に確認済み)。
      新しい技法(f2208型/id_型/ディレクトリ全体検索/公表年元号フォルダ)を出尽くした状態で、
      次に伸びるとすれば①IA側の技術障害が解消し新規クロールが発生する②sagaの👤判断が来る
      ③第三者サイトやミラーからの新発見、のいずれか。以後のセッションは無駄な再試行より
      T-Y13/T-Y14等の10月公表待ち県の確認や他タスクを優先してよい。
