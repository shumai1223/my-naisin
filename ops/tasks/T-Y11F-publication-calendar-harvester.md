...
本イテレーションでは行っていない。
| 7 | **★段階台帳**⚠️進行中(2026-09-09・千葉「県立全日制」区分は完全収録・公表計と完全一致) | 定員・推薦・合格者数・二次募集を**学校×学科でR8とR7に積む**。12県の実測51単位から47県で**約350単位**。公表値の転記のみでY-0内 | **90〜170h** |
...
転記できない様式なら無理せず別県・別区分を選ぶこと。次は千葉の残り区分（県立定時制・
8〜9頁目）への継続、または別県（横展開）への適用のいずれかを次回セッションで判断する。
