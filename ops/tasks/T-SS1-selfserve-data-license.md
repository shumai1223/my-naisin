...
- [x] `api_hits` / `api_keys` / `api_usage` を実際に数え、**どこまで到達して離脱しているか**を出す
      → **2026-09-04実測に成功**。生の`wrangler whoami`は認証トークン無しでブロックされたが、
      既存の`scripts/d1q.mjs`（読み取り専用SQL・書き込み系は正規表現で拒否・
      `NODE_TLS_REJECT_UNAUTHORIZED=0`を内蔵）が本番`my-naishin-leads`へ普通に疎通した
      （認証は`wrangler`側に別途保存されていたと見られる）。**次回以降はD1調査に
      `node scripts/d1q.mjs "SELECT ..."`を使えばよい・生wranglerコマンドを直接叩く必要は無い**、
      という運用上の発見自体も記録しておく。実測結果:
      - `api_keys`（発行された全キー・ティア問わず）: **0行**。Free/Pro/Business/Scale問わず、
        ただの1件もキーが発行されたことがない（≒「顧客0」は文字通り0）。
      - `api_usage`（月次クォータ消費の記録）: **0行**（api_keysが0なので当然の帰結）。
      - `api_hits`（匿名アクセス含む全APIアクセスログ・`endpoint`列で識別）: 累計**19,073件**
        （ログ開始2026-08-30〜直近2026-09-03の5日弱のみ・古いログは無い）。うち**19,007件
        （99.7%）は`stats-percentile`**（結果ページ内蔵の「あなたの順位は上位◯%」表示が
        裏で叩く同一オリジンAPI＝外部からの「発見」ではなく自サイト内利用）。
        Business専用の`schools-detail`（＝今回SS1-2/3/4で手を入れた`/api/schools/{pref}`）は
        **累計わずか7件**。その内訳をUser-Agent別に見ると、Googlebotモバイル×2・bingbot×1・
        非ブラウザUA（PowerShell/curl。日時が近接しており人手または自動スクリプトでの動作確認と
        推測されるが、送信元は特定できないため断定はしない）×4、で**外部の見込み客と断定できる
        アクセスは1件も含まれていない**。
      - **結論**: 「どこまで到達して離脱しているか」ではなく、**そもそも到達自体が発生していない**
        （検索エンジンのクロール以外の外部トラフィックが実質ゼロ）。これはSS1-4で実測したGSCの
        薄い露出（90日で73表示・3クリック）と整合する一枚の絵になる：**露出が薄い→到達が無い→
        当然キーも発行されない**、という単純な因果で「買えない」以前に「見つかっていない」段階。
        SS1-2/3/4のコード・コンテンツ整備は前提条件として必要だったが、**次のボトルネックは
        技術ではなく認知（露出）**であり、これはloopの技術的権限の外（対外的な告知・被リンク・
        コンテンツ流通は👤ゲートまたはT-X系の別タスク）。
...
**D1実測は2026-09-04に完了済み**（`node scripts/d1q.mjs`で本番D1へ直接疎通できた・SS1-1参照）。
結論: **技術的な「買えない」はSS1-2/3/4でほぼ解消したが、そもそも見込み客が`/developers`まで
到達していない**（api_hitsの`schools-detail`累計7件は全てbot/非ブラウザUAで見込み客ゼロ）。
次のボトルネックは露出（対外的な認知）であり、loopの技術的権限の外側にある。
