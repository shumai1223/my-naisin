# 学校ページ index 切り戻し手順（TH-3/TH-7対応・2026-08-17新設）
...
同じ手順は脅威9（TH-7・学校ページ低品質判定）の「特定waveだけをnoindexへ戻す」回復策
としても流用できる（下記「部分ロールバック」節を参照）。

## いつ使うか

- **TH-3（手動対策）**: GSC「セキュリティと手動対策」ページ（**API取得不可・👤が管理画面を
  開いて見るしかない**）で、学校ページ群（`/pref/[code]/school/[schoolCode]`、
  2026-08-17時点で47/47県3,089枚）に対する「scaled content abuse」等の手動対策を検知した場合。
  → 下記「全面ロールバック」を使う。
- **TH-7（低品質判定）**: 特定のwave（解禁時期のまとまり）だけクリック消失等の異常兆候が
  出た場合、その wave の県コードだけを対象に切り戻す。→ 下記「部分ロールバック」を使う。
