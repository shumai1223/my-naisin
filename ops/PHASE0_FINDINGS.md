...
> **その後、同日中にniigataへ令和6年度分を追加し3年度化を完了（Wayback Machine経由でR6資料を
> 取得・全91レコードが学科区分別「計」行8本＋全日制合計行と完全一致・`content(niigata)`
> commit `dad1dcf`）。43/47に前進し、残るは`hokkaido`/`nara`/`oita`/`saga`の4県。**
> `ops/BAR.md`側も同時に43/47へ再訂正済み。項目1〜6・9〜11は今回は確認していない（次回の候補）。
| # | 断定 | 場所 | なぜ裏取り不足か（実測） |
|---|---|---|---|
...
> **同日中に追記**: niigataへ3年度目（令和6年度）を追加し完了（`dad1dcf`）。**43/47に前進・
> 残るはhokkaido/nara/oita/sagaの4県**。`ops/BAR.md`も43/47へ再訂正済み。
