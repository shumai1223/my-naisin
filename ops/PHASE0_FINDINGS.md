...
> 当時残っていたのは`hokkaido`/`oita`/`saga`の3県（`oita`/`saga`はWayback CDX広域検索まで尽くし断念確定・`hokkaido`は
> R6版PDF（全14頁）の存置をWebSearchで確認済み・HTTP200で直接取得可能だが大規模転記は当時は着手せず次回セッションへ持ち越し）。**
> `ops/BAR.md`側も同時に44/47へ再訂正済み。項目1〜6・9〜11は今回は確認していない（次回の候補）。
>
> ✅ **2026-08-24 19:17（`94292dd`）に解消済み（2026-08-25にloopが実ファイル再集計で確認）**: 上記で
> 「持ち越し」とした`hokkaido`のR6転記は、実は**同日中に掛-1委任エージェント（第20〜27弾）が
> 全14管内分を完走していた**（本注記がその完走前に書かれたため未反映のまま残っていた）。
> `src/data/competition-rates/hokkaido.ts`を実ファイル再集計すると`令和6/7/8年度`の3年度が
> 確認でき、**45/47に前進**。残るは`oita`・`saga`の2県のみで、いずれも一次ソースPDF自体が
> 未アーカイブと確定済み（構造的に断念・追加調査の見込みなし）。`ops/BAR.md`側も45/47へ再訂正済み。
| # | 断定 | 場所 | なぜ裏取り不足か（実測） |
|---|---|---|---|
...
> 🟡 **2026-08-24部分確認（loopが実施・完全解決ではない）**: `ops/DEADWIRE.md:332`の「28日で人間だった
> 可能性がある20件」の表を実際に精読したところ、**表内の`参照元`列に`google.com`等の外部ドメインは
> 1件も現れず、全て内部パス（`/tokyo/total-score`・`/hensachi`・`/mendan`等）か`(null)`のみ**だった。
> これは本項目が指す「M1の外部3件（本節冒頭の`google.com`3件）」とは**別の集計**である可能性が高い
> （DEADWIRE表自体のSQLが`referer <> 'https://my-naishin.com/'`で明示的にroot_onlyを除外している
> ため、M1が算出した「内部パス付き17+外部3=20」という数字とDEADWIRE表の「20件」は、同じ20件を
> 指しているのではなく、別々の抽出条件でたまたま同じ合計になった可能性がある）。**この点は2026-08-10
> 時点のD1スナップショットに依存する比較のため、現在のD1データから当時のM1の「外部3件」を独立に
> 再現することはできない**（生きたテーブルのため過去のクエリ結果を遡って再現不可）。
> **ただし本項目の核心的な結論（「20は上限でも下限でもない」＝GA4のaffiliate_click実測とD1の20件
> リストが完全には一致しない）自体は、GA4とD1の計測系統がそもそも独立している（[[ga4-undercounts-conversions]]
> の教訓と同型）ことから見て、依然として妥当と判断する。**「20件」という数字を絶対的な人間クリック数の
> 確定値として引用しないこと、という結論は維持してよい。
...
> 🟡 **2026-08-24関連知見（loopが実施・完全解決ではない）**: 本項目と直接は解決しないが、関連する
> 重要な事実を今回のセッション中に発見した（項目2参照）: **トップページ（`/`）自身のURLはパス
> 無しのため、トップページ上のCTAをクリックした際のrefererは、ブラウザの仕様どおり正しく動作
> していても必然的にroot_only（`https://my-naishin.com/`）になる**。これはroot_onlyの一部が
> 「refererを送らない/改変する特殊なブラウザ・拡張機能」由来ではなく、**単に「トップページ発の
> 正規クリック」という別の（bot以外の）発生源でも同じ見た目になりうる**ことを意味し、本項目が
> 指摘する「同じ機序が他行で起きていない証明が無い」という懸念を側面から補強する。**F2（referer=null
> は実ブラウザでない）についても、Safari/Brave等のプライバシー指向ブラウザやreferrer遮断系の
> 拡張機能がsame-origin遷移でもrefererを送らないケースが実在するため、「null=bot確定」という
> 強い主張は過度に単純化されている可能性が高い**。ただし、これを覆すには実際のブラウザ種別・
> バージョン分布のUA解析等の追加調査が必要で、今回はそこまで踏み込まず「F2は依然として未検証の
> 前提に依存している」という本項目の結論を追認するに留める。
...
> ✅🟡 **2026-08-25追記（loopが実ファイルで裏取り・8件中4件を確認）**:
> - **出血1**（係数表1枚化）: ✅解消済み。`ops/COEFFICIENTS.md`が既に単一係数表として存在し、
>   `TRUSTED_CLAUSE`是正（`f9aac30`）でroot_only除外も実装済み。
> - **出血2**（63.16全国統計バグ）: 🟡部分解消。①`isBotUserAgent`は`/api/stats/submit`に
>   実装済み（コード内コメントが本項目を直接参照）。③「全国」の語は
>   `src/app/api/stats/distribution/route.ts:45`にまだ残存（`'匿名統計（内申点・偏差値等の全国分布）'`）
>   ＝最も安価な是正案が未実施のまま。②④は未確認。
> - **出血6**（click-hop通過率）: 🟡部分解消。①href生存確認は`npm run check:affiliate-links`
>   として実装・運用中（`NODE_TLS_REJECT_UNAUTHORIZED=0`必須・月次目安で実行中）。
>   ②ホップ通過率の計測（D1/GA4イベント）は`src/lib/click-hop.ts`を確認したが**未実装のまま**。
>   ③外部referer付与の理由も未確認。
> - **出血8**（対立する結論の仲裁）: 主要な対立点(a)「placement 89箇所」は本セッションで
>   MONEY.md出血6/9を通じて**「直せ」側（是正）を実行し43/43箇所で完了**したため、この対立自体が
>   解消された。(b)「/hogosha最優先か」は`ops/DISTANCE.md`§7.2の疑義注記が既に扱っている
>   （DEADWIRE側の「スクレイパ」判定が優勢）。(c)(d)は未確認。
> - 出血3/4/5/7は👤ゲート必須（ASP実績・G1判断）または内容の設計判断（学校名クエリ受け皿・
>   /naishin-oru title）を要するため今回は手を付けず。
