...
いまやっていること     T-Y11F §11項目4(X-14・∞常駐)。freshness-queue.tsのgetStaleTop()
                      で最古県を機械選定→再検証(累計26県)。6日グループ進行中
                      (okinawa/iwate/miyazaki完了)。
今日やったこと         上記26県を再検証・現行方式は全て変更なし。naraで令和10年度
                      以降の内申点満点拡大計画を新発見(未反映・記録済み)。
次にやること           getStaleTop()を再実行し6日グループ(miyagi/kumamoto/gunma/
                      kagawa/kochi/saga/yamagata/yamanashi/kagoshima等)を続ける。
